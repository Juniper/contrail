...
sync_contrail_openstack_projects()
{
    cmd="source /var/lib/kolla/config_files/admin-openrc.sh && \
openstack project list -c ID -f value | \
python -c 'import sys,uuid; print \"\n\".join(str(uuid.UUID(proj_id.strip())) for proj_id in sys.stdin)' | \
xargs -I PROJ_ID curl $(uname -n):8082/project/PROJ_ID"

    docker exec kolla_toolbox sh -c "$cmd"
}
