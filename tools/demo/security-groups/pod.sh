...
kubectl run -i --tty "$1" --image=busybox --attach=false -l "name=$1" --namespace "atom-pink"
POD_NAME=$(kubectl get pod -l "name=$1" -o jsonpath="{.items[0].metadata.name}" --namespace "atom-pink")

echo "Waiting for container to start..."
kubectl rollout status deployment "$1" --namespace "atom-pink"
...
EOF
chmod +x echo.sh
kubectl cp echo.sh "$POD_NAME:/echo.sh" --namespace "atom-pink"

echo "Container running, pod name: $POD_NAME"
echo "Attach shell using 'kubectl attach -it $POD_NAME --namespace atom-pink'"
