...
// IntPoolPlugin is a plugin that allows defining int pools and (de)allocating values from them.
...
func (p *IntPoolPlugin) RegisterGRPCAPI(r apiserver.GRPCRouter) {
	r.RegisterService(&_IntPool_serviceDesc, p)
}
...
func (p *IntPoolPlugin) CreateIntPool(
	...
	if err := p.InTransactionDoer.DoInTransaction(ctx, func(ctx context.Context) error {
		return p.IntPoolAllocator.CreateIntPool(ctx, r.Pool, r.Start, r.End)
	}); err != nil {
		return nil, err
	}
...
func (p *IntPoolPlugin) GetIntOwner(
	...
	err = p.InTransactionDoer.DoInTransaction(ctx, func(ctx context.Context) error {
		var owner string
		owner, err = p.IntPoolAllocator.GetIntOwner(ctx, request.GetPool(), request.GetValue())
		if err != nil {
			return err
		}
...
func (p *IntPoolPlugin) DeleteIntPool(
	...
	if err := p.InTransactionDoer.DoInTransaction(ctx, func(ctx context.Context) error {
		return p.IntPoolAllocator.DeleteIntPool(ctx, r.Pool)
	}); err != nil {
		return nil, err
	}
	return &types.Empty{}, nil
...
func (p *IntPoolPlugin) AllocateInt(
	...
	if err := p.InTransactionDoer.DoInTransaction(ctx, func(ctx context.Context) error {
		var err error
		if v, err = p.IntPoolAllocator.AllocateInt(ctx, request.GetPool(), request.GetOwner()); err != nil {
...
func (p *IntPoolPlugin) SetInt(ctx context.Context, request *SetIntRequest) (*types.Empty, error) {
	if request.GetPool() == "" {
		err := errutil.ErrorBadRequest("Missing pool name for int-pool allocation")
		return nil, err
	}
	if err := p.InTransactionDoer.DoInTransaction(ctx, func(ctx context.Context) error {
		if err := p.IntPoolAllocator.SetInt(
