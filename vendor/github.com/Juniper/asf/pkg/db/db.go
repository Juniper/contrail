...
const (
	...
	dbDSNFormatPostgreSQL = "sslmode=disable user=%s password=%s host=%s dbname=%s port=%s"
)

// DB struct for base function.
type DB struct {
	db            *sql.DB
	Dialect       Dialect
...
// NewDB makes new base db instance.
...
func (db *DB) DoInTransaction(ctx context.Context, do func(context.Context) error) error {
...
func (db *DB) DoInTransactionWithOpts(
...
func (db *DB) DoWithoutConstraints(ctx context.Context, do func(context.Context) error) (err error) {
...
func (db *DB) disableConstraints() error {
...
func (db *DB) enableConstraints() error {
...
func (db *DB) Count(
	ctx context.Context, qb *QueryBuilder, spec *services.ListSpec,
...
func (db *DB) CheckPolicy(ctx context.Context, qb *QueryBuilder, uuid string) (err error) {
	tx := GetTransaction(ctx)
...
func (db *DB) CreateParentReference(
	ctx context.Context,
	obj childObject,
	qb *QueryBuilder,
...
func (db *DB) ListRows(ctx context.Context, schemaID string, spec *services.ListSpec) (*MapRows, error) {
...
// in Watcher.
func (db *DB) Dump(ctx context.Context) (DatabaseData, error) {
	result := DatabaseData{}
...
func (db *DB) dumpTable(ctx context.Context, schemaID string) (TableData, error) {
	var result TableData
	rows, err := db.ListRows(ctx, schemaID, &services.ListSpec{Detail: true})
	if err != nil {
		return nil, err
	}
	defer rows.Close()
