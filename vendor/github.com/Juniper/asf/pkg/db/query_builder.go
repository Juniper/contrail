...
// UUIDColumn is name of column that stores an object's UUID.
...
func newQueryContext(auth authContext, spec *services.ListSpec) *queryContext {
	if auth == nil {
...
func (qb *QueryBuilder) buildFilterQuery(ctx *queryContext) {
	...
	filters = services.AppendFilter(filters, UUIDColumn, spec.ObjectUUIDs...)
...
func (qb *QueryBuilder) ListQuery(auth authContext, spec *services.ListSpec) (string, Columns, []interface{}) {
...
func (qb *QueryBuilder) CountQuery(auth authContext, spec *services.ListSpec) (string, []interface{}) {
	ctx := newQueryContext(auth, spec)
