...
func (r *httpRouter) Add(method string, path string, h HandlerFunc, options ...RouteOption) {
	params := makeRouteParameters(options, method, path)
...
func (r *httpRouter) Group(prefix string, options ...RouteOption) {
	params := makeRouteParameters(options, "", prefix)
	r.applyRouteParameters(params, prefix)
	r.echo.Group(prefix, echoMiddleware(params.middleware)...)
}

func makeRouteParameters(options []RouteOption, method, path string) (params RouteParameters) {
	path = strings.TrimPrefix(strings.TrimSuffix(path, "/:id"), "/")
	params.homepageEntry = linkDetails{
		Path: path,
		Name: path,
		Rel:  string(ActionEndpoint),
	}
	if method != "" {
		params.homepageEntry.Method = &method
	}
...
func (r *httpRouter) applyRouteParameters(params RouteParameters, path string) {
	...
	r.homepage.Register(params.homepageEntry)
}
