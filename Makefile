...
SPECTACLE_OPTS=-1 -t $(dir $(CONTRAIL_APIDOC_PATH)) $(CONTRAIL_OPENAPI_PATH)

$(CONTRAIL_APIDOC_PATH): $(CONTRAIL_OPENAPI_PATH)
#ifeq (, $(shell which spectacle))
	$(info No spectacle in $(PATH), running docker.)
	docker run --rm -v $(PWD)/public:/public/ node:10.15.3-alpine sh -c \
		"ls /public; npm install --unsafe-perm -g spectacle-docs@1.0.7 && spectacle $(SPECTACLE_OPTS)"
#else
	#spectacle $(SPECTACLE_OPTS)
#endif
