...
docker_prepare: ## Prepare common data to generate Docker files (use target `docker` or `docker_config_api` instead)
	...
	mkdir -p $(BUILD_DIR)/docker/contrail_go/templates/ && cp pkg/command/cluster/templates/instances.tmpl $(BUILD_DIR)/docker/contrail_go/templates/
	mkdir -p $(BUILD_DIR)/docker/contrail_go/templates/ && cp pkg/command/cluster/templates/inventory.tmpl $(BUILD_DIR)/docker/contrail_go/templates/
