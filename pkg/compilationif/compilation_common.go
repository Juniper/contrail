...
func (service *CompilationService) handleEtcdMessages(ctx context.Context, oper int32, key, value string) error {
	...
	// TODO Make eventProducer not connect to ETCD
	eventProducer, err := etcd.NewEventProducer(processor)
	if err != nil {
		return errors.Wrap(err, "failed to create an ETCD event producer")
	}

	event, err := eventProducer.ParseEvent(oper, key, []byte(value))
	if err != nil {
