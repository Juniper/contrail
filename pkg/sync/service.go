...
func createWatcher(processor services.EventProcessor) (watchCloser, error) {
	...
	case basedb.DriverPostgreSQL:
		return createPostgreSQLWatcher(log.NewLogger("sync-psql-watcher"), rowSink, dbService, processor)
	case basedb.DriverMySQL:
		return createMySQLWatcher(log.NewLogger("sync-mysql-watcher"), rowSink)
	default:
