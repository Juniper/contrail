...
func createPostgreSQLWatcher(log *logrus.Entry, sink replication.RowSink, dbService *db.Service) (watchCloser, error) {
	...
	return replication.NewPostgresWatcher(conf, dbService, replConn, handler.Handle, &services.ServiceEventProcessor{&services.BaseService{}}) // TODO(Michal): replace with ETCD event processor
}
