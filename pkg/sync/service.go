...
// NewEventProducer creates an event producer that feeds EventProcessor with changes coming
...
func NewEventProducer(
	...
	options := []replication.WatcherOption{replication.Slot(id)}
	if !viper.GetBool("sync.dump") {
		options = append(options, replication.NoDump())
	}
	return replication.NewPostgresWatcher(handler, dbService, options...)
}
