...
type RowSink interface {
	Create(ctx context.Context, schemaID string, pk string, data map[string]interface{}) error
	Update(ctx context.Context, schemaID string, pk string, data map[string]interface{}) error
	Delete(ctx context.Context, schemaID string, pk string, data map[string]interface{}) error
}
...
func (h *PgoutputEventHandler) Handle(ctx context.Context, msg pgoutput.Message) error {
	...
	case pgoutput.Insert:
		...
		return h.sink.Create(ctx, tableName, pk.String(), data)
	case pgoutput.Update:
		...
		return h.sink.Update(ctx, tableName, pk.String(), data)
	case pgoutput.Delete:
		...
		return h.sink.Delete(ctx, tableName, pk.String(), data)
	}
	return nil
}
...
func (h *CanalEventHandler) handleCreate(rows [][]interface{}, t *schema.Table) error {
	for _, row := range rows {
		...
		if err := h.sink.Create(context.Background(), t.Name, pk, kvs); err != nil {
			return err
		}
	}
	return nil
...
func (h *CanalEventHandler) handleUpdate(rows [][]interface{}, t *schema.Table) error {
	for _, row := range rows {
		...
		if err := h.sink.Update(context.Background(), t.Name, pk, kvs); err != nil {
			return err
		}
	}
	return nil
...
func (h *CanalEventHandler) handleDelete(rows [][]interface{}, t *schema.Table) error {
	for _, row := range rows {
		...
		if err := h.sink.Delete(context.Background(), t.Name, pk, kvs); err != nil {
			return err
		}
	}
	return nil
