...
func (h *PgoutputHandler) updateFQNameCache(event *services.Event) {
	...
	case services.UpdateRequest:
		r := request.GetResource()
		h.idToFQName[r.GetUUID()] = r.GetFQName()
	case services.DeleteRequest:
		delete(h.idToFQName, request.GetID())
	}
}

func (h *PgoutputHandler) sanitizeCreateRefEvent(event *services.Event) error {
