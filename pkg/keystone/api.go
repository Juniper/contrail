...
func rawAuthURLs(urls []string) []string {
	var u []string
...
func (k *Keystone) fetchClusterToken(c echo.Context, sar keystone.AuthRequest, scope string, endpointKey string) error {
	...
	if sar.GetIdentity().Password != nil {
		...
		if !sar.GetIdentity().Password.User.HasCredential() {
			...
			sar.SetCredential(k.endpointStore.ReadUsername(scope, endpointKey), k.endpointStore.ReadPassword(scope, endpointKey))
		}
	}
...
func getKeystoneEndpoints(clusterID string, es EndpointStore) []string {
	...
	return es.ReadEndpointURLs(PrivateURLScope, path.Join("/proxy", clusterID, keystoneService, PrivateURLScope))
}
