...
func Init(e *echo.Echo, es *endpoint.Store) (*Keystone, error) {
	...
	e.POST("/keystone/v3/users", keystone.CreateUserAPI)
...
func (k *Keystone) setAssignment(clusterID string) (configEndpoint string, err error) {
	...
	err = apiAssignment.Init(
		configEndpoint, k.staticAssignment.Users)
	if err != nil {
		logrus.Error(err)
...
//CreateUserAPI is an API handler to create user.
func (k *Keystone) CreateUserAPI(c echo.Context) error {
	clusterID := c.Request().Header.Get(xClusterIDKey)
	if ke := getKeystoneEndpoints(clusterID, k.endpointStore); len(ke) > 0 {
		return k.proxyRequest(c, ke)
	}
	...
	providedName := c.Param("name")
	for _, user := range k.Assignment.ListUsers() {
		if user.Name == providedName {
			return c.JSON(http.StatusConflict, nil)
		}
	}
	//TODO: change status code to created?
...
func (k *Keystone) ListUsersAPI(c echo.Context) error {
	...
	providedName := c.QueryParam("name")
	if providedName != "" {
		//get user by name
		for _, user := range k.Assignment.ListUsers() {
			if user.Name == providedName {
				return c.JSON(http.StatusOK, &UserListResponse{
					Users: []*keystone.User{user},
				})
			}
		}
		return c.JSON(http.StatusOK, &UserListResponse{Users: nil})
	}
