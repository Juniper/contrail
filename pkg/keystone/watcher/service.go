...
// Watch starts listening for project changes in keystone
...
func getClient() *client.HTTP {
	...
	log.Infof("Making keystone watcher: %+v with Scope %+v (Domain: %+v; Project: %+v)",
		client, client.Scope, client.Scope.Domain, client.Scope.Project)
	return client
}
