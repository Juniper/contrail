...
// MapApplier interface for defining custom map applying logic.
type MapApplier interface {
	ApplyMap(map[string]interface{}) error
}

// ApplyMap applies map onto interface which is a struct.
func ApplyMap(m map[string]interface{}, o interface{}) error {
	if len(m) == 0 {
		return nil
	}
	sv, err := getSettableStructValue(o)
	if err != nil {
		return err
	}
	applied, err := useMapApplierIfAvailable(sv, m)
	if err != nil {
		return err
	}
	if applied {
...
// nolint: gocyclo
func apply(v reflect.Value, i interface{}) error {
	switch v.Kind() {
