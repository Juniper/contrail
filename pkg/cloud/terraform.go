...
func updateTopology(c *Cloud, providers []string, clusterUUID string) error {
	...
	return player.StartExecuteAndRemove(
		c.ctx, imageRef, registry.Username, registry.Password, volumes, "/root/contrail-multi-cloud",
...
func (c *Cloud) getMultiCloudImageRef(clusterUUID string) (string, error) {
	...
	tagVersion := clusterResp.ContrailCluster.ContrailConfiguration.GetValue("CONTRAIL_CONTAINER_TAG")
	return registry + "/" + MultiCloudContainer + ":" + tagVersion, nil
}
