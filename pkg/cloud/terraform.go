...
func updateTopology(c *Cloud, reporter *report.Reporter) error {
	...
	if err := osutil.ExecCmdAndWait(reporter, cmd, args, []string{}, workDir); err != nil {
		return err
	}
	args = []string{"topology", "build", "--retry", "5"}
	return osutil.ExecCmdAndWait(reporter, cmd, args, []string{}, workDir)
}
