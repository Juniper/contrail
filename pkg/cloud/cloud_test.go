...
func assertYAMLFileEqual(t *testing.T, expectedFilePath, actualFilePath string, msg string) bool {
	var actualYAML interface{}
	err := fileutil.LoadFile(actualFilePath, &actualYAML)
	if err != nil {
		return assert.Fail(t, fmt.Sprintf("Failed read yaml from %s: %v", actualFilePath, err), msg)
	}

	var expectedYAML interface{}
	err = fileutil.LoadFile(expectedFilePath, &expectedYAML)
	if err != nil {
		return assert.Fail(t, fmt.Sprintf("Failed read yaml from %s: %v", expectedFilePath, err), msg)
	}

	if !testutil.AssertEqual(t, expectedYAML, actualYAML) {
		return assert.Fail(t, fmt.Sprintf("YAML files %s and %s are not equal", expectedFilePath, actualFilePath), msg)
	}
	return true
}
