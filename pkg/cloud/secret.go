		...
		for _, cred := range d.credentials {
			for _, keyPairRef := range cred.KeypairRefs {
				...
				if keypair.SSHKeyPath == "" && keypair.SSHPublicKey == "" {
					err = createSSHKey(cloudID)
					if err != nil {
						s.log.Errorf("error while creating ssh keys: %v", err)
						return nil, err
					}
					// get pub key content
					pubKey, err := common.GetContent("file://" + getCloudSSHKeyPath(cloudID, defaultSSHPubKey))
					if err != nil {
						return nil, err
					}
					// update keypair
					keypair.SSHPublicKey = string(pubKey)
					keypair.SSHKeyPath = getCloudSSHKeyPath(cloudID, defaultSSHPvtKey)
...
func createSSHKey(cloudID string) error {
	...
	return common.WriteToFile(getCloudSSHKeyPath(cloudID, defaultSSHPubKey),
