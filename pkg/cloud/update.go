...
func updateInstanceIP(ctx context.Context,
	instance *instanceData, tfState terraformState) error {
	privateIP, err := getPrivateIPFromTFState(instance, tfState)
	if err != nil {
		return err
	}
...
func updateGatewayInstanceIP(ctx context.Context,
	...
	publicIP, err := tfState.GetKeyValue(fmt.Sprintf("%s.public_ip", instance.info.Hostname))
	if err != nil {
		return err
	}
...
func getPrivateIPFromTFState(instance *instanceData, tfState terraformState) (string, error) {
	return tfState.GetKeyValue(fmt.Sprintf("%s.private_ip", instance.info.Hostname))
}
