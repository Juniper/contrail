...
func (c *Cloud) updateNodeIP(ctx context.Context, data *Data) error {
	tfState, err := readStateFile(GetTFStateFile(c.config.CloudID))
	if err != nil {
		return err
	}
	for _, instance := range data.instances {
		err := updateIP(ctx, instance, tfState)
		if err != nil {
			return err
		}
	}
...
func updateIP(ctx context.Context, instance *instanceData, tfState *tf.State) error {

	gwExists := gwRoleExists(instance)

	privateIP, err := getIPFromTfState(tfState,
		fmt.Sprintf("private_ip.%s",
			instance.info.Name))
