...
func (c *onPremCloud) MarshalAndSave(topoFile string) error {
	marshaled, err := yaml.Marshal([]*onPremCloud{c})
	if err != nil {
...
func (c *onPremCloud) getTags(cloudUUID string) ([]*models.Tag, error) {
	...
	if err = validateOnPremResourceUniqueness(
		"CloudRegions", len(provObj.CloudProvider.CloudRegions), c.log,
	); err != nil {
		return nil, err
	}
	...
	if err = validateOnPremResourceUniqueness(
		"VirtualClouds", len(regionObj.CloudRegion.VirtualClouds), c.log,
	); err != nil {
		return nil, err
	}
...
func (i *privateInstance) fillVrouterDetails(vrouterNodes []*models.ContrailVrouterNode) error {
	...
	if i.Gateway == "" {
		return fmt.Errorf(
			"default gateway is neither set for vrouter_node uuid: %s nor for contrail_cluster uuid: %s",
...
func (i *privateInstance) fillSubNetDetails(subnets []*models.NodeCloudPrivateSubnetRef) error {
	...
	for _, s := range subnets {
		resp, err := i.cli.GetCloudPrivateSubnet(
			i.ctx, &services.GetCloudPrivateSubnetRequest{ID: s.UUID},
		)
		if err != nil {
			return err
		}
		privateSubnets = append(privateSubnets, resp.CloudPrivateSubnet.CidrBlock)
	}
