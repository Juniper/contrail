...
func tfPrivIPKey(instance *instanceData) string {
	return fmt.Sprintf("%s.private_ip", instance.info.Hostname)
}

func tfPublicIPKey(instance *instanceData) string {
	return fmt.Sprintf("%s.public_ip", instance.info.Hostname)
}
...
func (m updateNodePrivatePortMatcher) Matches(x interface{}) bool {
	...
	return ok &&
		...
		request.Node.Ports[0].Name == "private" &&
		request.Node.Ports[0].ParentType == portParentType &&
...
func (m updateNodePrivatePortMatcher) String() string {
	return fmt.Sprintf(
		"is equal to UpdateNodeRequest with Node.Hostname: %v , Node.Port{Name: %v, ParentType: %v ,ParentUUID:%v, IPAddress: %v",
		m.expectedHostname, "private", portParentType, m.expectedUUID, m.expectedIP)
}
...
func (m updatePrivatePortMatcher) Matches(x interface{}) bool {
	...
	return ok &&
		request.Port.Name == "private" &&
		request.Port.ParentType == portParentType &&
...
func (m updatePrivatePortMatcher) String() string {
	return fmt.Sprintf("is equal to UpdateNodeRequest with Port{Name: %v, ParentType: %v ,ParentUUID:%v, IPAddress: %v",
		"private", portParentType, m.expectedUUID, m.expectedIP)
}
...
func (s *updateIPDetailsTestSuite) TestUpdateNotGatewayInstancePublicIP() {
	...
	s.tfStateMock.EXPECT().GetKeyValue(tfPrivIPKey(instance)).Return(privateIP, nil)
...
func (s *updateIPDetailsTestSuite) TestUpdateGatewayInstancePublicIPAndCreatePrivatePortWhenPortIsNotPresent() {
	...
	s.tfStateMock.EXPECT().GetKeyValue(tfPrivIPKey(instance)).Return(privateIP, nil)
	s.tfStateMock.EXPECT().GetKeyValue(tfPublicIPKey(instance)).Return(publicIP, nil)
