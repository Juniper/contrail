...
func (c *Cloud) delete() error {
	...
	switch cloudType {
	case azure:
		azFilePath := filepath.Join(c.getHomeDir(), azTFJson)
		if _, err = os.Stat(azFilePath); err == nil {
			files = append(files, azFilePath)
		}
	case aws:
		awsFilePath := filepath.Join(c.getHomeDir(), awsTFJson)
		if _, err = os.Stat(awsFilePath); err == nil {
			files = append(files, awsFilePath)
		}
	}

	// add topo file to list
	files = append(files, topo.getTopoFile())
	//add secret file to list
	files = append(files, secret.getSecretFile())

	err = deleteFiles(c.reporter, files)
	if err != nil {
		return err
	}
