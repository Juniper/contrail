...
func validateOnPremResourceUniqueness(resourceName string, count int) error {
	if count == 0 {
...
func getTags(ctx context.Context, cli *client.HTTP, log *logrus.Entry, cloudUUID string) ([]*models.Tag, error) {
	...
	if err = validateOnPremResourceUniqueness("CloudProviders", len(cloudObj.Cloud.CloudProviders)); err != nil {
		return nil, err
	}
	...
	if err = validateOnPremResourceUniqueness("CloudRegions", len(provObj.CloudProvider.CloudRegions)); err != nil {
		return nil, err
	}
	...
	if err = validateOnPremResourceUniqueness("VirtualClouds", len(regionObj.CloudRegion.VirtualClouds)); err != nil {
		return nil, err
	}
...
func (i *privateInstance) fill(ctx context.Context, cli *client.HTTP, log *logrus.Entry, instanceUUID string) error {
	...
	if err = i.fillGWDetails(
		ctx, cli, ii.Node.ContrailMulticloudGWNodeBackRefs, ii.Node.CloudPrivateSubnetRefs); err != nil {
		return err
	}
...
func (i *privateInstance) fillCredentialDetails(
	ctx context.Context, cli *client.HTTP, credentialRefs []*models.NodeCredentialRef,
) error {
