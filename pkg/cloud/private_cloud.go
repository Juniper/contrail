...
func (c *onPremCloud) getTags(cloudUUID string) ([]*models.Tag, error) {
	...
	if err = validateOnPremResourceUniqueness("CloudProviders", len(cloudObj.Cloud.CloudProviders), c.log); err != nil {
		return nil, err
	}
	...
	if err = validateOnPremResourceUniqueness("CloudRegions", len(provObj.CloudProvider.CloudRegions), c.log); err != nil {
		return nil, err
	}
	...
	if err = validateOnPremResourceUniqueness("VirtualClouds", len(regionObj.CloudRegion.VirtualClouds), c.log); err != nil {
		return nil, err
	}
...
func (i *privateInstance) fillGWDetails(
	...
	gwNodeResp, err := i.cli.GetContrailMulticloudGWNode(i.ctx,
...
func (i *privateInstance) fillVrouterDetails(vrouterNodes []*models.ContrailVrouterNode) error {
	...
	if len(vrouterNodes) > 1 {
		i.log.Warnf("onPrem node has invalid number of VrouterNodes: %d, only first will be considered", len(vrouterNodes))
	}

	vrouterNodeUUID := vrouterNodes[0].UUID

	vrouterNodeResponse, err := i.cli.GetContrailVrouterNode(i.ctx,
	...
	contrailClusterResponse, err := i.cli.GetContrailCluster(i.ctx,
...
func (i *privateInstance) fillSubNetDetails(subnets []*models.NodeCloudPrivateSubnetRef) error {
	...
	subnetRef := subnets[0]
	privateSubnetResp, err := i.cli.GetCloudPrivateSubnet(i.ctx,
