...
func getTags(ctx context.Context, cli *client.HTTP, log *logrus.Entry, cloudUUID string) ([]*models.Tag, error) {
	...
	cloudProvidersSize := len(cloudObj.Cloud.CloudProviders)

	if cloudProvidersSize == 0 {
		return nil, errors.New("onPrem cloud has no CloudProviders")
	}
	if cloudProvidersSize > 1 {
		log.Warnf("onPrem cloud has wrong number of CloudProviders: %d, only first will be considered", cloudProvidersSize)
	}
	...
	cloudRegionsSize := len(provObj.CloudProvider.CloudRegions)

	if cloudRegionsSize == 0 {
		return nil, errors.New("onPrem cloud has no CloudRegions")
	}
	if cloudRegionsSize > 1 {
		log.Warnf("onPrem cloud has wrong number of CloudRegions: %d, only first will be considered", cloudRegionsSize)
	}
	...
	virtualCloudsSize := len(regionObj.CloudRegion.VirtualClouds)

	if virtualCloudsSize == 0 {
		return nil, errors.New("onPrem cloud has no VirtualClouds")
	}
	if virtualCloudsSize > 1 {
		log.Warnf("onPrem cloud has wrong number of VirtualClouds: %d, only first will be considered", virtualCloudsSize)
	}
...
func (c *onPremCloud) createInstances(ctx context.Context, cli *client.HTTP, log *logrus.Entry, cloudUUID string, tags []*models.Tag) error {
...
func (i *privateInstance) fill(ctx context.Context, cli *client.HTTP, log *logrus.Entry, instanceUUID string) error {
	...
	if err = i.fillCredentialDetails(ctx, cli, ii.Node.CredentialRefs); err != nil {
		return err
	}
	if err = i.fillGWDetails(ctx, cli, ii.Node.ContrailMulticloudGWNodeBackRefs, ii.Node.CloudPrivateSubnetRefs); err != nil {
		return err
	}
	if err = i.fillVrouterDetails(ctx, cli, ii.Node.ContrailVrouterNodeBackRefs); err != nil {
		return err
	}
	i.fillRoles(ii.Node)
...
func (i *privateInstance) fillCredentialDetails(ctx context.Context, cli *client.HTTP, credentialRefs []*models.NodeCredentialRef) error {
...
func (i *privateInstance) fillGWDetails(ctx context.Context, cli *client.HTTP, gwNodes []*models.ContrailMulticloudGWNode, subnets []*models.NodeCloudPrivateSubnetRef) error {
	if len(gwNodes) == 0 {
		return nil
	}
	...
	if err = i.fillSubNetDetails(ctx, cli, subnets); err != nil {
		return err
	}
...
func (i *privateInstance) fillVrouterDetails(ctx context.Context, cli *client.HTTP, vrouterNodes []*models.ContrailVrouterNode) error {
...
func (i *privateInstance) fillSubNetDetails(ctx context.Context, cli *client.HTTP, subnets []*models.NodeCloudPrivateSubnetRef) error {
...
func (c *onPremCloud) createTors(ctx context.Context, cli *client.HTTP, log *logrus.Entry, cloudUUID string, tags []*models.Tag) error {
