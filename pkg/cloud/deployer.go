...
func (d Deployer) DestroyTopology(providers Providers) error {
	if err := d.generateEmptyTopology(); err != nil {
		return err
	}
	return d.UpdateTopology(providers)
}

func (d Deployer) generateEmptyTopology() error {
	return fileutil.WriteToFile(d.config.TopologyFile, []byte{}, defaultRWOnlyPerm)
}
