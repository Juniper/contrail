...
func (d *Data) updateAccounts() error {
	for _, account := range d.info.CloudAccounts {
...
func (d *Data) updateRegions() error {
	...
	for _, region := range d.info.CloudRegions {
...
func (d *Data) isCloudUpdated() bool {
	...
	status := d.info.ProvisioningState
	if d.cloud.config.Action == updateAction && (status == "NOSTATE" || status == "") {
		return false
	}
	return true
}
