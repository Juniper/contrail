...
func (d *Data) isCloudCreated() bool {
	...
	status := d.cloudInfo.ProvisioningState
	if d.cloud.config.Action == "create" && (status == "NOSTATE" || status == "") {
		return false
	}
	d.cloud.log.Infof("Cloud %s already provisioned, STATE: %s", d.cloudInfo.UUID, status)
	return true
}
