...
func (t *torData) updateBgpRouter() error {
	for _, bgpRouterRef := range t.info.BgpRouterBackRefs {
		bgpRouterResp, err := t.client.GetBgpRouter(t.ctx,
			&services.GetBgpRouterRequest{
				ID: bgpRouterRef.UUID,
			},
		)
		if err != nil {
			return err
		...
		t.bgpRouters = append(t.bgpRouters, bgpRouterResp.GetBgpRouter())
	}
	if len(t.bgpRouters) == 0 {
		return errors.New("tor does not have a bgp-router ref")
	}
	return nil
}
...
func (v *virtualCloudData) newTorInstance(tor *models.PhysicalRouter) (*torData, error) {
	data := v.parentRegion.parentProvider.parentCloud
	if !data.isCloudPrivate() {
		return nil, nil
	}
...
func (v *virtualCloudData) updateTorInstances() error {
	...
	tors, err := v.getTorInstancesWithTag(v.info.TagRefs)
	if err != nil {
		return err
	}

	for _, tor := range tors {
		newI, err := v.newTorInstance(tor)
		if err != nil {
			return err
		}
...
func (v *virtualCloudData) getTorInstancesWithTag(tagRefs []*models.VirtualCloudTagRef) ([]*models.PhysicalRouter, error) {
