...
func (c *publicCloud) MarshalAndSave(destination string) error {
	marshaled, err := yaml.Marshal(c.Providers)
	if err != nil {
...
type region interface {
	AddVirtualCloud(v *virtualCloud)
...
func (r *azureRegion) Fill(ctx context.Context, cli *client.HTTP, regionUUID string, providerType string) error {
	...
	for _, v := range regionObj.CloudRegion.VirtualClouds {
		...
		r.AddVirtualCloud(vv)
	}
	return nil
}

func (r *azureRegion) AddVirtualCloud(v *virtualCloud) {
...
func (r *awsGCPRegion) AddVirtualCloud(v *virtualCloud) {
