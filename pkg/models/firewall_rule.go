...
// Validate validates firewall rule resource:
// - it checks for existence of service and serviceGroupRefs property
// - checks scope of firewallRule refs that global scoped firewall rule cannot reference scoped resources
// - checks protocol property
func (fr *FirewallRule) Validate() error {

	if err := fr.checkAssociatedRefsInSameScope(); err != nil {
		return err
	}

	if err := fr.checkServiceProperties(); err != nil {
		return err
	}

	if _, err := fr.getProtocolID(); err != nil {
		return err
	}
