...
func (fr *FirewallRule) CheckServiceProperties(databaseFR *FirewallRule, fm *types.FieldMask) error {
	...
	if fm != nil && service == nil && !basemodels.FieldMaskContains(fm, FirewallRuleFieldService) {
...
func (fr *FirewallRule) SetProtocolID(fm *types.FieldMask) error {
	if fm != nil &&
		!basemodels.FieldMaskContains(
			fm,
			FirewallRuleFieldService,
			FirewallServiceTypeFieldProtocol,
		) {
		return nil
	}
	...
	if fm != nil && !basemodels.FieldMaskContains(fm, FirewallRuleFieldService, FirewallServiceTypeFieldProtocolID) {
		fm.Paths = append(
			fm.Paths,
			basemodels.JoinPath(FirewallRuleFieldService, FirewallServiceTypeFieldProtocolID),
		)
	}
	return err
}
