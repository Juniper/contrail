...
func TestCheckIfSubnetParamsAreValid(t *testing.T) {
	...
	wrongAllocPools := []*AllocationPoolType{{Start: "10.0.0.5", End: "10.0.0.6"}, {Start: "11.0.0.5", End: "11.0.0.6"}}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := tt.ipamSubnet.CheckIfSubnetParamsAreValid()
			if tt.fails {
				assert.Error(t, err)
				return
			}
...
func TestIpamSubnetIsValid(t *testing.T) {
	subnet := SubnetType{IPPrefix: "10.0.0.0", IPPrefixLen: 24}
