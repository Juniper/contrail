...
// TODO(Daniel): move to pkg/apisrv/client
type CLI struct {
	client.HTTP
}
...
func NewCLIByViper() (*CLI, error) {
	return NewCLI(&client.HTTPConfig{
...
func NewCLI(c *client.HTTPConfig) (*CLI, error) {
	client := client.NewHTTP(c)
...
func (c *CLI) ListResources(schemaID string) (string, error) {
	if schemaID == "" {
...
func (c *CLI) showHelp(schemaID string, template string) (string, error) {
	serverSchema := filepath.Join(viper.GetString("client.schema_root"), serverSchemaFile)
