...
func startServer() {
	...
	plugins := []asfapiserver.APIPlugin{
		...
		&intpool.IntPoolPlugin{Allocator: dbService, InTransactionDoer: dbService},
		&plugin.UserAgentKVPlugin{UserAgentKVService: dbService},
	...
	if viper.GetBool("server.enable_vnc_neutron") {
		plugins = append(plugins, &neutron.Server{
			...
			UserAgentKV:       &plugin.UserAgentKVPlugin{UserAgentKVService: dbService},
