...
func testCreateProjectAndSecurityGroup(hc *integration.HTTPAPIClient, ec *integrationetcd.EtcdClient) func(t *testing.T) {
	return func(t *testing.T) {
		...
		collectAPSEvs := ec.WatchKeyN(
			integrationetcd.JSONEtcdKey(integrationetcd.ApplicationPolicySetSchemaID, ""), expectedAPSCount, wTime, clientv3.WithPrefix(),
		)
		defer collectAPSEvs()

		collectACLEvs := ec.WatchKeyN(
			integrationetcd.JSONEtcdKey(integrationetcd.AccessControlListSchemaID, ""), expectedACLCount, wTime, clientv3.WithPrefix(),
		)
		defer collectACLEvs()

		project := loadProject(t, demoProjectRequestPath)
		collectProjectEvs := ec.WatchKeyN(integrationetcd.JSONEtcdKey(integrationetcd.ProjectSchemaID, project.UUID), expectedProjectCount, wTime)
		...
		collectSGEvs := ec.WatchKeyN(
			integrationetcd.JSONEtcdKey(integrationetcd.SecurityGroupSchemaID, ""), expectedSGCount, wTime, clientv3.WithPrefix(),
		)
		defer collectSGEvs()
