...
func TestSyncSynchronizesPostgresDataToEtcdUsingJSONStorage(t *testing.T) {
	...
	wrRed := retrieveWatchResponse(t, vnRedWatch, redCtx)
	wrGreen := retrieveWatchResponse(t, vnGreenWatch, greenCtx)
	wrBlue := retrieveWatchResponse(t, vnBlueWatch, blueCtx)
...
func retrieveWatchResponse(t *testing.T, watch clientv3.WatchChan, ctx context.Context) *clientv3.WatchResponse {
	wr := <-watch
