...
func TestSyncSynchronizesPostgresDataToEtcdUsingJSONStorage(t *testing.T) {
	...
	vnRedWatch, redCtx, redCancel := watchVirtualNetworkInEtcd(ec, vnRedUUID)
	defer redCancel()
	vnGreenWatch, greenCtx, greenCancel := watchVirtualNetworkInEtcd(ec, vnGreenUUID)
	defer greenCancel()
	vnBlueWatch, blueCtx, blueCancel := watchVirtualNetworkInEtcd(ec, vnBlueUUID)
	defer blueCancel()
