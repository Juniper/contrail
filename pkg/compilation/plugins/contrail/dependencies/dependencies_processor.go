...
func (d *DependencyProcessor) canAdd(key string, obj interface{}) bool {
	if objMap, ok := d.resources.Load(key); ok {
		_, found := objMap.(*sync.Map).Load(d.getUUID(obj))
		if found {
...
func (d *DependencyProcessor) getCachedObject(objTypeStr, uuid string) interface{} {
	intent, err := d.cache.Load(objTypeStr, uuid)
	if err != nil {
		log.Errorf("Failed to get intent from cache. type: %s, uuid: %s", objTypeStr, uuid)
		return nil
	}
	return intent
}
...
func (d *DependencyProcessor) Evaluate(obj interface{}, objTypeStr, fromTypeStr string) { // nolint: gocyclo
	log.Infof("Evaluating: Type: %s, UUID: %s, From: %s", objTypeStr, d.getUUID(obj), fromTypeStr)
