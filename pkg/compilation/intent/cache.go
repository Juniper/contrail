...
func (c *Cache) LoadByFQName(typeName, fqName []string) (Intent, error) {
...
func (s *intentStore) loadByFQName(typeName, fqName []string) (Intent, bool) {
	s.RLock()
	defer s.RUnlock()
...
func (s *intentStore) load(typeName, uuid string) (Intent, bool) {
	s.RLock()
	defer s.RUnlock()
	return s.loadIntents(typeName).idToIntent[uuid]
}
...
func (s *intentStore) store(typeName, intent Intent) {
	s.Lock()
	defer s.Unlock()
	is := s.loadIntents(typeName)
...
func (s *intentStore) debug() {
	...
	for t, is := range s.typeToIntents {
		for fqname, uuid := range fqNameToID {
