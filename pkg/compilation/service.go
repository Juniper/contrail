...
func (ics *IntentCompilationService) handleEtcdMessage(ctx context.Context, oper int32, key, value string) {
	...
	if err != nil {
		logrus.WithFields(messageFields).WithField(logrus.ErrorKey, fmt.Sprintf("%+v", err)).Error("failed to parse ETCD event")
	}
	...
	if err != nil {
		ics.log.WithFields(messageFields).WithField(logrus.ErrorKey, fmt.Sprintf("%+v", err)).Error("Failed to handle etcd message")
	}
}
