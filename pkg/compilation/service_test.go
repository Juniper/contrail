...
func TestIntentCompilationSeviceConcurrency(t *testing.T) {
	...
	// store some messages in etcd first
	firstResp, err := etcdClient.Put(context.Background(), "/"+testEtcdPath+"/test", "value")
	require.NoError(t, err)

	secondResp, err := etcdClient.Put(context.Background(), "/"+testEtcdPath+"/test", "another value")
	require.NoError(t, err)

	_, err = etcdClient.Put(context.Background(), "/"+testEtcdPath+"/test", "final irrelevant message that should be read but not handled")
	require.NoError(t, err)
