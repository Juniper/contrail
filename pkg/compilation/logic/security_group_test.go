...
func TestPolicyRulesToACLRulesUsesSecurityGroupID(t *testing.T) {
	...
	assert.Equal(t, []*models.AclRuleType{aclRule},
		securityGroupToACLRules(securityGroup))
}
...
func TestSplitACLRules(t *testing.T) {
	realIngressRules := []*models.AclRuleType{
		{
			MatchCondition: &models.MatchConditionType{
				SRCAddress: &models.AddressType{
					SecurityGroup: "8000002",
	...
	realEgressRules := []*models.AclRuleType{
		{
			MatchCondition: &models.MatchConditionType{
				SRCAddress: &models.AddressType{},
				DSTAddress: allV4Addresses(),
	...
	testCases := []struct {
		name                                   string
		input, expectedIngress, expectedEgress []*models.AclRuleType
	}{
		{
			name:            "demo rules",
			input:           append(append([]*models.AclRuleType{}, realIngressRules...), realEgressRules...),
			expectedIngress: realIngressRules,
			expectedEgress:  realEgressRules,
		},
		{
			name: "both local",
			input: []*models.AclRuleType{
				{
					MatchCondition: &models.MatchConditionType{
						SRCAddress: &models.AddressType{},
						DSTAddress: &models.AddressType{},
			...
			expectedIngress: []*models.AclRuleType{
				{
					MatchCondition: &models.MatchConditionType{
						SRCAddress: &models.AddressType{},
						DSTAddress: &models.AddressType{},
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			ingress, egress := splitACLRules(tt.input)
			assert.Equal(t, tt.expectedIngress, ingress, "ingress rules don't match")
			assert.Equal(t, tt.expectedEgress, egress, "egress rules don't match")
		})
	}
}
...
func TestIsLocal(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "security group (by numeric ID) addresses",
			address: &models.AddressType{
				SecurityGroup: "8000002",
			},
			is: false,
