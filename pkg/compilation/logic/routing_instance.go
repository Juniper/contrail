...
// It is based on __init__ method from class RoutingInstanceST in Python config_db.py
// schema_transformer
func (s *Service) CreateRoutingInstance(
	...
	if err := s.locateRouteTarget(ctx, request); err != nil {
		return nil, err
	}
...
// This method is based on locate_route_target method from class RoutingInstanceST
// written in schema transformer - a part of python contrail
// It is just a huge TODO method which implements code necessary to run demo.
func (s *Service) locateRouteTarget(
	ctx context.Context, request *services.CreateRoutingInstanceRequest,
) error {
	// TODO: Check if there is any old route target connected to this
	// routing instance
	...
	// TODO: It should be checked in DB cache if this RT is not created already first
	_, err := s.api.CreateRouteTarget(ctx, &services.CreateRouteTargetRequest{
	...
	if request.RoutingInstance.GetRoutingInstanceIsDefault() {
		// TODO: Create Instance Target Type for both import and export
	} else if false { // TODO: It should be done when Virtual Network attached to RI requires external RT
		// TODO: Create Instance Target Type for export
	} else {
		// TODO: Instance Target Type should be nil
	}

	// TODO: check if VirtualNetwork parent for RI exists in DB cache
	// If not throw an error
	_ = request.RoutingInstance.GetParentUUID
	if false {
		return errors.Errorf("parent VN not found for RI: %v", request.RoutingInstance.Name)
	}
