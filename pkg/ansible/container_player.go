...
func (p *ContainerPlayer) pullAndStart(
	...
	volumes, tmpfsVolumes []mount.Mount, forceVolumes bool,
) (string, error) {
	...
	if !isFound {
		containerName, err = p.createRunningContainer(ctx, imageRef, volumes, tmpfsVolumes)
...
func validateVolumesSourcePaths(volumes []mount.Mount) error {
	...
	if len(fileErrors) != 0 {
		errors.New(strings.Join(fileErrors, ", "))
	}
	return nil
}
...
func (p *ContainerPlayer) StartExecuteAndRemove(
	ctx context.Context, imageRef string, imageRefUsername string, imageRefPassword string, volumes, tmpfsVolumes []Volume,
	...
	containerName, err := p.pullAndStart(
		ctx, imageRef, imageRefUsername, imageRefPassword, workingDirectory, toMounts(volumes), toMounts(tmpfsVolumes), true,
	)
	if err != nil {
		return errors.Wrap(err, "container couldn't be started")
	}
...
func (p *ContainerPlayer) createRunningContainer(
	ctx context.Context, imageRef string, mounts, tmpfsMounts []mount.Mount,
) (string, error) {
