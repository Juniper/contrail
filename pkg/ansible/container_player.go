...
func (p *ContainerPlayer) pullAndStart(
	ctx context.Context, imageRef, imageRefUsername, imageRefPassword string, volumes []mount.Mount, forceVolumes bool,
) (string, error) {
	if err := p.pullImage(ctx, imageRef, imageRefUsername, imageRefPassword); err != nil {
		return "", errors.Wrap(err, "image pulling failed")
	}
	containerName, err := p.startContainer(ctx, imageRef, volumes, forceVolumes)
	if err != nil {
...
func (p *ContainerPlayer) startContainer(
	ctx context.Context, imageRef string, volumes []mount.Mount, forceVolumes bool,
) (string, error) {
	...
	return p.createRunningContainer(ctx, imageRef, volumes)
}
...
func (p *ContainerPlayer) StartExecuteAndRemove(ctx context.Context, cp ContainerExecParameters) (err error) {
	...
	containerName, err := p.pullAndStart(
		ctx, cp.ImageRef, cp.ImageRefUsername, cp.ImageRefPassword, toMounts(cp.Volumes), true,
	)
	if err != nil {
		return errors.Wrap(err, "container couldn't be started")
	}
