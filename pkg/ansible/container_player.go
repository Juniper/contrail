...
func (p *ContainerPlayer) Play(
	...
	containerName, err := p.pullAndStart(
		ctx, imageRef, imageRefUsername, imageRefPassword, []mount.Mount{
			{
				Type:   mount.TypeBind,
				Source: workRoot,
				Target: workRoot,
...
func (p *ContainerPlayer) pullAndStart(
	ctx context.Context, imageRef, imageRefUsername, imageRefPassword string, volumes []mount.Mount,
	forceVolumes, hostNetwork bool,
) (string, error) {
	if err := p.pullImage(ctx, imageRef, imageRefUsername, imageRefPassword); err != nil {
		return "", errors.Wrap(err, "image pulling failed")
	}
	containerName, err := p.startContainer(ctx, imageRef, volumes, forceVolumes, hostNetwork)
	if err != nil {
...
func (p *ContainerPlayer) startContainer(
	ctx context.Context, imageRef string, volumes []mount.Mount, forceVolumes bool, hostNetwork bool,
) (string, error) {
	...
	return p.createRunningContainer(ctx, imageRef, volumes, hostNetwork)
}
...
// ContainerExecParameters holds all the necessary parameters for executing a command in container.
...
func (p *ContainerPlayer) StartExecuteAndRemove(ctx context.Context, cp ContainerExecParameters) (err error) {
	p.log.WithFields(logrus.Fields{
		"command": cp.Cmd,
		"image":   cp.ImageRef,
	...
	containerName, err := p.pullAndStart(
		ctx, cp.ImageRef, cp.ImageRefUsername, cp.ImageRefPassword, toMounts(cp.Volumes), true, true,
	)
	if err != nil {
		return errors.Wrap(err, "container couldn't be started")
	}
	...
	return p.execCmd(ctx, containerName, cp.Cmd, cp.Env, cp.WorkingDirectory)
}
...
func (p *ContainerPlayer) createRunningContainer(
	ctx context.Context, imageRef string, mounts []mount.Mount, hostNetwork bool,
) (string, error) {
	...
	resp, err := p.client.ContainerCreate(
		...
		p.hostConfig(mounts, hostNetwork),
		nil,
		containerName,
	)
	if err != nil {
...
func (p *ContainerPlayer) hostConfig(mounts []mount.Mount, hostNetwork bool) *container.HostConfig {
	if !hostNetwork {
		return &container.HostConfig{Mounts: mounts}
	}
