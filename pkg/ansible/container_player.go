...
func (p *ContainerPlayer) Play(
	...
	cmd := p.prepareDockerCmd(workingDirectory, ansibleArgs)
...
// TODO: return error code if ansible fails
// playbooks: in /root/contrail-ansible-deployer
// instances: /var/tmp/contrail-cluster, but you wanna mount /var/tmp
func (p *ContainerPlayer) prepareDockerCmd(workingDirectory string, ansibleArgs []string) *exec.Cmd {
...
func (p *ContainerPlayer) execCmd(ctx context.Context, containerName string, cmd *exec.Cmd) error {
	...
	status := execInspectResp.ExitCode
	if status != 0 {
		return errors.New("status code " + strconv.Itoa(status))
	}
