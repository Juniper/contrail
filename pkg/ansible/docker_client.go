...
func (c *DockerClient) Play(repositoryPath string, ansibleArgs []string) error {
	...
	err := pullImage(c.context, c.client, c.imageRegistry, c.imageRegistryUsername, c.imageRegistryPassword)
	if err != nil {
		return err
	}
	...
	c.log.WithFields(logrus.Fields{
		"image-registry": c.imageRegistry,
	}).Debug("Searching for exsiting container created from specified image registry")
	containerInstance, isFound, err := searchContainer(c.context, c.client, c.imageRegistry)
	if err != nil {
		return err
	}
	...
	if isFound {
		containerID, containerName, err = startExistingContainer(c.context, c.client, containerInstance, c.imageRegistry)
	} else {
		containerID, containerName, err = createRunningContainer(c.context, c.client, c.imageRegistry)
	}
	if err != nil {
		return err
	}
	...
	err = execCmd(c.context, c.client, containerID, cmd, true)
	if err != nil {
		return err
	}
	...
	err = cleanContainer(c.context, c.client, containerID, c.keepContainerAlive)
	if err != nil {
		return err
	}
...
func searchContainer(ctx context.Context, cli apiClient, imageRegistry string) (*types.Container, bool, error) {
...
func startExistingContainer(ctx context.Context, cli apiClient, containerInstance *types.Container, imageRegistry string) (containerName string, containerID string, err error) {
...
func createRunningContainer(ctx context.Context, cli apiClient, imageRegistry string) (containerName string, containerID string, err error) {
