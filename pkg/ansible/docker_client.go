...
func (c *DockerClient) Play(imageRef string, imageRefUsername string, imageRefPassword string, repositoryPath string, ansibleArgs []string, keepContainerAlive bool) error {
	...
	err := pullImage(ctx, c.client, imageRef, imageRefUsername, imageRefPassword)
	if err != nil {
		return err
	}
	...
	containerInstance, isFound, err := searchContainer(ctx, c.client, imageRef)
	if err != nil {
		return err
	}
	...
	if isFound {
		containerName, err = startExistingContainer(ctx, c.client, containerInstance)
	} else {
		containerName, err = createRunningContainer(ctx, c.client, imageRef)
	}
	if err != nil {
		return err
	}
	...
	err = execCmd(ctx, c.client, containerName, cmd, true)
	if err != nil {
		return err
	}
	...
	err = cleanContainer(ctx, c.client, containerName, keepContainerAlive)
	if err != nil {
		return err
	}
...
func pullImage(ctx context.Context, cli apiClient, imageRef string, username string, password string) error {
	...
	reader, err := cli.ImagePull(ctx, imageRef, types.ImagePullOptions{RegistryAuth: authStr})
	if err != nil {
		return err
	}
	defer func() {
...
func searchContainer(ctx context.Context, cli apiClient, imageRef string) (*types.Container, bool, error) {
	containerInstances, err := cli.ContainerList(ctx, types.ContainerListOptions{All: true})
	if err != nil {
...
func startExistingContainer(ctx context.Context, cli apiClient, containerInstance *types.Container) (string, error) {
...
func createRunningContainer(ctx context.Context, cli apiClient, imageRef string) (string, error) {
...
func execCmd(ctx context.Context, cli apiClient, containerName string, cmd *exec.Cmd, writeLog bool) error {
...
func cleanContainer(ctx context.Context, cli apiClient, containerName string, keepContainerAlive bool) error {
	if !keepContainerAlive {
		err := cli.ContainerStop(ctx, containerName, new(time.Duration))
		if err != nil {
			return err
		}
