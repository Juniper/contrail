	...
	tests := []struct {
		...
		},
		{
			...
			initMock: func(o oner) {
				...
				o.On("RenewPublication", publication).Return(nil).Once()
				o.On("StartReplication", slot, publication, uint64(0)).Return(nil).Once()
				o.On("WaitForReplicationMessage", mock.Anything).Return((*pgx.ReplicationMessage)(nil), context.DeadlineExceeded).Twice()
				o.On("WaitForReplicationMessage", mock.Anything).Return((*pgx.ReplicationMessage)(nil), assert.AnError).Once()
			},
		},
		{
				...
				o.On("WaitForReplicationMessage", mock.Anything).Return(
					&pgx.ReplicationMessage{
						WalMessage: &pgx.WalMessage{
							WalData: getBeginData(pgoutput.Begin{}),
						},
					},
					nil,
				).Twice()
...
func (m *mockPostgresWatcherConnection) GetReplicationSlot(name string) (lastLSN uint64, snapshotName string, err error) {
...
func (m *mockPostgresWatcherConnection) WaitForReplicationMessage(ctx context.Context) (*pgx.ReplicationMessage, error) {
