...
// CreateFloatingIP does pre check for floating ip.
...
// DeleteFloatingIP does post actions for delete floating ip.
...
func (sv *ContrailTypeLogicService) getVirtualNetwork(
	...
	floatingIPPoolRes, err := sv.DB.GetFloatingIPPool(ctx,
	...
	virtualNetworkResponse, err := sv.DB.GetVirtualNetwork(ctx,
		&models.GetVirtualNetworkRequest{
			ID: floatingIPPoolRes.GetFloatingIPPool().GetParentUUID(),
		})
	if err != nil {
		return nil, err
	}
...
func (sv *ContrailTypeLogicService) getFloatingIP(
	...
	floatingIPRes, err := sv.DB.GetFloatingIP(ctx,
...
func (sv *ContrailTypeLogicService) getFloatingIPPoolSubnets(
	...
	floatingIPPoolResponse, err := sv.DB.GetFloatingIPPool(ctx, &models.GetFloatingIPPoolRequest{ID: floatingIP.GetParentUUID()})
	if err != nil {
		return nil, err
	}
