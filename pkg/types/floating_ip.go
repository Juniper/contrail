...
func (s *ContrailTypeLogicService) CreateFloatingIP(
	...
	err = db.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			isAllocated, _ := s.AddressManager.IsIPAllocated(ctx, &ipam.IsIPAllocatedParams{VirtualNetwork: virtualNetwork, IPAddress: ipAddress})
			if isAllocated {
			...
			if floatingIPPoolSubnets == nil {
				...
				floatingIPAddress, _, err = s.AddressManager.AllocateIP(
					ctx, &ipam.AllocateIPParams{VirtualNetwork: virtualNetwork,
				...
				for _, floatingIPPoolSubnetUUID := range floatingIPPoolSubnets.GetSubnetUUID() {
					...
					allocateIPParams := &ipam.AllocateIPParams{
...
func (s *ContrailTypeLogicService) DeleteFloatingIP(
	...
	err = db.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			deallocateIPParams := &ipam.DeallocateIPParams{
				VirtualNetwork: nil,
				IPAddress:      floatingIP.GetFloatingIPAddress(),
				AllocatorID:    floatingIP.GetUUID(),
			}

			s.AddressManager.DeallocateIP(ctx, deallocateIPParams)
			return nil
		})
