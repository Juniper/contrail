...
func (sv *ContrailTypeLogicService) checkIpamSubnets(
	...
	if common.ContainsString(fieldMask, models.NetworkIpamPropertyIDIpamSubnets) {
		...
		for _, vn := range newIpam.GetVirtualNetworkBackRefs() {
			oldIpamUUID := oldIpam.GetUUID()
			sv.processRefIpams(oldIpamUUID, vn, refIpamUUIDList)
			sv.processBackRefVNSubnets(vn, refSubnetsList)
		}

		err = sv.processFlatSubnetIpams(ctx, refIpamUUIDList, refSubnetsList)
		if err != nil {
			return err
		}
...
func (sv *ContrailTypeLogicService) processFlatSubnetIpams(
	...
	refSubnetsList []*models.IpamSubnetType) error {
...
func (sv *ContrailTypeLogicService) processRefIpams(
	...
	for _, ipamRef := range vn.GetNetworkIpamRefs() {
		if ipamRef.GetUUID() == oldIpamUUID || common.ContainsString(refIpamUUIDList, ipamRef.GetUUID()) {
			continue
		}
...
func (sv *ContrailTypeLogicService) processIpamUpdate(
	...
	if common.ContainsString(fieldMask, models.NetworkIpamPropertyIDIpamSubnets) {
		...
		newIpamSubnets := newIpam.GetIpamSubnets().GetSubnets()
		oldIpamSubnets := oldIpam.GetIpamSubnets().GetSubnets()
		err = sv.validateSpecificSubnetChanges(oldIpamSubnets, newIpamSubnets)
		if err != nil {
			return err
		}
...
func (sv *ContrailTypeLogicService) validateSpecificSubnetChanges(
...
func (sv *ContrailTypeLogicService) checkSubnetAllocPools(
	...
	//newAllocPools := newSubnet.GetAllocationPools()
	//oldAllocPools := oldSubnet.GetAllocationPools()
	return nil
}
