...
func (sv *ContrailTypeLogicService) CreateNetworkIpam(
	...
	err = sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			if ipamSubnets == nil {
				response, err = sv.Next().CreateNetworkIpam(ctx, request)
				return err
			}
...
func (sv *ContrailTypeLogicService) DeleteNetworkIpam(
	...
	err = sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			response, err = sv.Next().DeleteNetworkIpam(ctx, request)
			return err
		})
	return response, err
}
...
func (sv *ContrailTypeLogicService) UpdateNetworkIpam(
	...
	err = sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			response, err = sv.Next().UpdateNetworkIpam(ctx, request)
			return err
		})
	return response, err
}
