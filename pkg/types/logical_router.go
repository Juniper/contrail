...
func (sv *ContrailTypeLogicService) CreateLogicalRouter(
	...
	err := sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			vxLanRouting, err := sv.checkForExternalGateway(ctx, logicalRouter, nil, nil)
			if err != nil {
				return err
			}
...
func (sv *ContrailTypeLogicService) UpdateLogicalRouter(
	...
	err := sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			vxLanRouting, err := sv.checkForExternalGateway(ctx, logicalRouter, dbLogicalRouter, &fieldMask)
			if err != nil {
				return err
			}
