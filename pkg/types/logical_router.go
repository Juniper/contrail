...
func (sv *ContrailTypeLogicService) checkRouterSupportsVPNType(ctx context.Context, lr *models.LogicalRouter) error {
	...
	bgpvpns, err := sv.ReadService.ListBGPVPN(
		ctx, &services.ListBGPVPNRequest{baseservices.FieldsListSpec(ctx, bgpvpnUUIDs, models.BGPVPNFieldBGPVPNType)},
	)
	if err != nil {
		return err
	}
...
func (sv *ContrailTypeLogicService) checkRouterHasBGPVPNAssocViaNetwork(
	...
	if !hasLinkedBGPVPNs(lr, dbLR, fm) {
		return nil
	}
	...
	vnResp, err := sv.ReadService.ListVirtualNetwork(
		ctx, &services.ListVirtualNetworkRequest{
			baseservices.FieldsListSpec(ctx, vnUUIDs, models.VirtualNetworkFieldBGPVPNRefs),
		},
	)
	if err != nil {
		return err
...
func hasLinkedBGPVPNs(lr, dbLR *models.LogicalRouter, fm *types.FieldMask) bool {
	var bgpvpnRefs []*models.LogicalRouterBGPVPNRef
	if fm == nil || basemodels.FieldMaskContains(fm, models.LogicalRouterFieldBGPVPNRefs) {
		bgpvpnRefs = lr.GetBGPVPNRefs()
	} else {
		bgpvpnRefs = dbLR.GetBGPVPNRefs()
	}
	return len(bgpvpnRefs) != 0
}
...
func (sv *ContrailTypeLogicService) getLinkedVnUUIDs(
	...
	vmisResp, err := sv.ReadService.ListVirtualMachineInterface(
		ctx, &services.ListVirtualMachineInterfaceRequest{
			baseservices.FieldsListSpec(ctx, vmiUUIDs, models.VirtualMachineInterfaceFieldVirtualNetworkRefs),
		},
	)
	if err != nil {
		return nil, err
