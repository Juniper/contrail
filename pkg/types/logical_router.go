...
func (sv *ContrailTypeLogicService) CreateLogicalRouter(
	...
	err := sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			_, err = sv.checkForExternalGateway(ctx, logicalRouter)
			if err != nil {
				return err
			}
...
func (sv *ContrailTypeLogicService) UpdateLogicalRouter(
	...
	err := sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			databaseLogicalRouter, err := sv.getLogicalRouter(ctx, logicalRouter.GetUUID())
			if err != nil {
				return err
			}

			err = sv.checkPortGatewayNotInSameNetwork(ctx, logicalRouter, databaseLogicalRouter)
			if err != nil {
				return err
			}
