...
func virtualMachineInterfaceSetupNextServiceMocks(s *ContrailTypeLogicService) {
	...
	nextService.EXPECT().CreateVirtualMachineInterface(gomock.Not(gomock.Nil()), gomock.Not(gomock.Nil())).DoAndReturn(
...
func TestCreateVirtualMachineInterface(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Try to create virtual machine interface when cannot find virtual-network with corresponding uuid",
			paramVMI: models.VirtualMachineInterface{
				UUID: "uuid",
		...
		{
			name: "Try to create virtual machine interface corresponding to a virtual-network",
			paramVMI: models.VirtualMachineInterface{
				UUID: "uuid",
				VirtualNetworkRefs: []*models.VirtualMachineInterfaceVirtualNetworkRef{
					{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NotNil(t, createVirtualMachineInterfaceResponse)
			assert.EqualValues(t, &expectedResponse, createVirtualMachineInterfaceResponse)
		})
	}
}
