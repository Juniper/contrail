...
func (sv *ContrailTypeLogicService) CreateServiceTemplate(
	...
	err = sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			if domainUUID == "" {
				domainUUID, err = sv.getUUIDFromFQName(ctx, serviceTemplate.GetFQName(), serviceTemplate.GetParentType())
				if err != nil {
					return err
				}
			}

			serviceTemplate.Perms2, err = enableDomainSharing(serviceTemplate.GetPerms2(), domainUUID, basemodels.PermsRX)
			if err != nil {
				return err
			}
