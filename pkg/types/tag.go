...
func (sv *ContrailTypeLogicService) CreateTag(
	...
	err := sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) (err error) {
			...
			_, err = sv.WriteService.CreateTagTagTypeRef(ctx, &services.CreateTagTagTypeRefRequest{
				ID: response.GetTag().GetUUID(),
				TagTagTypeRef: &models.TagTagTypeRef{
					UUID: tagType.GetUUID(),
					To: tagType.GetFQName(),
				},
			})
			if err != nil {
				return err
...
func (sv *ContrailTypeLogicService) DeleteTag(
	...
	err := sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) (err error) {
			...
			_, err = sv.WriteService.DeleteTagTagTypeRef(ctx, &services.DeleteTagTagTypeRefRequest{
				ID: tagResponse.GetTag().GetUUID(),
				TagTagTypeRef: &models.TagTagTypeRef{
					UUID: tagType.GetUUID(),
					To: tagType.GetFQName(),
				},
			...
			// sv.WriteService.DeleteTagType(ctx, &services.DeleteTagTypeRequest{ID: tagType.GetUUID()})
