...
type addrMgrError ipam.AddressManagerErrorCode

func (e addrMgrError) GetAddressManagementErrorCode() ipam.AddressManagerErrorCode {
	return ipam.AddressManagerErrorCode(e)
}
func (e addrMgrError) Error() string {
	return ""
}
...
func setupIPAMMocks() {
	ipamMock.EXPECT().AllocateIP(gomock.Not(gomock.Nil()), gomock.Not(gomock.Nil())).DoAndReturn(
		func(ctx context.Context, request *ipam.AllocateIPRequest) (address string, subnetUUID string, err error) {
			...
			exhaustedError := addrMgrError(ipam.SubnetExhausted)
	...
	ipamMock.EXPECT().IsIPAllocated(gomock.Not(gomock.Nil()),
		&ipam.IsIPAllocatedRequest{VirtualNetwork: &models.VirtualNetwork{}, IPAddress: "10.0.0.2"}).Return(true, nil).AnyTimes()
}
...
func setupNextServiceMocks() {
	...
	nextServiceMock.EXPECT().DeleteFloatingIP(gomock.Not(gomock.Nil()), gomock.Not(gomock.Nil())).DoAndReturn(
		func(ctx context.Context, request *models.DeleteFloatingIPRequest) (response *models.DeleteFloatingIPResponse, err error) {
			return &models.DeleteFloatingIPResponse{ID: request.ID}, nil
		}).AnyTimes()
}

func prepareParent(floatingIPPool *models.FloatingIPPool) {
	if floatingIPPool != nil {
		getFloatingIPPoolResponse := &models.GetFloatingIPPoolResponse{FloatingIPPool: floatingIPPool}
		...
		dbServiceMock.EXPECT().GetFloatingIPPool(gomock.Not(gomock.Nil()), gomock.Not(gomock.Nil())).Return(nil, fmt.Errorf("DB error")).AnyTimes()
	}
}
...
func TestCreateFloatingIP(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Try to create floating ip when parent can't be got from DB ",
			createRequest: models.CreateFloatingIPRequest{FloatingIP: &models.FloatingIP{
				ParentType: "floating-ip-pool",
			}},
			fails: true,
...
func TestDeleteFloatingIP(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "Try to delete floating ip if it doesn't exist",
			deleteRequest: &models.DeleteFloatingIPRequest{ID: "uuid-1"},
			deallocatesIP: false,
			fails:         true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.floatingIP != nil {
				getFloatingIPResponse := &models.GetFloatingIPResponse{FloatingIP: tt.floatingIP}
				dbServiceMock.EXPECT().GetFloatingIP(gomock.Not(gomock.Nil()), gomock.Not(gomock.Nil())).Return(getFloatingIPResponse, nil).AnyTimes()
			} else {
				dbServiceMock.EXPECT().GetFloatingIP(gomock.Not(gomock.Nil()), gomock.Not(gomock.Nil())).Return(nil, fmt.Errorf("Not found")).AnyTimes()
			}
