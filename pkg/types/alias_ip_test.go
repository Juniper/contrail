...
func aliasIPSetupReadServiceMocks(s *ContrailTypeLogicService) {
...
func aliasIPSetupIPAMMocks(s *ContrailTypeLogicService) {
...
func aliasIPSetupNextServiceMocks(s *ContrailTypeLogicService) {
...
func aliasIPPrepareParent(s *ContrailTypeLogicService, aliasIPPool *models.AliasIPPool) {
	readService := s.ReadService.(*servicesmock.MockReadService) //nolint: errcheck
	if aliasIPPool != nil {
...
func TestCreateAliasIP(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			aliasIPSetupReadServiceMocks(service)
			aliasIPSetupIPAMMocks(service)
			aliasIPSetupNextServiceMocks(service)

			aliasIPPrepareParent(service, tt.aliasIPParent)
...
func TestDeleteAliasIP(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "Try to delete inexistent alias ip",
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.deallocatesIP {
				...
				addressManager.EXPECT().DeallocateIP(gomock.Not(gomock.Nil()), gomock.Not(gomock.Nil())).Return(
					fmt.Errorf("not allocated")).AnyTimes()
			}
