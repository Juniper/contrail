...
func (sv *ContrailTypeLogicService) CreateBGPRouter(
	...
	err := sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			if bgpRouterParams := bgpRouter.GetBGPRouterParameters(); bgpRouterParams != nil {
				asn = bgpRouterParams.GetAutonomousSystem()
			}

			if err = sv.checkSubClusterASN(ctx, bgpRouter, asn); err != nil {
				return err
			}
...
func (sv *ContrailTypeLogicService) checkSubClusterASN(
	ctx context.Context, bgpRouter *models.BGPRouter, asn int64,
) error {
