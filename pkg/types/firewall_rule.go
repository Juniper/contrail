...
func (sv *ContrailTypeLogicService) CreateFirewallRule(
	...
	err := sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			if err = firewallRule.CheckServiceProperties(nil, nil); err != nil {
				return err
			}
...
func (sv *ContrailTypeLogicService) UpdateFirewallRule(
	...
	err := sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			if err = firewallRule.CheckServiceProperties(databaseFR, &fm); err != nil {
				return err
			}
...
func (sv *ContrailTypeLogicService) setMatchTagTypes(
	...
	path := strings.Join(
		[]string{models.FirewallRuleFieldMatchTagTypes, models.FirewallRuleMatchTagsTypeIdListFieldTagType},
		".",
	)
	if len(fr.GetMatchTagTypes().GetTagType()) > 0 && fm != nil &&
		!basemodels.FieldMaskContains(fm, path) {
...
func (sv *ContrailTypeLogicService) setTagRefs(
	...
	for i, ep := range endpoints {
		...
		if ep == nil && (fm == nil || !basemodels.FieldMaskPartlyContains(fm, []string{fields[i]})) {
			ep = dbEndpoints[i]
		}
		...
		path := strings.Join([]string{fields[i], models.FirewallRuleEndpointTypeFieldTagIds}, ".")
		if len(endpoints[i].GetTagIds()) > 0 && fm != nil && !basemodels.FieldMaskContains(fm, path) {
			fm.Paths = append(fm.Paths, path)
		}
	}
