...
//CreateAlarm do pre check for alarm.
func (sv *ContrailTypeLogicService) CreateAlarm(
	...
	err = sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			// check if alarm rules exist
			err := alarm.CheckAlarmRules()
			if err != nil {
				return err
			}

			// check if alarm rules are valid
			err = alarm.CheckAlarmExpressions()
			if err != nil {
				return err
			}
...
//UpdateAlarm do pre check for alarm.
func (sv *ContrailTypeLogicService) UpdateAlarm(
	...
	err = sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			response, err = sv.BaseService.UpdateAlarm(ctx, request)
			if err != nil {
				return err
			}
