...
func (sv *ContrailTypeLogicService) CreateVirtualNetwork(
	...
	err = sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			// TODO is 0 the same as None?
			if vxLanID != 0 {
				err = sv.IntPoolAllocator.SetInt(ctx, VxlanNetworkIDPoolKey, vxLanID, virtualNetwork.VxLANIntOwner())
				if err != nil {
...
func (sv *ContrailTypeLogicService) DeleteVirtualNetwork(
	...
	err = sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			vxLanID := vn.GetVirtualNetworkProperties().GetVxlanNetworkIdentifier()
			err = sv.IntPoolAllocator.DeallocateInt(ctx, VxlanNetworkIDPoolKey, vxLanID)
			if err != nil {
				return errutil.ErrorBadRequestf("couldn't deallocate vxlan network id(%v): %v",
					vxLanID, err)
			}
...
func (sv *ContrailTypeLogicService) processVxlanIDUpdate(
	...
	// TODO consider equality of VxLANID == 0 and None
	if currentVxlanID == requestedVxlanID || requestedVxlanID == 0 {
		return nil
	}

	// Deallocate old VxLAN ID
	// TODO deallocated_vxlan_network_identifier ???
	err := sv.IntPoolAllocator.DeallocateInt(ctx,
		VxlanNetworkIDPoolKey, currentVxlanID)
	if err != nil {
		return errutil.ErrorBadRequestf("couldn't deallocate vxlan network id(%v): %v",
			currentVxlanID, err)
	}

	err = sv.IntPoolAllocator.SetInt(ctx, VxlanNetworkIDPoolKey, requestedVxlanID, requestedVN.VxLANIntOwner())
	if err != nil {
		return errutil.ErrorBadRequestf("cannot allocate provided vxlan identifier(%v): %v", requestedVxlanID, err)
	}
