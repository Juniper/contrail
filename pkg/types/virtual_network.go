...
func (sv *ContrailTypeLogicService) CreateVirtualNetwork(
	...
	if virtualNetwork.HasVirtualNetworkNetworkID() {
		return nil, grpc.Errorf(codes.PermissionDenied, "Cannot set the virtual network ID, it's allocated by the server")
	}
	...
	err = db.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			response, err = sv.Next().CreateVirtualNetwork(ctx, request)
...
func (sv *ContrailTypeLogicService) DeleteVirtualNetwork(
	...
	err = db.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			response, err = sv.Next().DeleteVirtualNetwork(ctx, request)
...
func (sv *ContrailTypeLogicService) processIpamNetworkSubnets(ctx context.Context, virtualNetwork *models.VirtualNetwork) error {
	...
	for _, ipamReference := range ipamReferences {
		ipamResponse, err := sv.DB.GetNetworkIpam(ctx, &services.GetNetworkIpamRequest{
...
func (sv *ContrailTypeLogicService) checkBGPVPNRefs(
	...
	for _, logicalRouterUUID := range virtualNetwork.LogicalRouterRefs {
		...
		logicalRouterResponse, err := sv.DB.GetLogicalRouter(ctx, &services.GetLogicalRouterRequest{ID: refUUID})
		if err != nil {
			return err
		}
...
func (sv *ContrailTypeLogicService) getVirtualNetworkID(ctx context.Context, id string) (int64, error) {
	getResponse, err := sv.DB.GetVirtualNetwork(ctx, &services.GetVirtualNetworkRequest{
		ID: id,
	})
	if err != nil {
		return 0, err
...
func (sv *ContrailTypeLogicService) getBGPVPNType(ctx context.Context, id string) (string, error) {
	getResponse, err := sv.DB.GetBGPVPN(ctx, &services.GetBGPVPNRequest{
		ID: id,
	})
	if err != nil {
		return "", err
