...
func (sv *ContrailTypeLogicService) updateVnSubnetsInAddrMgmt(
	...
	vnSubnetsToDelete := models.IpamSubnetSetsSubtract(currentVN.GetSubnets(), requestedVN.GetSubnets())
...
func (sv *ContrailTypeLogicService) checkInstanceIP(
	...
	return validateIfSubnetSetNotIncludeIP(subnetsSet, instanceIP.GetInstanceIP().GetInstanceIPAddress())
}
...
func (sv *ContrailTypeLogicService) checkFloatingIPPool(
	...
	for _, floatingIP := range floatingIPPool.GetFloatingIPPool().GetFloatingIPs() {
		err = validateIfSubnetSetNotIncludeIP(subnetsSet, floatingIP.GetFloatingIPAddress())
		if err != nil {
			return err
		}
	}
...
func (sv *ContrailTypeLogicService) checkAliasIPPool(
	...
	for _, aliasIP := range aliasIPPool.GetAliasIPPool().GetAliasIPs() {
		err = validateIfSubnetSetNotIncludeIP(subnetsSet, aliasIP.GetAliasIPAddress())
		if err != nil {
			return err
		}
	}
...
func validateIfSubnetSetNotIncludeIP(
	subnetsSet []*models.IpamSubnetType,
	ipString string,
) error {
