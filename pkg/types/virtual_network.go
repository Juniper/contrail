...
func ValidateMultiPolicyServiceChainConfigWithUpdate(
	...
	fmt.Println(fieldMask.Paths)
	for _, field := range fieldMask.GetPaths() {
		if field == models.VirtualNetworkFieldImportRouteTargetList {
			oldVN.ImportRouteTargetList = updateVN.ImportRouteTargetList
		} else if field == models.VirtualNetworkFieldExportRouteTargetList {
			oldVN.ExportRouteTargetList = updateVN.ExportRouteTargetList
		} else if field == (models.VirtualNetworkFieldRouteTargetList + "." + models.RouteTargetListFieldRouteTarget) {
