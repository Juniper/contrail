...
func (sv *ContrailTypeLogicService) DeleteTagType(
	...
	err := sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			//tagTypeIdHex := strings.TrimPrefix(tagTypeResponse.TagType.TagTypeID, "0x")
			//val, err := strconv.ParseInt(tagTypeIdHex, 10, 64)
			//if err != nil {
			//	return errutil.ErrorBadRequestf("Convert hex to int error for (%v): %s", tagTypeIdHex, err)
			//}
			//
