		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fails {
				...
				if tt.errorCode != codes.OK {
					...
					assert.Equal(t, tt.errorCode, status.Code(), "Expected different grpc status")
				}
			} else {
...
func virtualNetworkSetupDataServiceMocks(s *ContrailTypeLogicService) {
	dateServiceMock := s.DataService.(*servicesmock.MockService)

	// Virtual Networks
	dateServiceMock.EXPECT().GetVirtualNetwork(gomock.Not(gomock.Nil()),
	...
	dateServiceMock.EXPECT().GetVirtualNetwork(gomock.Not(gomock.Nil()), gomock.Not(gomock.Nil())).Return(
		nil, common.ErrorNotFound).AnyTimes()

	// BGPVPN
	bgpVPNL3 := models.MakeBGPVPN()
	bgpVPNL3.BGPVPNType = models.L3Mode
	dateServiceMock.EXPECT().GetBGPVPN(gomock.Not(gomock.Nil()),
	...
	dateServiceMock.EXPECT().GetBGPVPN(gomock.Not(gomock.Nil()), gomock.Not(gomock.Nil())).Return(nil,
		common.ErrorNotFound).AnyTimes()

	// Logical Routers
	logicalRouter := models.MakeLogicalRouter()
	dateServiceMock.EXPECT().GetLogicalRouter(gomock.Not(gomock.Nil()),
...
func virtualNetworkSetupNetworkIpam(s *ContrailTypeLogicService, ipamSubnetMethod string) {
	...
	dateServiceMock.EXPECT().GetNetworkIpam(gomock.Not(gomock.Nil()),
	...
	dateServiceMock.EXPECT().GetNetworkIpam(gomock.Not(gomock.Nil()), gomock.Not(gomock.Nil())).Return(
		nil, common.ErrorNotFound).AnyTimes()
}
