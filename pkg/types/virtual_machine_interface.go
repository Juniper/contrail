...
func (sv *ContrailTypeLogicService) UpdateVirtualMachineInterface(
	...
	err := sv.InTransactionDoer.DoInTransaction(
		...
		func(ctx context.Context) error {
			...
			if err != nil {
				return errors.WithMessage(err, "can't read virtual machine interface from database")
			}
			vmi := vmiResp.GetVirtualMachineInterface()
			kvps := vmi.GetVirtualMachineInterfaceBindings()

			value := kvps.GetValue(vnicKeyName)
			if value == vnicTypeDirect && err == nil {
				if err = sv.updateVirtualRouterToVirtualMachinesRefLinks(ctx, vmi, updatedVmi); err != nil {
					return err
				}
			}
...
func (sv *ContrailTypeLogicService) updateVirtualRouterToVirtualMachinesRefLinks(
	...
	if err != nil {
		if code := errutil.CauseCode(err); code == codes.NotFound || vRouterUUID == "" {
			return nil
		}
		return err
	}
