...
func tagSetUpWriteServiceMock(s *ContrailTypeLogicService) {
	...
	writeService.EXPECT().CreateTagTagTypeRef(
		gomock.Not(gomock.Nil()), gomock.Not(gomock.Nil()),
	).DoAndReturn(
		func(_ context.Context, _ *services.CreateTagTagTypeRefRequest) (*services.CreateTagTagTypeRefResponse, error) {
			res := &services.CreateTagTagTypeRefResponse{}
			return res, nil
		},
	).MaxTimes(1)
}
