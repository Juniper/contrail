...
func TestVirtualDNSRecord(t *testing.T) {
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateVirtualDNSRecord(ctx, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListVirtualDNSRecord(ctx, tx, &models.ListVirtualDNSRecordRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualDNSRecord(ctxDemo, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualDNSRecord(ctx, tx,
			&models.DeleteVirtualDNSRecordRequest{
				ID: model.UUID})
	})
	if err != nil {
