...
func TestFlavor(t *testing.T) {
	...
	common.UseTable(db, "flavor")
	defer func() {
		common.ClearTable(db, "flavor")
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListFlavor(ctx, tx, &models.ListFlavorRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteFlavor(ctxDemo, tx,
