...
func TestNetworkDeviceConfig(t *testing.T) {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeletePhysicalRouter(tx, "network_device_config_physical_router_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref network_device_config_physical_router_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeletePhysicalRouter(tx, "network_device_config_physical_router_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref network_device_config_physical_router_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeletePhysicalRouter(tx, "network_device_config_physical_router_ref_uuid2", nil)
    })
	if err != nil {
