...
const insertAPIAccessListQuery = "insert into `api_access_list` (`rbac_rule`,`global_access`,`share`,`owner`,`owner_access`,`uuid`,`fq_name`,`created`,`creator`,`user_visible`,`last_modified`,`other_access`,`group`,`group_access`,`permissions_owner`,`permissions_owner_access`,`enable`,`description`,`display_name`,`key_value_pair`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
const updateAPIAccessListQuery = "update `api_access_list` set `rbac_rule` = ?,`global_access` = ?,`share` = ?,`owner` = ?,`owner_access` = ?,`uuid` = ?,`fq_name` = ?,`created` = ?,`creator` = ?,`user_visible` = ?,`last_modified` = ?,`other_access` = ?,`group` = ?,`group_access` = ?,`permissions_owner` = ?,`permissions_owner_access` = ?,`enable` = ?,`description` = ?,`display_name` = ?,`key_value_pair` = ?;"
...
func CreateAPIAccessList(tx *sql.Tx, model *models.APIAccessList) error {
	...
	_, err = stmt.Exec(common.MustJSON(model.APIAccessListEntries.RbacRule),
