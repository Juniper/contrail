...
func TestOpenstackCluster(t *testing.T) {
	...
	mutexTable := common.UseTable(db, "openstack_cluster")
	defer func() {
		mutexTable.Unlock()
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateOpenstackCluster(ctx, tx,
			&models.CreateOpenstackClusterRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListOpenstackCluster(ctx, tx, &models.ListOpenstackClusterRequest{
			Spec: &models.ListSpec{Limit: 1}})
		if err != nil {
			return err
		}
		if len(response.OpenstackClusters) != 1 {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteOpenstackCluster(ctxDemo, tx,
			&models.DeleteOpenstackClusterRequest{
				ID: model.UUID},
		)
	})
	if err == nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteOpenstackCluster(ctx, tx,
			&models.DeleteOpenstackClusterRequest{
				ID: model.UUID})
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		...
		if len(response.OpenstackClusters) != 0 {
