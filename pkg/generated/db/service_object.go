...
const insertServiceObjectQuery = "insert into `service_object` (`owner_access`,`other_access`,`group`,`group_access`,`owner`,`enable`,`description`,`created`,`creator`,`user_visible`,`last_modified`,`display_name`,`key_value_pair`,`global_access`,`share`,`perms2_owner`,`perms2_owner_access`,`uuid`,`fq_name`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
const updateServiceObjectQuery = "update `service_object` set `owner_access` = ?,`other_access` = ?,`group` = ?,`group_access` = ?,`owner` = ?,`enable` = ?,`description` = ?,`created` = ?,`creator` = ?,`user_visible` = ?,`last_modified` = ?,`display_name` = ?,`key_value_pair` = ?,`global_access` = ?,`share` = ?,`perms2_owner` = ?,`perms2_owner_access` = ?,`uuid` = ?,`fq_name` = ?;"
...
func ListServiceObject(tx *sql.Tx, spec *db.ListSpec) ([]*models.ServiceObject, error) {
	var rows *sql.Rows
	var err error
	//TODO (check input)
...
func ShowServiceObject(tx *sql.Tx, uuid string) (*models.ServiceObject, error) {
	list, err := ListServiceObject(tx, &db.ListSpec{
