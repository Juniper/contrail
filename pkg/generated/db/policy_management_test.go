...
func TestPolicyManagement(t *testing.T) {
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreatePolicyManagement(ctx, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListPolicyManagement(ctx, tx, &models.ListPolicyManagementRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeletePolicyManagement(ctxDemo, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeletePolicyManagement(ctx, tx,
			&models.DeletePolicyManagementRequest{
				ID: model.UUID})
	})
	if err != nil {
