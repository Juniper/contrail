...
func TestVirtualMachineInterface(t *testing.T) {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateInterfaceRouteTable(tx, InterfaceRouteTablerefModel)
	})
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateRoutingInstance(tx, RoutingInstancerefModel)
	})
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreatePortTuple(tx, PortTuplerefModel)
	})
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateVirtualMachine(tx, VirtualMachinerefModel)
	})
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateBridgeDomain(tx, BridgeDomainrefModel)
	})
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateServiceEndpoint(tx, ServiceEndpointrefModel)
	})
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateServiceHealthCheck(tx, ServiceHealthCheckrefModel)
	})
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteSecurityLoggingObject(tx, "virtual_machine_interface_security_logging_object_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_security_logging_object_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteSecurityLoggingObject(tx, "virtual_machine_interface_security_logging_object_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_security_logging_object_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteSecurityLoggingObject(tx, "virtual_machine_interface_security_logging_object_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteQosConfig(tx, "virtual_machine_interface_qos_config_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_qos_config_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteQosConfig(tx, "virtual_machine_interface_qos_config_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_qos_config_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteQosConfig(tx, "virtual_machine_interface_qos_config_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeletePhysicalInterface(tx, "virtual_machine_interface_physical_interface_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_physical_interface_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeletePhysicalInterface(tx, "virtual_machine_interface_physical_interface_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_physical_interface_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeletePhysicalInterface(tx, "virtual_machine_interface_physical_interface_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteServiceHealthCheck(tx, "virtual_machine_interface_service_health_check_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_service_health_check_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteServiceHealthCheck(tx, "virtual_machine_interface_service_health_check_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_service_health_check_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteServiceHealthCheck(tx, "virtual_machine_interface_service_health_check_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "virtual_machine_interface_virtual_machine_interface_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_virtual_machine_interface_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "virtual_machine_interface_virtual_machine_interface_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_virtual_machine_interface_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "virtual_machine_interface_virtual_machine_interface_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBGPRouter(tx, "virtual_machine_interface_bgp_router_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_bgp_router_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBGPRouter(tx, "virtual_machine_interface_bgp_router_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_bgp_router_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBGPRouter(tx, "virtual_machine_interface_bgp_router_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteInterfaceRouteTable(tx, "virtual_machine_interface_interface_route_table_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_interface_route_table_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteInterfaceRouteTable(tx, "virtual_machine_interface_interface_route_table_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_interface_route_table_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteInterfaceRouteTable(tx, "virtual_machine_interface_interface_route_table_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualNetwork(tx, "virtual_machine_interface_virtual_network_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_virtual_network_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualNetwork(tx, "virtual_machine_interface_virtual_network_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_virtual_network_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualNetwork(tx, "virtual_machine_interface_virtual_network_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteRoutingInstance(tx, "virtual_machine_interface_routing_instance_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_routing_instance_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteRoutingInstance(tx, "virtual_machine_interface_routing_instance_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_routing_instance_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteRoutingInstance(tx, "virtual_machine_interface_routing_instance_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeletePortTuple(tx, "virtual_machine_interface_port_tuple_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_port_tuple_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeletePortTuple(tx, "virtual_machine_interface_port_tuple_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_port_tuple_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeletePortTuple(tx, "virtual_machine_interface_port_tuple_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteSecurityGroup(tx, "virtual_machine_interface_security_group_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_security_group_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteSecurityGroup(tx, "virtual_machine_interface_security_group_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_security_group_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteSecurityGroup(tx, "virtual_machine_interface_security_group_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachine(tx, "virtual_machine_interface_virtual_machine_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_virtual_machine_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachine(tx, "virtual_machine_interface_virtual_machine_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_virtual_machine_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachine(tx, "virtual_machine_interface_virtual_machine_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBridgeDomain(tx, "virtual_machine_interface_bridge_domain_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_bridge_domain_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBridgeDomain(tx, "virtual_machine_interface_bridge_domain_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_bridge_domain_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBridgeDomain(tx, "virtual_machine_interface_bridge_domain_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteServiceEndpoint(tx, "virtual_machine_interface_service_endpoint_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_service_endpoint_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteServiceEndpoint(tx, "virtual_machine_interface_service_endpoint_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_machine_interface_service_endpoint_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteServiceEndpoint(tx, "virtual_machine_interface_service_endpoint_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, model.UUID,
