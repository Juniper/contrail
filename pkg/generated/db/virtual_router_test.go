...
func TestVirtualRouter(t *testing.T) {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteNetworkIpam(tx, "virtual_router_network_ipam_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref virtual_router_network_ipam_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteNetworkIpam(tx, "virtual_router_network_ipam_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref virtual_router_network_ipam_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteNetworkIpam(tx, "virtual_router_network_ipam_ref_uuid2", nil)
    })
	if err != nil {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualMachine(tx, "virtual_router_virtual_machine_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref virtual_router_virtual_machine_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualMachine(tx, "virtual_router_virtual_machine_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref virtual_router_virtual_machine_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualMachine(tx, "virtual_router_virtual_machine_ref_uuid2", nil)
    })
	if err != nil {
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        models, err := ListVirtualRouter(tx, &common.ListSpec{Limit: 1})
        if err != nil {
            return err
        }
        if len(models) != 1 {
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        return DeleteVirtualRouter(tx, model.UUID, 
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        return DeleteVirtualRouter(tx, model.UUID, nil)
    })
    if err != nil {
