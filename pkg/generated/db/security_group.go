...
func (db *DB) createSecurityGroup(
	...
	err = common.CreateSharing(tx, "security_group", model.UUID, model.GetPerms2().GetShare())
	if err != nil {
		return err
	}
	log.WithFields(log.Fields{
...
func scanSecurityGroup(values map[string]interface{}) (*models.SecurityGroup, error) {
	...
	if value, ok := values["security_group_id"]; ok {
		...
		m.SecurityGroupID = schema.InterfaceToInt64(value)
	...
	if value, ok := values["configured_security_group_id"]; ok {
		...
		m.ConfiguredSecurityGroupID = schema.InterfaceToInt64(value)
