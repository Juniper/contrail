...
func TestVirtualIP(t *testing.T) {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateLoadbalancerPool(tx, LoadbalancerPoolrefModel)
	})
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteLoadbalancerPool(tx, "virtual_ip_loadbalancer_pool_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_ip_loadbalancer_pool_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteLoadbalancerPool(tx, "virtual_ip_loadbalancer_pool_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_ip_loadbalancer_pool_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteLoadbalancerPool(tx, "virtual_ip_loadbalancer_pool_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "virtual_ip_virtual_machine_interface_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_ip_virtual_machine_interface_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "virtual_ip_virtual_machine_interface_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref virtual_ip_virtual_machine_interface_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "virtual_ip_virtual_machine_interface_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		models, err := ListVirtualIP(tx, &common.ListSpec{Limit: 1})
		if err != nil {
			return err
		}
		if len(models) != 1 {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualIP(tx, model.UUID,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualIP(tx, model.UUID, nil)
	})
	if err != nil {
