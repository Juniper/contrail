...
func CreateProviderAttachment(
    ...
    err = common.CreateSharing(tx, "provider_attachment", model.UUID, model.GetPerms2().GetShare())
    if err != nil {
        return err
    }
    log.WithFields(log.Fields{
...
func ListProviderAttachment(ctx context.Context, tx *sql.Tx, request *models.ListProviderAttachmentRequest) (response *models.ListProviderAttachmentResponse, err error) {
    ...
    spec := request.Spec
    qb.Spec = spec
    qb.Table = "provider_attachment"
    qb.Fields = ProviderAttachmentFields
    qb.RefFields = ProviderAttachmentRefFields
    qb.BackRefFields = ProviderAttachmentBackRefFields
