...
const insertConfigRootQuery = "insert into `config_root` (`uuid`,`fq_name`,`group_access`,`owner`,`owner_access`,`other_access`,`group`,`enable`,`description`,`created`,`creator`,`user_visible`,`last_modified`,`display_name`,`key_value_pair`,`share`,`perms2_owner`,`perms2_owner_access`,`global_access`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
const updateConfigRootQuery = "update `config_root` set `uuid` = ?,`fq_name` = ?,`group_access` = ?,`owner` = ?,`owner_access` = ?,`other_access` = ?,`group` = ?,`enable` = ?,`description` = ?,`created` = ?,`creator` = ?,`user_visible` = ?,`last_modified` = ?,`display_name` = ?,`key_value_pair` = ?,`share` = ?,`perms2_owner` = ?,`perms2_owner_access` = ?,`global_access` = ?;"
...
func ListConfigRoot(tx *sql.Tx, spec *db.ListSpec) ([]*models.ConfigRoot, error) {
	var rows *sql.Rows
	var err error
	//TODO (check input)
...
func ShowConfigRoot(tx *sql.Tx, uuid string) (*models.ConfigRoot, error) {
	list, err := ListConfigRoot(tx, &db.ListSpec{
