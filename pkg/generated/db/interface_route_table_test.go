...
func TestInterfaceRouteTable(t *testing.T) {
	...
	mutexTable := common.UseTable(db, "interface_route_table")
	// mutexProject := common.UseTable(db, "interface_route_table")
	defer func() {
		mutexTable.Unlock()
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateInterfaceRouteTable(ctx, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListInterfaceRouteTable(ctx, tx, &models.ListInterfaceRouteTableRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteInterfaceRouteTable(ctxDemo, tx,
