...
func TestPhysicalRouter(t *testing.T) {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualNetwork(tx, "physical_router_virtual_network_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref physical_router_virtual_network_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualNetwork(tx, "physical_router_virtual_network_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref physical_router_virtual_network_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualNetwork(tx, "physical_router_virtual_network_ref_uuid2", nil)
    })
	if err != nil {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteBGPRouter(tx, "physical_router_bgp_router_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref physical_router_bgp_router_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteBGPRouter(tx, "physical_router_bgp_router_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref physical_router_bgp_router_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteBGPRouter(tx, "physical_router_bgp_router_ref_uuid2", nil)
    })
	if err != nil {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualRouter(tx, "physical_router_virtual_router_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref physical_router_virtual_router_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualRouter(tx, "physical_router_virtual_router_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref physical_router_virtual_router_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualRouter(tx, "physical_router_virtual_router_ref_uuid2", nil)
    })
	if err != nil {
