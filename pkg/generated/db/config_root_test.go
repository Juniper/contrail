...
func TestConfigRoot(t *testing.T) {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateTag(tx, TagrefModel)
	})
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteTag(tx, "config_root_tag_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref config_root_tag_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteTag(tx, "config_root_tag_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref config_root_tag_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteTag(tx, "config_root_tag_ref_uuid2", nil)
    })
	if err != nil {
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        models, err := ListConfigRoot(tx, &common.ListSpec{Limit: 1})
        if err != nil {
            return err
        }
        if len(models) != 1 {
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        return DeleteConfigRoot(tx, model.UUID, nil)
    })
    if err != nil {
