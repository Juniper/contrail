...
const insertServiceEndpointQuery = "insert into `service_endpoint` (`owner`,`owner_access`,`global_access`,`share`,`uuid`,`fq_name`,`user_visible`,`last_modified`,`group_access`,`permissions_owner`,`permissions_owner_access`,`other_access`,`group`,`enable`,`description`,`created`,`creator`,`display_name`,`key_value_pair`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
const updateServiceEndpointQuery = "update `service_endpoint` set `owner` = ?,`owner_access` = ?,`global_access` = ?,`share` = ?,`uuid` = ?,`fq_name` = ?,`user_visible` = ?,`last_modified` = ?,`group_access` = ?,`permissions_owner` = ?,`permissions_owner_access` = ?,`other_access` = ?,`group` = ?,`enable` = ?,`description` = ?,`created` = ?,`creator` = ?,`display_name` = ?,`key_value_pair` = ?;"
...
func ListServiceEndpoint(tx *sql.Tx, spec *db.ListSpec) ([]*models.ServiceEndpoint, error) {
	var rows *sql.Rows
	var err error
	//TODO (check input)
...
func ShowServiceEndpoint(tx *sql.Tx, uuid string) (*models.ServiceEndpoint, error) {
	list, err := ListServiceEndpoint(tx, &db.ListSpec{
