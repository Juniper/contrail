...
func TestPhysicalInterface(t *testing.T) {
    ...
    common.UseTable(db, "physical_interface")
    defer func(){
        common.ClearTable(db, "physical_interface")
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeletePhysicalInterface(tx, "physical_interface_physical_interface_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref physical_interface_physical_interface_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeletePhysicalInterface(tx, "physical_interface_physical_interface_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref physical_interface_physical_interface_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeletePhysicalInterface(tx, "physical_interface_physical_interface_ref_uuid2", nil)
    })
	if err != nil {
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        models, err := ListPhysicalInterface(tx, &common.ListSpec{Limit: 1})
        if err != nil {
            return err
        }
        if len(models) != 1 {
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        return DeletePhysicalInterface(tx, model.UUID, 
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        return DeletePhysicalInterface(tx, model.UUID, nil)
    })
    if err != nil {
