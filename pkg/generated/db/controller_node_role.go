...
func CreateControllerNodeRole(
	ctx context.Context,
	tx *sql.Tx,
	request *models.CreateControllerNodeRoleRequest) error {
	...
	_, err = stmt.ExecContext(ctx, string(model.GetUUID()),
		string(model.GetStorageManagementBondInterfaceMembers()),
		...
		string(model.GetPerformanceDrives()),
		string(model.GetParentUUID()),
		string(model.GetParentType()),
		string(model.GetInternalapiBondInterfaceMembers()),
		...
		string(model.GetCapacityDrives()),
	...
	err = common.CreateSharing(tx, "controller_node_role", model.UUID, model.GetPerms2().GetShare())
	if err != nil {
		return err
	}
	log.WithFields(log.Fields{
...
func scanControllerNodeRole(values map[string]interface{}) (*models.ControllerNodeRole, error) {
	...
	if value, ok := values["storage_management_bond_interface_members"]; ok {
		...
		m.StorageManagementBondInterfaceMembers = schema.InterfaceToString(value)
	...
	if value, ok := values["performance_drives"]; ok {
		...
		m.PerformanceDrives = schema.InterfaceToString(value)
	...
	if value, ok := values["internalapi_bond_interface_members"]; ok {
		...
		m.InternalapiBondInterfaceMembers = schema.InterfaceToString(value)
	...
	if value, ok := values["capacity_drives"]; ok {
		...
		m.CapacityDrives = schema.InterfaceToString(value)
...
func ListControllerNodeRole(ctx context.Context, tx *sql.Tx, request *models.ListControllerNodeRoleRequest) (response *models.ListControllerNodeRoleResponse, err error) {
	var rows *sql.Rows
	qb := &common.ListQueryBuilder{}
	qb.Auth = common.GetAuthCTX(ctx)
	spec := request.Spec
	qb.Spec = spec
	qb.Table = "controller_node_role"
	qb.Fields = ControllerNodeRoleFields
	qb.RefFields = ControllerNodeRoleRefFields
	qb.BackRefFields = ControllerNodeRoleBackRefFields
	result := []*models.ControllerNodeRole{}
	...
	response = &models.ListControllerNodeRoleResponse{
		ControllerNodeRoles: result,
	}
	return response, nil
}

// UpdateControllerNodeRole updates a resource
func UpdateControllerNodeRole(
	ctx context.Context,
	tx *sql.Tx,
...
func DeleteControllerNodeRole(
	ctx context.Context,
	tx *sql.Tx,
	request *models.DeleteControllerNodeRoleRequest) error {
