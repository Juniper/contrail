...
func TestControllerNodeRole(t *testing.T) {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		models, err := ListControllerNodeRole(tx, &common.ListSpec{Limit: 1})
		if err != nil {
			return err
		}
		if len(models) != 1 {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteControllerNodeRole(tx, model.UUID,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteControllerNodeRole(tx, model.UUID, nil)
	})
	if err != nil {
