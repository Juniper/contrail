...
func (db *DB) createDiscoveryServiceAssignment(
	...
	err = common.CreateSharing(tx, "discovery_service_assignment", model.UUID, model.GetPerms2().GetShare())
	if err != nil {
		return err
	}
	log.WithFields(log.Fields{
	...
	if value, ok := values["backref_dsa_rule"]; ok {
		...
		for _, childResource := range childResources {
			...
			if propertyValue, ok := childResourceMap["subscriber"]; ok && propertyValue != nil {
				...
				json.Unmarshal(schema.InterfaceToBytes(propertyValue), &childModel.DsaRuleEntry.Subscriber)

			}

			if propertyValue, ok := childResourceMap["ep_version"]; ok && propertyValue != nil {

				childModel.DsaRuleEntry.Publisher.EpVersion = schema.InterfaceToString(propertyValue)

			}

			if propertyValue, ok := childResourceMap["ep_type"]; ok && propertyValue != nil {

				childModel.DsaRuleEntry.Publisher.EpType = schema.InterfaceToString(propertyValue)

			}

			if propertyValue, ok := childResourceMap["ip_prefix_len"]; ok && propertyValue != nil {

				childModel.DsaRuleEntry.Publisher.EpPrefix.IPPrefixLen = schema.InterfaceToInt64(propertyValue)

			}

			if propertyValue, ok := childResourceMap["ip_prefix"]; ok && propertyValue != nil {

				childModel.DsaRuleEntry.Publisher.EpPrefix.IPPrefix = schema.InterfaceToString(propertyValue)

			}

			if propertyValue, ok := childResourceMap["ep_id"]; ok && propertyValue != nil {

				childModel.DsaRuleEntry.Publisher.EpID = schema.InterfaceToString(propertyValue)
