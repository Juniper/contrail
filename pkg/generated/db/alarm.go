...
func (db *DB) createAlarm(
	...
	_, err = stmt.ExecContext(ctx, common.MustJSON(model.GetUveKeys().GetUveKey()),
	...
	err = common.CreateSharing(tx, "alarm", model.UUID, model.GetPerms2().GetShare())
	if err != nil {
		return err
	}
	log.WithFields(log.Fields{
...
func scanAlarm(values map[string]interface{}) (*models.Alarm, error) {
	...
	if value, ok := values["alarm_severity"]; ok {
		...
		m.AlarmSeverity = schema.InterfaceToInt64(value)
...
func (db *DB) listAlarm(ctx context.Context, request *models.ListAlarmRequest) (response *models.ListAlarmResponse, err error) {
	...
	qb.Spec = spec
	qb.Table = "alarm"
	qb.Fields = AlarmFields
	qb.RefFields = AlarmRefFields
	qb.BackRefFields = AlarmBackRefFields
	result := []*models.Alarm{}
