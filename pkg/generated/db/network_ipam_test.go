...
func TestNetworkIpam(t *testing.T) {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateVirtualDNS(tx, VirtualDNSrefModel)
	})
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualDNS(tx, "network_ipam_virtual_DNS_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref network_ipam_virtual_DNS_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualDNS(tx, "network_ipam_virtual_DNS_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref network_ipam_virtual_DNS_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualDNS(tx, "network_ipam_virtual_DNS_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteNetworkIpam(tx, model.UUID,
