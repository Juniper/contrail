...
func TestContrailAnalyticsDatabaseNodeRole(t *testing.T) {
	...
	mutexTable := common.UseTable(db, "contrail_analytics_database_node_role")
	defer func() {
		mutexTable.Unlock()
	...
	projectModel.UUID = "contrail_analytics_database_node_role_admin_project_uuid"
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateContrailAnalyticsDatabaseNodeRole(ctx, tx,
			&models.CreateContrailAnalyticsDatabaseNodeRoleRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListContrailAnalyticsDatabaseNodeRole(ctx, tx, &models.ListContrailAnalyticsDatabaseNodeRoleRequest{
			Spec: &models.ListSpec{Limit: 1}})
		if err != nil {
			return err
		}
		if len(response.ContrailAnalyticsDatabaseNodeRoles) != 1 {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteContrailAnalyticsDatabaseNodeRole(ctxDemo, tx,
			&models.DeleteContrailAnalyticsDatabaseNodeRoleRequest{
				ID: model.UUID},
		)
	})
	if err == nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteContrailAnalyticsDatabaseNodeRole(ctx, tx,
			&models.DeleteContrailAnalyticsDatabaseNodeRoleRequest{
				ID: model.UUID})
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		...
		if len(response.ContrailAnalyticsDatabaseNodeRoles) != 0 {
