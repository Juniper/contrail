...
func TestVirtualDNS(t *testing.T) {
	...
	mutexTable := common.UseTable(db, "virtual_DNS")
	// mutexProject := common.UseTable(db, "virtual_DNS")
	defer func() {
		mutexTable.Unlock()
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateVirtualDNS(ctx, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListVirtualDNS(ctx, tx, &models.ListVirtualDNSRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualDNS(ctxDemo, tx,
