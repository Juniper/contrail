...
func TestBaremetalPort(t *testing.T) {
	...
	mutexTable := common.UseTable(db, "baremetal_port")
	// mutexProject := common.UseTable(db, "baremetal_port")
	defer func() {
		mutexTable.Unlock()
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateBaremetalPort(ctx, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListBaremetalPort(ctx, tx, &models.ListBaremetalPortRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBaremetalPort(ctxDemo, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBaremetalPort(ctx, tx,
			&models.DeleteBaremetalPortRequest{
				ID: model.UUID})
	})
	if err != nil {
