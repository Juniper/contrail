...
func TestBaremetalPort(t *testing.T) {
	...
	mutexTable := common.UseTable(db.DB, "baremetal_port")
	// mutexProject := common.UseTable(db.DB, "baremetal_port")
	defer func() {
		mutexTable.Unlock()
	...
	//
	//
	_, err = db.CreateBaremetalPort(ctx,
		&models.CreateBaremetalPortRequest{
			BaremetalPort: model,
		})
	...
	response, err := db.ListBaremetalPort(ctx, &models.ListBaremetalPortRequest{
		Spec: &models.ListSpec{Limit: 1}})
	if err != nil {
		t.Fatal("list failed", err)
	}
	if len(response.BaremetalPorts) != 1 {
		t.Fatal("expected one element", err)
	}

	ctxDemo := context.WithValue(ctx, "auth", common.NewAuthContext("default", "demo", "demo", []string{}))
	_, err = db.DeleteBaremetalPort(ctxDemo,
		&models.DeleteBaremetalPortRequest{
			ID: model.UUID},
	)
	if err == nil {
		t.Fatal("auth failed")
	...
	_, err = db.DeleteBaremetalPort(ctx,
		&models.DeleteBaremetalPortRequest{
			ID: model.UUID})
	if err != nil {
	...
	if len(response.BaremetalPorts) != 0 {
