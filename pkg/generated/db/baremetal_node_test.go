...
func TestBaremetalNode(t *testing.T) {
	...
	mutexTable := common.UseTable(db.DB, "baremetal_node")
	// mutexProject := common.UseTable(db.DB, "baremetal_node")
	defer func() {
		mutexTable.Unlock()
	...
	//
	//
	_, err = db.CreateBaremetalNode(ctx,
	...
	response, err := db.ListBaremetalNode(ctx, &models.ListBaremetalNodeRequest{
	...
	_, err = db.DeleteBaremetalNode(ctxDemo,
	...
	_, err = db.DeleteBaremetalNode(ctx,
		&models.DeleteBaremetalNodeRequest{
			ID: model.UUID})
	if err != nil {
