...
func TestBaremetalNode(t *testing.T) {
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        return CreateBaremetalNode(tx, model)
    })
    if err != nil {
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        models, err := ListBaremetalNode(tx, &common.ListSpec{Limit: 1})
        if err != nil {
            return err
        }
        if len(models) != 1 {
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        return DeleteBaremetalNode(tx, model.UUID, 
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        return DeleteBaremetalNode(tx, model.UUID, nil)
    })
    if err != nil {
