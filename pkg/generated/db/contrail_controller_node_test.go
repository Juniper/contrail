...
func TestContrailControllerNode(t *testing.T) {
	...
	mutexTable := common.UseTable(db, "contrail_controller_node")
	// mutexProject := common.UseTable(db, "contrail_controller_node")
	defer func() {
		mutexTable.Unlock()
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateContrailControllerNode(ctx, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListContrailControllerNode(ctx, tx, &models.ListContrailControllerNodeRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteContrailControllerNode(ctxDemo, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteContrailControllerNode(ctx, tx,
			&models.DeleteContrailControllerNodeRequest{
				ID: model.UUID})
	})
	if err != nil {
