...
func CreateAccessControlList(
	...
	err = common.CreateSharing(tx, "access_control_list", model.UUID, model.GetPerms2().GetShare())
	if err != nil {
		return err
	}
	log.WithFields(log.Fields{
...
func scanAccessControlList(values map[string]interface{}) (*models.AccessControlList, error) {
	...
	if value, ok := values["access_control_list_hash"]; ok {
		...
		m.AccessControlListHash = schema.InterfaceToInt64(value)

	}

	if value, ok := values["dynamic"]; ok {

		m.AccessControlListEntries.Dynamic = schema.InterfaceToBool(value)
