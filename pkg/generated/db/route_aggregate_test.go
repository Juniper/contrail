...
func TestRouteAggregate(t *testing.T) {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteServiceInstance(tx, "route_aggregate_service_instance_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref route_aggregate_service_instance_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteServiceInstance(tx, "route_aggregate_service_instance_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref route_aggregate_service_instance_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteServiceInstance(tx, "route_aggregate_service_instance_ref_uuid2", nil)
    })
	if err != nil {
