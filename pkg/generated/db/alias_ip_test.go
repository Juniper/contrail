...
func TestAliasIP(t *testing.T) {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteProject(tx, "alias_ip_project_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref alias_ip_project_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteProject(tx, "alias_ip_project_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref alias_ip_project_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteProject(tx, "alias_ip_project_ref_uuid2", nil)
    })
	if err != nil {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualMachineInterface(tx, "alias_ip_virtual_machine_interface_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref alias_ip_virtual_machine_interface_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualMachineInterface(tx, "alias_ip_virtual_machine_interface_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref alias_ip_virtual_machine_interface_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteVirtualMachineInterface(tx, "alias_ip_virtual_machine_interface_ref_uuid2", nil)
    })
	if err != nil {
