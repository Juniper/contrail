...
func TestSecurityGroup(t *testing.T) {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return UpdateSecurityGroup(tx, model.UUID, model)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		models, err := ListSecurityGroup(tx, &common.ListSpec{Limit: 1})
		if err != nil {
			return err
		}
		if len(models) != 1 {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteSecurityGroup(tx, model.UUID,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteSecurityGroup(tx, model.UUID, nil)
	})
	if err != nil {
