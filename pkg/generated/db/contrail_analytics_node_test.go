...
func TestContrailAnalyticsNode(t *testing.T) {
	...
	mutexTable := common.UseTable(db.DB, "contrail_analytics_node")
	// mutexProject := common.UseTable(db.DB, "contrail_analytics_node")
	defer func() {
		mutexTable.Unlock()
	...
	if len(response.ContrailAnalyticsNodes) != 0 {
