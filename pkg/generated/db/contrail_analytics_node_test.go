...
func TestContrailAnalyticsNode(t *testing.T) {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListContrailAnalyticsNode(ctx, tx, &models.ListContrailAnalyticsNodeRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteContrailAnalyticsNode(ctxDemo, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteContrailAnalyticsNode(ctx, tx,
			&models.DeleteContrailAnalyticsNodeRequest{
				ID: model.UUID})
	})
	if err != nil {
