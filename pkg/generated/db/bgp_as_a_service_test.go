...
func TestBGPAsAService(t *testing.T) {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "bgp_as_a_service_virtual_machine_interface_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref bgp_as_a_service_virtual_machine_interface_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "bgp_as_a_service_virtual_machine_interface_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref bgp_as_a_service_virtual_machine_interface_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "bgp_as_a_service_virtual_machine_interface_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteServiceHealthCheck(tx, "bgp_as_a_service_service_health_check_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref bgp_as_a_service_service_health_check_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteServiceHealthCheck(tx, "bgp_as_a_service_service_health_check_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref bgp_as_a_service_service_health_check_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteServiceHealthCheck(tx, "bgp_as_a_service_service_health_check_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		models, err := ListBGPAsAService(tx, &common.ListSpec{Limit: 1})
		if err != nil {
			return err
		}
		if len(models) != 1 {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBGPAsAService(tx, model.UUID,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBGPAsAService(tx, model.UUID, nil)
	})
	if err != nil {
