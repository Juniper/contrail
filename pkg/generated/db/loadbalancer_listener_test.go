...
func TestLoadbalancerListener(t *testing.T) {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateLoadbalancer(tx, LoadbalancerrefModel)
	})
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteLoadbalancer(tx, "loadbalancer_listener_loadbalancer_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref loadbalancer_listener_loadbalancer_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteLoadbalancer(tx, "loadbalancer_listener_loadbalancer_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref loadbalancer_listener_loadbalancer_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteLoadbalancer(tx, "loadbalancer_listener_loadbalancer_ref_uuid2", nil)
    })
	if err != nil {
