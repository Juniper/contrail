...
func TestServiceAppliance(t *testing.T) {
	...
	mutexTable := common.UseTable(db, "service_appliance")
	// mutexProject := common.UseTable(db, "service_appliance")
	defer func() {
		mutexTable.Unlock()
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateServiceAppliance(ctx, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListServiceAppliance(ctx, tx, &models.ListServiceApplianceRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteServiceAppliance(ctxDemo, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteServiceAppliance(ctx, tx,
			&models.DeleteServiceApplianceRequest{
				ID: model.UUID})
	})
	if err != nil {
