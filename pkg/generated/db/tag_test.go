...
func TestTag(t *testing.T) {
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateTagType(tx, TagTyperefModel)
	})
    ...
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteTagType(tx, "tag_tag_type_ref_uuid", nil)
    })
	if err != nil {
		t.Fatal("delete ref tag_tag_type_ref_uuid  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteTagType(tx, "tag_tag_type_ref_uuid1", nil)
    })
	if err != nil {
		t.Fatal("delete ref tag_tag_type_ref_uuid1  failed", err)
	}
    err = common.DoInTransaction(db, func(tx *sql.Tx) error {
    	return DeleteTagType(tx, "tag_tag_type_ref_uuid2", nil)
    })
	if err != nil {
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        models, err := ListTag(tx, &common.ListSpec{Limit: 1})
        if err != nil {
            return err
        }
        if len(models) != 1 {
    ...
    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
        return DeleteTag(tx, model.UUID, nil)
    })
    if err != nil {
