...
func TestBridgeDomain(t *testing.T) {
	...
	projectModel.UUID = "bridge_domain_admin_project_uuid"
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateBridgeDomain(ctx, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListBridgeDomain(ctx, tx, &models.ListBridgeDomainRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBridgeDomain(ctxDemo, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		...
		if len(response.BridgeDomains) != 0 {
