...
func (db *DB) createVirtualNetwork(
	...
	for _, ref := range model.NetworkIpamRefs {
		...
		_, err = stmtNetworkIpamRef.ExecContext(ctx, model.UUID, ref.UUID, common.MustJSON(ref.Attr.GetHostRoutes().GetRoute()),
	...
	for _, ref := range model.NetworkPolicyRefs {
		...
		_, err = stmtNetworkPolicyRef.ExecContext(ctx, model.UUID, ref.UUID, string(ref.Attr.GetTimer().GetOffInterval()),
