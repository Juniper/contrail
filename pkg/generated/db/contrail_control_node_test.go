...
func TestContrailControlNode(t *testing.T) {
	...
	mutexTable := common.UseTable(db, "contrail_control_node")
	// mutexProject := common.UseTable(db, "contrail_control_node")
	defer func() {
		mutexTable.Unlock()
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateContrailControlNode(ctx, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListContrailControlNode(ctx, tx, &models.ListContrailControlNodeRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteContrailControlNode(ctxDemo, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteContrailControlNode(ctx, tx,
			&models.DeleteContrailControlNodeRequest{
				ID: model.UUID})
	})
	if err != nil {
