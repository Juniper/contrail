...
func TestCustomerAttachment(t *testing.T) {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateFloatingIP(tx, FloatingIPrefModel)
	})
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "customer_attachment_virtual_machine_interface_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref customer_attachment_virtual_machine_interface_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "customer_attachment_virtual_machine_interface_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref customer_attachment_virtual_machine_interface_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteVirtualMachineInterface(tx, "customer_attachment_virtual_machine_interface_ref_uuid2", nil)
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteFloatingIP(tx, "customer_attachment_floating_ip_ref_uuid", nil)
	})
	if err != nil {
		t.Fatal("delete ref customer_attachment_floating_ip_ref_uuid  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteFloatingIP(tx, "customer_attachment_floating_ip_ref_uuid1", nil)
	})
	if err != nil {
		t.Fatal("delete ref customer_attachment_floating_ip_ref_uuid1  failed", err)
	}
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteFloatingIP(tx, "customer_attachment_floating_ip_ref_uuid2", nil)
	})
	if err != nil {
