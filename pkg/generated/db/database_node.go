...
func CreateDatabaseNode(
    ...
    err = common.CreateSharing(tx, "database_node", model.UUID, model.GetPerms2().GetShare())
    if err != nil {
        return err
    }
    log.WithFields(log.Fields{
...
func scanDatabaseNode(values map[string]interface{} ) (*models.DatabaseNode, error) {
    ...
    if value, ok := values["database_node_ip_address"]; ok {
               ...
               m.DatabaseNodeIPAddress = schema.InterfaceToString(value)
...
func ListDatabaseNode(ctx context.Context, tx *sql.Tx, request *models.ListDatabaseNodeRequest) (response *models.ListDatabaseNodeResponse, err error) {
    var rows *sql.Rows
    qb := &common.ListQueryBuilder{}
    qb.Auth = common.GetAuthCTX(ctx) 
    spec := request.Spec
    qb.Spec = spec
    qb.Table = "database_node"
    qb.Fields = DatabaseNodeFields
    qb.RefFields = DatabaseNodeRefFields
    qb.BackRefFields = DatabaseNodeBackRefFields
