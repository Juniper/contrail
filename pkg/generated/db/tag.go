...
func (db *DB) createTag(
	...
	for _, ref := range model.TagTypeRefs {
		...
		_, err = stmtTagTypeRef.ExecContext(ctx, model.UUID, ref.UUID)
		if err != nil {
	...
	err = common.CreateSharing(tx, "tag", model.UUID, model.GetPerms2().GetShare())
	if err != nil {
		return err
	}
	log.WithFields(log.Fields{
...
func scanTag(values map[string]interface{}) (*models.Tag, error) {
	...
	if value, ok := values["tag_value"]; ok {
		...
		m.TagValue = schema.InterfaceToString(value)

	}

	if value, ok := values["tag_type_name"]; ok {

		m.TagTypeName = schema.InterfaceToString(value)

	}

	if value, ok := values["tag_id"]; ok {

		m.TagID = schema.InterfaceToString(value)
...
func (db *DB) listTag(ctx context.Context, request *models.ListTagRequest) (response *models.ListTagResponse, err error) {
	...
	qb.Table = "tag"
	qb.Fields = TagFields
	qb.RefFields = TagRefFields
	qb.BackRefFields = TagBackRefFields
	result := []*models.Tag{}
