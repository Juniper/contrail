...
func TestOpenstackStorageNodeRole(t *testing.T) {
	...
	mutexTable := common.UseTable(db, "openstack_storage_node_role")
	defer func() {
		mutexTable.Unlock()
	...
	projectModel.UUID = "openstack_storage_node_role_admin_project_uuid"
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateOpenstackStorageNodeRole(ctx, tx,
			&models.CreateOpenstackStorageNodeRoleRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListOpenstackStorageNodeRole(ctx, tx, &models.ListOpenstackStorageNodeRoleRequest{
			Spec: &models.ListSpec{Limit: 1}})
		if err != nil {
			return err
		}
		if len(response.OpenstackStorageNodeRoles) != 1 {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteOpenstackStorageNodeRole(ctxDemo, tx,
			&models.DeleteOpenstackStorageNodeRoleRequest{
				ID: model.UUID},
		)
	})
	if err == nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteOpenstackStorageNodeRole(ctx, tx,
			&models.DeleteOpenstackStorageNodeRoleRequest{
				ID: model.UUID})
	})
	if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		...
		if len(response.OpenstackStorageNodeRoles) != 0 {
