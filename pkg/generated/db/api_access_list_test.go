...
func TestAPIAccessList(t *testing.T) {
	...
	//
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return CreateAPIAccessList(ctx, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListAPIAccessList(ctx, tx, &models.ListAPIAccessListRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteAPIAccessList(ctxDemo, tx,
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteAPIAccessList(ctx, tx,
			&models.DeleteAPIAccessListRequest{
				ID: model.UUID})
	})
	if err != nil {
