...
func TestBGPVPN(t *testing.T) {
	...
	projectModel.UUID = "bgpvpn_admin_project_uuid"
	...
	//
	//
	//    common.SetValueByPath(updateMap, ".BGPVPNType", ".", "test")
	//
	//
	//
	...
	//
	//
	//    common.SetValueByPath(updateMap, "uuid", ".", "bgpvpn_dummy_uuid")
	...
	//    err = common.DoInTransaction(db, func (tx *sql.Tx) error {
	//        return UpdateBGPVPN(tx, model.UUID, updateMap)
	//    })
	//    if err != nil {
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		response, err := ListBGPVPN(ctx, tx, &models.ListBGPVPNRequest{
	...
	err = common.DoInTransaction(db, func(tx *sql.Tx) error {
		return DeleteBGPVPN(ctxDemo, tx,
