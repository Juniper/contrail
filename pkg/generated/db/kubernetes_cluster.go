...
func CreateKubernetesCluster(
	ctx context.Context,
	tx *sql.Tx,
	request *models.CreateKubernetesClusterRequest) error {
	...
	_, err = stmt.ExecContext(ctx, string(model.GetUUID()),
		...
		string(model.GetKuberunetesDashboard()),
	...
	err = common.CreateSharing(tx, "kubernetes_cluster", model.UUID, model.GetPerms2().GetShare())
	if err != nil {
		return err
	}
	log.WithFields(log.Fields{
...
func scanKubernetesCluster(values map[string]interface{}) (*models.KubernetesCluster, error) {
	...
	if value, ok := values["kuberunetes_dashboard"]; ok {
		...
		m.KuberunetesDashboard = common.InterfaceToString(value)
...
func ListKubernetesCluster(ctx context.Context, tx *sql.Tx, request *models.ListKubernetesClusterRequest) (response *models.ListKubernetesClusterResponse, err error) {
	var rows *sql.Rows
	qb := &common.ListQueryBuilder{}
	qb.Auth = common.GetAuthCTX(ctx)
	spec := request.Spec
	qb.Spec = spec
	qb.Table = "kubernetes_cluster"
	qb.Fields = KubernetesClusterFields
	qb.RefFields = KubernetesClusterRefFields
	qb.BackRefFields = KubernetesClusterBackRefFields
	result := []*models.KubernetesCluster{}
	...
	response = &models.ListKubernetesClusterResponse{
		KubernetesClusters: result,
	}
	return response, nil
}
...
func DeleteKubernetesCluster(
	ctx context.Context,
	tx *sql.Tx,
	request *models.DeleteKubernetesClusterRequest) error {
