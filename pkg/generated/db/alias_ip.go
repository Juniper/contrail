...
func CreateAliasIP(
	...
	err = common.CreateSharing(tx, "alias_ip", model.UUID, model.GetPerms2().GetShare())
	if err != nil {
		return err
	}
	log.WithFields(log.Fields{
...
func scanAliasIP(values map[string]interface{}) (*models.AliasIP, error) {
	...
	if value, ok := values["alias_ip_address_family"]; ok {
		...
		m.AliasIPAddressFamily = schema.InterfaceToString(value)

	}

	if value, ok := values["alias_ip_address"]; ok {

		m.AliasIPAddress = schema.InterfaceToString(value)
