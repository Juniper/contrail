...
import "fmt"
func TestDiscoveryServiceAssignmentType(t *testing.T) {
    model := MakeDiscoveryServiceAssignmentType()
    fmt.Println(model)
}


func TestDiscoveryServiceAssignmentTypeFQName(t *testing.T) {
    obj := MakeDiscoveryServiceAssignmentType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for DiscoveryServiceAssignmentType")
    }
    parent := "fake-parent"
