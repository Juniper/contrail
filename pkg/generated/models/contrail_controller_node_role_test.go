...
func TestContrailControllerNodeRoleFQName(t *testing.T) {
    obj := MakeContrailControllerNodeRole()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for ContrailControllerNodeRole")
    }
    parent := "fake-parent"
