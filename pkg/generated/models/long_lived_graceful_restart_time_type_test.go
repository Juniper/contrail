...
func TestLongLivedGracefulRestartTimeTypeFQName(t *testing.T) {
    obj := MakeLongLivedGracefulRestartTimeType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for LongLivedGracefulRestartTimeType")
    }
    parent := "fake-parent"
