...
func TestVxlanNetworkIdentifierTypeFQName(t *testing.T) {
    obj := MakeVxlanNetworkIdentifierType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for VxlanNetworkIdentifierType")
    }
    parent := "fake-parent"
