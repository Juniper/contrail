...
import "fmt"
func TestLinklocalServicesTypes(t *testing.T) {
    model := MakeLinklocalServicesTypes()
    fmt.Println(model)
}


func TestLinklocalServicesTypesFQName(t *testing.T) {
    obj := MakeLinklocalServicesTypes()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for LinklocalServicesTypes")
    }
    parent := "fake-parent"
