...
func (model *KuberunetesNode) String() string {
	b, _ := json.Marshal(model)
	return string(b)
}

func MakeKuberunetesNode() *KuberunetesNode {
	return &KuberunetesNode{
...
func InterfaceToKuberunetesNode(iData interface{}) *KuberunetesNode {
...
func InterfaceToKuberunetesNodeSlice(data interface{}) []*KuberunetesNode {
	list := data.([]interface{})
	result := MakeKuberunetesNodeSlice()
	for _, item := range list {
		result = append(result, InterfaceToKuberunetesNode(item))
	}
	return result
}

func MakeKuberunetesNodeSlice() []*KuberunetesNode {
	return []*KuberunetesNode{}
}
