...
func TestFirewallRuleMatchTagsTypeIdListFQName(t *testing.T) {
    obj := MakeFirewallRuleMatchTagsTypeIdList()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for FirewallRuleMatchTagsTypeIdList")
    }
    parent := "fake-parent"
