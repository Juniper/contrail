...
func TestPolicyBasedForwardingRuleTypeFQName(t *testing.T) {
    obj := MakePolicyBasedForwardingRuleType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for PolicyBasedForwardingRuleType")
    }
    parent := "fake-parent"
