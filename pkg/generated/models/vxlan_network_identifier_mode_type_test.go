...
func TestVxlanNetworkIdentifierModeTypeFQName(t *testing.T) {
    obj := MakeVxlanNetworkIdentifierModeType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for VxlanNetworkIdentifierModeType")
    }
    parent := "fake-parent"
