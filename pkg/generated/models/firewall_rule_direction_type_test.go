...
func TestFirewallRuleDirectionTypeFQName(t *testing.T) {
    obj := MakeFirewallRuleDirectionType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for FirewallRuleDirectionType")
    }
    parent := "fake-parent"
