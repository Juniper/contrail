...
func TestDiscoveryServiceAssignmentFQName(t *testing.T) {
    obj := MakeDiscoveryServiceAssignment()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for DiscoveryServiceAssignment")
    }
    parent := "fake-parent"
