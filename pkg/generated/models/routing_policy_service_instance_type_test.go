...
func TestRoutingPolicyServiceInstanceTypeFQName(t *testing.T) {
    obj := MakeRoutingPolicyServiceInstanceType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for RoutingPolicyServiceInstanceType")
    }
    parent := "fake-parent"
