...
func TestServiceInstanceInterfaceTypeFQName(t *testing.T) {
    obj := MakeServiceInstanceInterfaceType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for ServiceInstanceInterfaceType")
    }
    parent := "fake-parent"
