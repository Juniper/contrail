...
func TestQosIdForwardingClassPairsFQName(t *testing.T) {
    obj := MakeQosIdForwardingClassPairs()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for QosIdForwardingClassPairs")
    }
    parent := "fake-parent"
