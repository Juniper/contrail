...
func TestEncapsulationPrioritiesTypeFQName(t *testing.T) {
    obj := MakeEncapsulationPrioritiesType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for EncapsulationPrioritiesType")
    }
    parent := "fake-parent"
