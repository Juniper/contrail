...
import "fmt"
func TestMACMoveLimitControlType(t *testing.T) {
    model := MakeMACMoveLimitControlType()
    fmt.Println(model)
}


func TestMACMoveLimitControlTypeFQName(t *testing.T) {
    obj := MakeMACMoveLimitControlType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for MACMoveLimitControlType")
    }
    parent := "fake-parent"
