...
func TestDiscoveryPubSubEndPointTypeFQName(t *testing.T) {
    obj := MakeDiscoveryPubSubEndPointType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for DiscoveryPubSubEndPointType")
    }
    parent := "fake-parent"
