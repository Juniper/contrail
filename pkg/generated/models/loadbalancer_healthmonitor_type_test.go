...
func TestLoadbalancerHealthmonitorTypeFQName(t *testing.T) {
    obj := MakeLoadbalancerHealthmonitorType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for LoadbalancerHealthmonitorType")
    }
    parent := "fake-parent"
