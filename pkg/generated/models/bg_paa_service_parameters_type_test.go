...
func TestBGPaaServiceParametersTypeFQName(t *testing.T) {
    obj := MakeBGPaaServiceParametersType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for BGPaaServiceParametersType")
    }
    parent := "fake-parent"
