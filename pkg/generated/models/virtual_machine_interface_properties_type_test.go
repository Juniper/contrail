...
func TestVirtualMachineInterfacePropertiesTypeFQName(t *testing.T) {
    obj := MakeVirtualMachineInterfacePropertiesType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for VirtualMachineInterfacePropertiesType")
    }
    parent := "fake-parent"
