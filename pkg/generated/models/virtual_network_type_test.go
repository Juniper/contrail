...
import "fmt"
func TestVirtualNetworkType(t *testing.T) {
    model := MakeVirtualNetworkType()
    fmt.Println(model)
}


func TestVirtualNetworkTypeFQName(t *testing.T) {
    obj := MakeVirtualNetworkType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for VirtualNetworkType")
    }
    parent := "fake-parent"
