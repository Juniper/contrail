...
func TestLinklocalServiceEntryTypeFQName(t *testing.T) {
    obj := MakeLinklocalServiceEntryType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for LinklocalServiceEntryType")
    }
    parent := "fake-parent"
