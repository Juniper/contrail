...
func TestGracefulRestartParametersTypeFQName(t *testing.T) {
    obj := MakeGracefulRestartParametersType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for GracefulRestartParametersType")
    }
    parent := "fake-parent"
