...
func TestSecurityLoggingObjectRuleEntryTypeFQName(t *testing.T) {
    obj := MakeSecurityLoggingObjectRuleEntryType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for SecurityLoggingObjectRuleEntryType")
    }
    parent := "fake-parent"
