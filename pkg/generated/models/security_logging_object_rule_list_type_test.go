...
func TestSecurityLoggingObjectRuleListTypeFQName(t *testing.T) {
    obj := MakeSecurityLoggingObjectRuleListType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for SecurityLoggingObjectRuleListType")
    }
    parent := "fake-parent"
