...
func TestVirtualRouterNetworkIpamTypeFQName(t *testing.T) {
    obj := MakeVirtualRouterNetworkIpamType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for VirtualRouterNetworkIpamType")
    }
    parent := "fake-parent"
