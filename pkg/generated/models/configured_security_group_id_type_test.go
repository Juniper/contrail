...
func TestConfiguredSecurityGroupIdTypeFQName(t *testing.T) {
    obj := MakeConfiguredSecurityGroupIdType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for ConfiguredSecurityGroupIdType")
    }
    parent := "fake-parent"
