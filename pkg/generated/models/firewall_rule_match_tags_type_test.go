...
func TestFirewallRuleMatchTagsTypeFQName(t *testing.T) {
    obj := MakeFirewallRuleMatchTagsType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for FirewallRuleMatchTagsType")
    }
    parent := "fake-parent"
