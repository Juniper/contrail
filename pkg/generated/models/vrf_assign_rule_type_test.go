...
import "fmt"
func TestVrfAssignRuleType(t *testing.T) {
    model := MakeVrfAssignRuleType()
    fmt.Println(model)
}


func TestVrfAssignRuleTypeFQName(t *testing.T) {
    obj := MakeVrfAssignRuleType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for VrfAssignRuleType")
    }
    parent := "fake-parent"
