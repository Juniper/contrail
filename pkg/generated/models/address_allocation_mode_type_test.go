...
func TestAddressAllocationModeTypeFQName(t *testing.T) {
    obj := MakeAddressAllocationModeType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for AddressAllocationModeType")
    }
    parent := "fake-parent"
