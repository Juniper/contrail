...
func TestServiceVirtualizationTypeFQName(t *testing.T) {
    obj := MakeServiceVirtualizationType()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for ServiceVirtualizationType")
    }
    parent := "fake-parent"
