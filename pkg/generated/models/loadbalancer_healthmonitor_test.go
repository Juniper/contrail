...
func TestLoadbalancerHealthmonitorFQName(t *testing.T) {
    obj := MakeLoadbalancerHealthmonitor()
    if fqname := obj.GetFQName(); len(fqname != 0) {
        t.Errorf("Initial FQName is not empty for LoadbalancerHealthmonitor")
    }
    parent := "fake-parent"
