...
type OpenstackComputeNodeRoleUpdateRequest struct {
...
func (api *OpenstackComputeNodeRoleRESTAPI) Update(c echo.Context) error {
	id := c.Param("id")
	requestData := &OpenstackComputeNodeRoleUpdateRequest{}
	if err := c.Bind(requestData); err != nil {
		log.WithFields(log.Fields{
			"err":      err,
			"resource": "openstack_compute_node_role",
	...
	return c.JSON(http.StatusOK, map[string]map[string]string{
		"openstack-compute-node-role": {
			"uuid": id,
			"uri":  "/" + "openstack-compute-node-role" + "/" + id},
	})
}
