...
func getXflowEndpointAddress(xflowData *XflowData) (string, error) {
	...
	if len(xflowData.ClusterInfo.AppformixFlowsNodes) == 1 {
		for _, xflowNode := range xflowData.ClusterInfo.AppformixFlowsNodes {
			for _, nodeRef := range xflowNode.NodeRefs {
				for _, nodeInfo := range xflowData.NodesInfo {
					if nodeRef.UUID == xflowNode.UUID {
