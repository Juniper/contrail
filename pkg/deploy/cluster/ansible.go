...
func (a *contrailAnsibleDeployer) createInstancesFile(destination string) error {
	...
	content, err := template.Apply(a.getInstanceTemplate(), pContext)
	if err != nil {
		return err
	}
...
func (a *contrailAnsibleDeployer) createDatapathEncryptionInventory(destination string) error {
	...
	content, err := template.Apply(a.getInventoryTemplate(), pContext)
	if err != nil {
		return err
	}
	if err = fileutil.WriteToFile(destination, content, defaultFilePermRWOnly); err != nil {
...
func (a *contrailAnsibleDeployer) createVcenterVarsFile(destination string) error {
	...
	content, err := template.Apply(a.getVcenterTemplate(), pContext)
	if err != nil {
		return err
	}
	if err = fileutil.WriteToFile(destination, content, defaultFilePermRWOnly); err != nil {
