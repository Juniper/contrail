...
const (
	...
	mcCreateFailPath    = "./test_data/multicloud/test_mc_create_fail.yml"
	...
	mcWorkRoot  = "/tmp/contrail_cluster"

	generatedSecret         = mcWorkRoot + "/" + mcClusterID + "/" + mcWorkDir + "/" + defaultSecretFile
	generatedTopology       = mcWorkRoot + "/" + mcClusterID + "/" + mcWorkDir + "/" + defaultTopologyFile
	generatedContrailCommon = mcWorkRoot + "/" + mcClusterID + "/" + mcWorkDir + "/" + defaultContrailCommonFile
	generatedGatewayCommon  = mcWorkRoot + "/" + mcClusterID + "/" + mcWorkDir + "/" + defaultGatewayCommonFile
	generatedTORCommon      = mcWorkRoot + "/" + mcClusterID + "/" + mcWorkDir + "/" + defaultTORCommonFile
...
func TestMCSuccess(t *testing.T) {
	// prerequisites
	...
	verifyClusterProvisioningStatus(context.Background(), t, c.APIServer, c.ClusterID, statusCreated)
	...
	//update mc
	makeRequests(t, mcClusterUpdatePath)

	cleanupMCFiles(t)
	c.Action = updateAction
	manageWithSuccess(t, c)
	assert.NoErrorf(t, isCloudSecretFilesDeleted(), "failed to delete public cloud secrets during %s", c.Action)
	verifyClusterProvisioningStatus(context.Background(), t, c.APIServer, c.ClusterID, statusUpdated)
	...
	//remove cloud
	makeRequests(t, mcClusterDeletePath)
	...
	manageWithSuccess(t, c)
	assert.True(t, verifyClusterDirDeleted(mcClusterID), "Cluster directory is not deleted during cluster delete")
}
...
func TestMCCreateFail(t *testing.T) {
	...
	//create mc
	cleanup := makeRequests(t, mcCreateFailPath)
	defer cleanup()

	cleanupMCFiles(t)
	manageWithFailure(t, c)
	assert.NoErrorf(t, isCloudSecretFilesDeleted(), "failed to delete public cloud secrets during %s", c.Action)
	verifyClusterProvisioningStatus(context.Background(), t, c.APIServer, c.ClusterID, statusCreateFailed)
...
func TestMCUpdateFail(t *testing.T) {
	...
	manageWithFailure(t, c)
	assert.NoErrorf(t, isCloudSecretFilesDeleted(), "failed to delete public cloud secrets during %s", c.Action)
	verifyClusterProvisioningStatus(context.Background(), t, c.APIServer, c.ClusterID, statusUpdateFailed)
...
func manageWithSuccess(t *testing.T, c *Config) {
	deployer := getDeployer(t, c)
	err := deployer.Deploy()
	assert.NoErrorf(t, err, "failed to manage(%s) cluster", c.Action)
}

func getDeployer(t *testing.T, c *Config) base.Deployer {
...
func compareGeneratedFiles(t *testing.T) {
	assertYAMLFileEqual(t, expectedMCClusterTopology, generatedTopology,
...
func verifyMCDeleted(httpClient *client.HTTP) bool {
	...
	if _, err := os.Stat(mcWorkRoot + "/" + mcClusterID + "/" + mcWorkDir); err == nil || !os.IsNotExist(err) {
