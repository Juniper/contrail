...
func generatedInstancesPath() string {
	return filepath.Join(workRoot, clusterID, defaultInstanceFile)
}

func generatedInventoryPath() string {
	return filepath.Join(workRoot, clusterID, defaultInventoryFile)
}

func generatedVcenterVarsPath() string {
	return filepath.Join(workRoot, clusterID, defaultVcenterFile)
}
...
func createDummyCloudSecretFile(t *testing.T) func() {
	...
	err := fileutil.CopyFile(src, dest, true)
...
func createDummyCloudFiles(t *testing.T) func() {
	...
	secretCleanup := createDummyCloudSecretFile(t)
...
func runClusterActionTest(
	...
	case addComputeProvisioningAction, addCVFMProvisioningAction,
		deleteComputeProvisioningAction, addCSNProvisioningAction:
...
func runTest(t *testing.T, expectedInstance, expectedInventory string,
	...
	ksClose := startMockKeystoneServer()
	defer ksClose()
	...
	for _, tt := range []struct {
		...
		action:            createAction,
		expectedPlaybooks: expectedPlaybooksList[0],
	}, {
		action:            updateAction,
		expectedPlaybooks: expectedPlaybooksList[1],
	}} {
		//cleanup old files
		...
		assertGeneratedInstancesEqual(t, expectedInstance,
			fmt.Sprintf("Instance file created during cluster %s is not as expected", tt.action))
		if expectedInventory != "" {
			assert.Truef(t, compareFiles(t, expectedInventory, generatedInventoryPath()),
				"Inventory file created during cluster %s is not as expected", tt.action)
		}
		assert.Truef(t, verifyPlaybooks(t, tt.expectedPlaybooks),
			"Expected list of %s playbooks are not executed", tt.action)
...
func startMockKeystoneServer() func() {
...
func runMCClusterTest(t *testing.T, pContext map[string]interface{}) {
	...
	for _, tt := range []struct {
		...
		assert.Truef(t, compareFiles(t, expectedMCClusterTopology, generatedTopologyPath()),
			"Topolgy file created during cluster %s is not as expected", tt.action)
		assert.Truef(t, compareFiles(t, expectedContrailCommon, generatedContrailCommonPath()),
			"Contrail common file created during cluster %s is not as expected", tt.action)
		assert.Truef(t, compareFiles(t, expectedGatewayCommon, generatedGatewayCommonPath()),
