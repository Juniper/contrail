...
var defaultAppFormixVer = []string{"appformix_version=3.0.0"}
...
func upgradeOpenstackPlaybookExecutions() []ansiblemock.ContainerExecution {
	return []ansiblemock.ContainerExecution {
		defaultPlayBookExecution(
			"playbooks/provision_instances.yml", defaultInventory, defaultConfigFile, openstackOrch,
		),
		defaultPlayBookExecution(
			"playbooks/configure_instances.yml", defaultInventory, defaultConfigFile, openstackOrch,
		),
		defaultPlayBookExecution(
			"playbooks/install_openstack.yml", defaultInventory, defaultConfigFile, openstackOrch, forceCheckout,
		),
		defaultPlayBookExecution(
			"playbooks/install_contrail.yml", defaultInventory, defaultConfigFile, openstackOrch,
		),
	}
}
...
func deleteComputeOpenstackPlaybookExecutions() []ansiblemock.ContainerExecution {
	return []ansiblemock.ContainerExecution {
		...
		defaultPlayBookExecution(
			"playbooks/install_openstack.yml", defaultInventory, defaultConfigFile, openstackOrch, forceCheckout, novaTag,
		),
		defaultPlayBookExecution(
			"playbooks/install_contrail.yml", defaultInventory, defaultConfigFile, openstackOrch,
		),
...
func defaultPlayBookExecution(playbook string, args ...playbookArg) ansiblemock.ContainerExecution {
	...
	return ansiblemock.ContainerExecution {
		...
		Parameters: &ansible.ContainerParameters{
			ImageRef:         "test_openstack_cluster_uuid",
			ImageRefUsername: "",
			ImageRefPassword: "",
...
func createUpdateAppformixPlaybookExecutions() []ansiblemock.ContainerExecution {
	return []ansiblemock.ContainerExecution {
		...
		defaultPlayBookExecution(
			"playbooks/install_openstack.yml", defaultInventory, defaultConfigFile, openstackOrch,
		),
		defaultPlayBookExecution(
			"playbooks/install_contrail.yml", defaultInventory, defaultConfigFile, openstackOrch,
		),
		// defaultPlayBookExecution(
		// 	"playbooks/install_appformix.yml", defaultConfigFile, defaultAppFormixVer, skipDockerInstall,
		// ),
	}
}
...
func destroyPlaybookExecutions() []ansiblemock.ContainerExecution {
	return []ansiblemock.ContainerExecution {
		defaultPlayBookExecution(
			"playbooks/openstack_destroy.yml", defaultInventory,
		),
		defaultPlayBookExecution(
			"playbooks/contrail_destroy.yml", defaultInventory,
		),
	}
}
...
func TestAllInOneVfabricManager(t *testing.T) {
	...
	runTest(
		...
		createOpenstackPlaybookExecutions(),
		updateOpenstackPlaybookExecutions(),
		[]clusterActionTestSpec{{
			action:            cluster.UpgradeProvisioningAction,
...
func createUpdateEncryptOpenstackPlaybookExecutions() []ansiblemock.ContainerExecution {
	return []ansiblemock.ContainerExecution {
		...
		defaultPlayBookExecution(
			"playbooks/deploy_and_run_all.yml", encryptInventory,
		),
	}
}
...
func createUpdateKubernetesPlaybookExecutions() []ansiblemock.ContainerExecution {
	return []ansiblemock.ContainerExecution {
		defaultPlayBookExecution(
			"playbooks/provision_instances.yml", defaultInventory, defaultConfigFile, k8sOrch,
		),
		defaultPlayBookExecution(
			"playbooks/configure_instances.yml", defaultInventory, defaultConfigFile, k8sOrch,
		),
		defaultPlayBookExecution(
			"playbooks/install_k8s.yml", defaultInventory, defaultConfigFile, k8sOrch,
		),
		defaultPlayBookExecution(
			"playbooks/install_contrail.yml", defaultInventory, defaultConfigFile, k8sOrch,
		),
	}
}
...
func runVcenterClusterTest(t *testing.T, expectedInstance, expectedVcenterVars string,
	...
	executor.AssertAndClear(createVCenterPlaybookExecutions())
	// Wait for the in-memory endpoint cache to get updated
	server.ForceProxyUpdate()
	...
	executor.AssertAndClear(updateVCenterPlaybookExecutions())
	// Wait for the in-memory endpoint cache to get updated
	server.ForceProxyUpdate()
	// make sure all endpoints are recreated as part of update
	err = verifyEndpoints(t, ts, expectedEndpoints)
...
func createVCenterPlaybookExecutions() []ansiblemock.ContainerExecution {
	return []ansiblemock.ContainerExecution {
		defaultPlayBookExecution(
			"playbooks/install_contrail.yml", defaultInventory, defaultConfigFile, k8sOrch,
		),
...
func updateVCenterPlaybookExecutions() []ansiblemock.ContainerExecution {
	return []ansiblemock.ContainerExecution {
		defaultPlayBookExecution(
			"playbooks/install_contrail.yml", defaultInventory, defaultConfigFile, k8sOrch,
		),
