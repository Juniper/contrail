...
func runClusterTest(
	...
	err := integration.LoadTestScenario(&ts, allInOneClusterTemplatePath, pContext)
...
func runAppformixClusterTest(
	...
	// Create the cluster and related objects
	var ts integration.TestScenario
	err := integration.LoadTestScenario(&ts, allInOneClusterAppformixTemplatePath, pContext)
	assert.NoError(t, err, "failed to load cluster test data")
	cleanup := integration.RunDirtyTestScenario(t, &ts, server)
	defer cleanup()
	// create cluster config
...
func TestXflow(t *testing.T) {
	...
	contrailDeployer, ok := deployer.(*contrailAnsibleDeployer)
...
func runKubernetesClusterTest(
	...
	// Create the cluster and related objects
	var ts integration.TestScenario
	err := integration.LoadTestScenario(&ts, allInOneKubernetesClusterTemplatePath, pContext)
	assert.NoError(t, err, "failed to load cluster test data")
	cleanup := integration.RunDirtyTestScenario(t, &ts, server)
	defer cleanup()
	// create cluster config
...
func runvcenterClusterTest(
	...
	// Create the cluster and related objects
	var ts integration.TestScenario
	err := integration.LoadTestScenario(&ts, allInOnevcenterClusterTemplatePath, pContext)
	assert.NoError(t, err, "failed to load cluster test data")
	cleanup := integration.RunDirtyTestScenario(t, &ts, server)
	defer cleanup()
	// create cluster config
...
func runMCClusterTest(t *testing.T, pContext map[string]interface{}) {
	...
	// Create the cluster and related objects
	var ts integration.TestScenario
	err := integration.LoadTestScenario(&ts, allInOneMCClusterTemplatePath, pContext)
