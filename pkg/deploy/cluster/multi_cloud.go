...
func (m *multiCloudProvisioner) unsetMulticloudProvisioningFlag() error {
	...
	r, err := m.cluster.APIServer.GetCloud(context.Background(), &services.GetCloudRequest{
		ID: pubCloudID,
	})
	if err != nil {
		return errors.Wrap(err, "failed to get Cloud resource")
	}

	c := r.GetCloud()
	if c == nil {
		return errors.Wrap(err, "retrieved Cloud resource is nil")
	}

	c.IsMulticloudProvisioning = false
	m.Log.Debug("Successfully unset cloud.is_multicloud_provisioning flag")
	return nil
}
