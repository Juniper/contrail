...
func (m *multiCloudProvisioner) manageSSHAgent(workDir string, action string) error {
	...
	m.ensureNoKeyInSSHAgent(sshPvtKeyPath)
	return m.addKeyToSSHAgent(sshPvtKeyPath)
}

// func (m *multiCloudProvisioner) addPvtKeyToSSHAgent(keyPath string) error {
// 	ensureNoKeyInSSHAgent(keyPath)
...
func (m *multiCloudProvisioner) ensureNoKeyInSSHAgent(keyPath string) {
	_ = osutil.ExecCmdAndWait(m.Reporter, "ssh-add", []string{"-d", keyPath}, m.workDir)
}

func (m *multiCloudProvisioner) addKeyToSSHAgent(keyPath string) error {
	return osutil.ExecCmdAndWait(m.Reporter, "ssh-add", []string{keyPath}, m.workDir)
}
