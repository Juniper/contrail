...
func (m *multiCloudProvisioner) getProviders() ([]string, error) {
	pubCloudID, _, err := m.getPubPvtCloudID()
	if err != nil {
	...
	return getProviderNames(cloudObj.GetCloudProviders()), nil
}

func getProviderNames(providers []*models.CloudProvider) []string {
	result := []string{}
...
func (m *multiCloudProvisioner) createClusterSecretFile() error {
	...
	pubCloudProviders, err := m.getProviders()
	if err != nil {
