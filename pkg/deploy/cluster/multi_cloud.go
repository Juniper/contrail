...
func (m *multiCloudProvisioner) setNodeProvisionFlag(ctx context.Context) error {
	...
	for _, tag := range tags {
		if tag.NodeBackRefs != nil {
			for _, node := range tag.NodeBackRefs {
				setNodeAnnotation(node, "provision", false)
...
func setNodeAnnotation(node *models.Node, key string, val bool) {
	...
	if !provisionKeyFound {
		inst.Annotations.KeyValuePair = append(inst.Annotations.KeyValuePair,
			&KeyValuePair{
				Key:   key,
