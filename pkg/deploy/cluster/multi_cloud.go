...
func (m *multiCloudProvisioner) multiCloudCLIRunAll() error {
	...
	if m.contrailAnsibleDeployer.ansibleClient.IsTest() {
		return m.mockCLI(deployerCommand)
	}

	result, err := exec.Command("bash", "-c", deployerCommand).CombinedOutput()
	if err != nil {
...
func (m *multiCloudProvisioner) multiCloudCLICleanup() error {
	...
	deployerCommand := fmt.Sprintf("deployer all clean --inventory %v --retry %v", invPath, 5)
