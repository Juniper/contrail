...
func createIncorrectKey(t *testing.T, destination string) func() {
	assert.NoError(t, fileutil.WriteToFile(destination, []byte("Hi, I am a Private Key. Trust Me."), keyPermissions),
...
func createPrivateKey(t *testing.T, destination string) func() {
	...
	assert.NoError(t, fileutil.WriteToFile(destination, pemdata, keyPermissions), "couldn't create key file")
	return func() {
		assert.NoError(t, os.Remove(destination), "couldn't remove created key file")
	}
}
