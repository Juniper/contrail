...
func writeRDBMS(events *services.EventList) (err error) {
	e, err := extractRefUpdateEvents(events)
	if err != nil {
		return errors.Wrap(err, "failed to extract ref events")
	}
	events = &services.EventList{e}
...
func extractRefUpdateEvents(e *services.EventList) (result []*services.Event, err error) {
	var refUpdateEvents []*services.Event
	for id := range e.Events {
		newEvent, err := e.Events[id].ExtractRefsEventFromEvent()
		if err != nil {
...
func writeHTTP(events *services.EventList, url string) (err error) {
	...
	return errors.Wrap(err, "some events were not processed, last error")
}
