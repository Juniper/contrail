...
func Convert(c *Config) error {
	events, err := readData(c.InType, c.InFile, c.CassandraPort, c.CassandraTimeout)
	if err != nil {
	...
	err = writeData(events, c.OutType, c.OutFile, c.EtcdNotifierPath)
	if err != nil {
...
func readData(inType, inFile string, cassandraPort, cassandraTimeout int) (*services.EventList, error) {
	...
	case CassandraType:
		events, err = cassandra.DumpCassandra(
			inFile,
			cassandraPort,
			time.Duration(cassandraTimeout)*time.Second,
		)
	case CassandraDumpType:
		events, err = cassandra.ReadCassandraDump(inFile)
	case YAMLType:
		events, err = readYAML(inFile)
	case RDBMSType:
		events, err = readRDBMS()
	default:
		return nil, errors.Errorf("unsupported input type %v", inType)
	}

	return events, err
}

func writeData(events *services.EventList, outType, outFile, etcdNotifierPath string) error {
	var err error
	switch outType {
	case RDBMSType:
		err = writeRDBMS(events)
	case YAMLType:
		err = writeYAML(events, outFile)
	case EtcdType:
		err = writeEtcd(events, etcdNotifierPath)
	default:
		return errors.Errorf("unsupported output type %v", outType)
	}
