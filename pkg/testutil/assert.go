...
var assertFunctions = map[string]assertFunction{
	...
	"null": func(path string, args, actual interface{}) error {
		if actual != nil {
			return errors.Errorf("expecetd null but got %s on path %s", actual, path)
		}
		return nil
	},
	"number": func(path string, args, actual interface{}) error {
		...
		return errors.Errorf("expecetd integer but got %s on path %s", actual, path)
	},
...
func checkDiff(path string, expected, actual interface{}) error {
	...
	case []interface{}:
		...
		for i, value := range t {
			...
			if !found {
				return fmt.Errorf("%s not found, last err: %v", path+"."+strconv.Itoa(i), err)
			}
		}
	case int:
