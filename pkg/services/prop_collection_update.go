...
func (p *ContrailEndpointPlugin) RESTPropCollectionUpdate(c echo.Context) error {
	...
	if err := p.InTransactionDoer.DoInTransaction(c.Request().Context(), func(ctx context.Context) error {
		...
		return p.updatePropCollection(ctx, &r, obj, objType)
	}); err != nil {
		return errutil.ToHTTPError(err)
	}
...
func (p *ContrailEndpointPlugin) PropCollectionUpdate(
	...
	err := p.InTransactionDoer.DoInTransaction(ctx, func(ctx context.Context) error {
		...
		return p.updatePropCollection(ctx, request, obj, objType)
	})
	return &types.Empty{}, err
}

func (p *ContrailEndpointPlugin) updatePropCollection(
	ctx context.Context,
