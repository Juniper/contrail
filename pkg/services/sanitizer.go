...
func (sv *SanitizerService) getIncompleteRefs(
	refs []basemodels.Reference) (map[string] basemodels.Reference,
) {
...
func (sv *SanitizerService) sanitizeRefs(
	...
	for _, ref := range incompleteRefsMap {
		metadatas = append(metadatas, &basemodels.Metadata{FQName: ref.GetTo(), UUID: ref.GetUUID(),  Type: ref.GetReferredKind()})
	}
