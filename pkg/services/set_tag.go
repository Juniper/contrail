...
func (service *ContrailService) SetTag(ctx context.Context, setTag SetTagRequest) error {
	...
	for tagType, tagAttr := range setTag.Tags {
		...
		case tagAttr.isDeleteRequest():
			references = removeTagsOfType(references, tagType)
		case isTagTypeUniquePerObject(tagType):
			...
			references = append(references, basemodels.NewReference(uuid, models.KindTag))
		case len(tagAttr.AddValues) > 0:
		...
		case len(tagAttr.DeleteValues) > 0:
			...
			references = references.Filter(func(r basemodels.Reference) bool {
