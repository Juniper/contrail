...
func (t *SetTagRequest) validate() error {
	...
	for tagType, tagAttrs := range t.Tags {
		...
		if isTagTypeUniquePerObject(tagType) {
			...
			if _, ok := tagAttrs.Value.(string); !ok && !tagAttrs.isDeleteRequest() {
...
func (service *ContrailService) SetTag(ctx context.Context, setTag SetTagRequest) error {
	...
	for tagType, tagAttrs := range setTag.Tags {
		...
		case len(tagAttrs.DeleteValues) > 0:
