...
func (service *ContrailService) RESTGetIntOwner(c echo.Context) error {
	...
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, fmt.Sprintf("invalid request: invalid value (%v) for getting int owner: %v", aValue, err))
	}
	if value < 0 {
		return echo.NewHTTPError(http.StatusBadRequest, fmt.Sprintf("invalid request: invalid value (%v) for getting int owner", value))
	}
