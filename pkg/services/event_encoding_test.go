...
func Test_eventsToEventNodes(t *testing.T) {
	...
	assert.True(t, reflect.DeepEqual(events[0], eventNodes[0].event))
	assert.True(t, reflect.DeepEqual(events[1], eventNodes[1].event))
	assert.True(t, reflect.DeepEqual(events[2], eventNodes[2].event))
...
func containsRefsTo(original *eventNode, referencedNodes []*eventNode) bool {
	refs := make(map[*eventNode]bool)
	for _, nodeRef := range original.refs {
		refs[nodeRef] = true
	}
