...
func TestApplyHref(t *testing.T) {
	runTest(t, func(t *testing.T, client *integration.HTTPAPIClient, server *integration.APIServer) {
		...
		tests := []struct {
			...
			{
				name:     "resource href",
				expected: server.URL() + "/" + vn.Kind() + "/" + vn.GetUUID(),
				actual:   virtualNetwork.Href,
			},
			{
				name:     "network ipam reference href",
				expected: server.URL() + "/" + ipamRef.GetReferredKind() + "/" + ipamRef.GetUUID(),
				actual:   ipamRef.Href,
			},
			{
				name:     "project child vn href",
				expected: server.URL() + "/" + childNetwork.Kind() + "/" + childNetwork.GetUUID(),
				actual:   childNetwork.Href,
			},
			{
				name:     "network ipam back-referenced vn href",
				expected: server.URL() + "/" + backReferencedVirtualNetwork.Kind() + "/" + backReferencedVirtualNetwork.GetUUID(),
