...
func (t *SetTag) validate() error {
	if t.ObjUUID == "" || t.ObjType == "" {
		return common.ErrorBadRequestf(
			"Bad Request: Both obj_uuid and obj_type should be specified: %s, %s",
			t.ObjUUID, t.ObjType,
		)
	}
	...
	if !t.App.validate() {
		return common.ErrorBadRequestf(
			"Bad Request: set-tag requires specified application value: %s %s",
			t.ObjUUID, t.ObjType,
		)
	}
	return nil
...
func (t *SetTag) checkIfObjTypeFits() bool {
	if t.ObjType == "project" {
		return true
	}
	if t.ObjType == "virtual-network" {
		return true
	}
	if t.ObjType == "virtual-machine" {
		return true
	}
	if t.ObjType == "virtual-machine-interface" {
		return true
	}
	if t.ObjType == "application-policy-set" {
		return true
	}
	return false
}
