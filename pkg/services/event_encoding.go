...
func fillEventNodesReferences(
	...
	for id, node := range eventNodes {
		...
		for _, ref := range refs {
			...
			if refNode != nil {
				eventNodes[id].referencesAndChildren = append(eventNodes[id].referencesAndChildren, refNode)
			}
		}
	}
...
func (e *Event) getRefs() ([]*ref, error) {
	...
	parentRef := basemodels.NewReference(res.GetParentUUID(), basemodels.ParentFQName(res.GetFQName()))
