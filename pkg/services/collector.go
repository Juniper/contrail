...
const (
	...
	defaultCollectorMaxMessageCount = 16
)
...
type collectorRestAPITrace struct {
...
func NewCollector(url string, maxMessageCount int) *Collector {
	if url == "" {
		logrus.Info("collector.url not specified in configuration file")
	}
	if maxMessageCount == 0 {
		maxMessageCount = defaultCollectorMaxMessageCount
	}
	return &Collector{
		url:             url,
...
func (collector *Collector) sendMessage(data []byte) {
	...
	if collector.messageCount > collector.maxMessageCount {
