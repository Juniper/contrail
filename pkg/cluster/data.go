...
func (k *KubernetesData) interfaceToKubernetesNode(kubernetesNodes interface{}, c *Cluster) error {
	for _, kubernetesNode := range kubernetesNodes.([]interface{}) {
		...
		k.clusterInfo.KubernetesNodes = append(k.clusterInfo.KubernetesNodes, kubernetesNodeInfo)
	}
	return nil
}
...
func (k *KubernetesData) interfaceToKubernetesMasterNode(kubernetesMasterNodes interface{}, c *Cluster) error {
	for _, kubernetesMasterNode := range kubernetesMasterNodes.([]interface{}) {
		...
		k.clusterInfo.KubernetesMasterNodes = append(k.clusterInfo.KubernetesMasterNodes, kubernetesMasterNodeInfo)
	}
	return nil
}
...
func (o *OpenstackData) interfaceToOpenstackControlNode(openstackControlNodes interface{}, c *Cluster) error {
	for _, openstackControlNode := range openstackControlNodes.([]interface{}) {
		...
		o.clusterInfo.OpenstackControlNodes = append(o.clusterInfo.OpenstackControlNodes, openstackControlNodeInfo)
	}
	return nil
}
...
func (o *OpenstackData) interfaceToOpenstackMonitoringNode(openstackMonitoringNodes interface{}, c *Cluster) error {
	for _, openstackMonitoringNode := range openstackMonitoringNodes.([]interface{}) {
		openstackMonitoringNodeInfo := models.InterfaceToOpenstackMonitoringNode(openstackMonitoringNode.(map[string]interface{}))
		// Read openstackMonitoring role node to get the node refs information
		openstackMonitoringNodeData, err := c.getResource(defaultOpenstackMonitoringNodeResPath, openstackMonitoringNodeInfo.UUID)
		if err != nil {
			return err
		}
		openstackMonitoringNodeInfo = models.InterfaceToOpenstackMonitoringNode(openstackMonitoringNodeData)
		o.clusterInfo.OpenstackMonitoringNodes = append(o.clusterInfo.OpenstackMonitoringNodes, openstackMonitoringNodeInfo)
	}
	return nil
}
...
func (o *OpenstackData) interfaceToOpenstackNetworkNode(openstackNetworkNodes interface{}, c *Cluster) error {
	for _, openstackNetworkNode := range openstackNetworkNodes.([]interface{}) {
		...
		o.clusterInfo.OpenstackNetworkNodes = append(o.clusterInfo.OpenstackNetworkNodes, openstackNetworkNodeInfo)
	}
	return nil
}
...
func (o *OpenstackData) interfaceToOpenstackStorageNode(openstackStorageNodes interface{}, c *Cluster) error {
	for _, openstackStorageNode := range openstackStorageNodes.([]interface{}) {
		openstackStorageNodeInfo := models.InterfaceToOpenstackStorageNode(openstackStorageNode.(map[string]interface{}))
		// Read openstackStorage role node to get the node refs information
		openstackStorageNodeData, err := c.getResource(defaultOpenstackStorageNodeResPath, openstackStorageNodeInfo.UUID)
		if err != nil {
			return err
		}
		openstackStorageNodeInfo = models.InterfaceToOpenstackStorageNode(openstackStorageNodeData)
		o.clusterInfo.OpenstackStorageNodes = append(o.clusterInfo.OpenstackStorageNodes, openstackStorageNodeInfo)
	}
	return nil
}
...
func (o *OpenstackData) interfaceToOpenstackComputeNode(openstackComputeNodes interface{}, c *Cluster) error {
	for _, openstackComputeNode := range openstackComputeNodes.([]interface{}) {
		openstackComputeNodeInfo := models.InterfaceToOpenstackComputeNode(openstackComputeNode.(map[string]interface{}))
		// Read openstackCompute role node to get the node refs information
		openstackComputeNodeData, err := c.getResource(defaultOpenstackComputeNodeResPath, openstackComputeNodeInfo.UUID)
		if err != nil {
			return err
		}
		openstackComputeNodeInfo = models.InterfaceToOpenstackComputeNode(openstackComputeNodeData)
		o.clusterInfo.OpenstackComputeNodes = append(o.clusterInfo.OpenstackComputeNodes, openstackComputeNodeInfo)
	}
	return nil
}
...
func (d *Data) interfaceToContrailVrouterNode(contrailVrouterNodes interface{}, c *Cluster) error {
	for _, contrailVrouterNode := range contrailVrouterNodes.([]interface{}) {
		contrailVrouterNodeInfo := models.InterfaceToContrailVrouterNode(contrailVrouterNode.(map[string]interface{}))
		// Read contrailVrouter role node to get the node refs information
		contrailVrouterNodeData, err := c.getResource(defaultContrailVrouterNodeResPath, contrailVrouterNodeInfo.UUID)
		if err != nil {
			return err
		}
		contrailVrouterNodeInfo = models.InterfaceToContrailVrouterNode(contrailVrouterNodeData)
		d.clusterInfo.ContrailVrouterNodes = append(d.clusterInfo.ContrailVrouterNodes, contrailVrouterNodeInfo)
	}
	return nil
}
...
func (d *Data) interfaceToContrailServiceNode(contrailServiceNodes interface{}, c *Cluster) error {
	for _, contrailServiceNode := range contrailServiceNodes.([]interface{}) {
		contrailServiceNodeInfo := models.InterfaceToContrailServiceNode(contrailServiceNode.(map[string]interface{}))
		// Read contrailService role node to get the node refs information
		contrailServiceNodeData, err := c.getResource(defaultContrailServiceNodeResPath, contrailServiceNodeInfo.UUID)
		if err != nil {
			return err
		}
		contrailServiceNodeInfo = models.InterfaceToContrailServiceNode(contrailServiceNodeData)
		d.clusterInfo.ContrailServiceNodes = append(d.clusterInfo.ContrailServiceNodes, contrailServiceNodeInfo)
	}
	return nil
}
...
func (d *Data) interfaceToContrailControlNode(contrailControlNodes interface{}, c *Cluster) error {
	for _, contrailControlNode := range contrailControlNodes.([]interface{}) {
		contrailControlNodeInfo := models.InterfaceToContrailControlNode(contrailControlNode.(map[string]interface{}))
		// Read contrailControl role node to get the node refs information
		contrailControlNodeData, err := c.getResource(defaultContrailControlNodeResPath, contrailControlNodeInfo.UUID)
		if err != nil {
			return err
		}
		contrailControlNodeInfo = models.InterfaceToContrailControlNode(
			contrailControlNodeData)
		d.clusterInfo.ContrailControlNodes = append(d.clusterInfo.ContrailControlNodes, contrailControlNodeInfo)
	}
	return nil
}
...
func (d *Data) interfaceToContrailConfigDatabaseNode(contrailConfigDatabaseNodes interface{}, c *Cluster) error {
	for _, contrailConfigDatabaseNode := range contrailConfigDatabaseNodes.([]interface{}) {
		contrailConfigDatabaseNodeInfo := models.InterfaceToContrailConfigDatabaseNode(contrailConfigDatabaseNode.(map[string]interface{}))
		// Read contrailConfigDatabase role node to get the node refs information
		contrailConfigDatabaseNodeData, err := c.getResource(defaultContrailConfigDatabaseNodeResPath, contrailConfigDatabaseNodeInfo.UUID)
		if err != nil {
			return err
		}
		contrailConfigDatabaseNodeInfo = models.InterfaceToContrailConfigDatabaseNode(
			contrailConfigDatabaseNodeData)
		d.clusterInfo.ContrailConfigDatabaseNodes = append(d.clusterInfo.ContrailConfigDatabaseNodes, contrailConfigDatabaseNodeInfo)
	}
	return nil
}
