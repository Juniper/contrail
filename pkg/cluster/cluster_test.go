...
const (
	...
	allInOneClusterWithAppformixTemplatePath = "./test_data/test_all_in_one_with_appformix.tmpl"
...
func runAppformixClusterTest(t *testing.T, expectedInstance, expectedInventory string,
	...
	// Create the cluster and related objects
	var testScenario apisrv.TestScenario
	err := apisrv.LoadTestScenario(&testScenario, allInOneClusterWithAppformixTemplatePath, context)
	assert.NoError(t, err, "failed to load cluster test data")
	cleanup := apisrv.RunDirtyTestScenario(t, &testScenario)
	defer cleanup()
	// create cluster config
	...
	// remove instances.yml to triger cluster update
	err = os.Remove(generatedInstancesPath())
	if err != nil {
		assert.NoError(t, err, "failed to delete instances.yml")
	}
...
func runAllInOneWithAppformixTest(t *testing.T, computeType string) {
	context := pongo2.Context{
		"TYPE":            computeType,
		"MGMT_INT_IP":     "127.0.0.1",
		"CONTROL_NODES":   "",
...
func TestAllInOneWitAppformix(t *testing.T) {
	runAllInOneWithAppformixTest(t, "kernel")
}
