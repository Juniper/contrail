...
func (m *multiCloudProvisioner) createClusterTopologyFile(workDir string) error {
	...
	for _, cloudRef := range m.clusterData.cloudInfo {
		for _, p := range cloudRef.CloudProviders {
			if p.Type == "private" {
				cloudInfo["private"] = cloudRef.UUID
				break
			}
	...
	if _, err := os.Stat(onPremTopoFile); err == nil {
		...
		err = fileutil.AppendToFile(m.getClusterTopoFile(workDir), onPremTopoContent, defaultFilePermRWOnly)
		if err != nil {
			return err
		}
	} else {
...
func (m *multiCloudProvisioner) createClusterSecretFile(workDir string) error {
	...
	if _, err := os.Stat(secretFile); err == nil {
		...
		err = fileutil.AppendToFile(m.getClusterSecretFile(workDir), secretFileContent, defaultFilePermRWOnly)
		if err != nil {
			return err
		}
	} else {
