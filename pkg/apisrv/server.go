...
func (s *Server) setupHomepage() {
	discoHandler := discovery.NewHandler(viper.GetString("server.address"))

	services.DiscoveryRegisterResourcesSingular(func(path string, name string) {
		discoHandler.Register(path, "", name, "resource-base")
	})
	services.DiscoveryRegisterResourcesPlural(func(path string, name string) {
		discoHandler.Register(path, "", name, "collection")
	})
	...
	s.Echo.GET("/", discoHandler.Handle)
}
