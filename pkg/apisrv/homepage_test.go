...
func TestRoutesAreRegistered(t *testing.T) {
	...
	err = services.ContrailService{}.RegisterHTTPAPI(&routes)
	require.NoError(t, err, "failed to add Contrail HTTP API to expected routes")
...
func (r *routeSet) Group(prefix string, _ ...baseapisrv.MiddlewareFunc) {
	r.add(prefix)
}
