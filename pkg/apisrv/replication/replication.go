...
func (r *Replicator) convertNodeToEndSystem(data interface{}) interface{} {
	var node *models.Node
	createRequest, ok := data.(*services.CreateNodeRequest)
	if ok {
		node = createRequest.Node
	} else {
		var updateRequest *services.UpdateNodeRequest
		updateRequest, ok = data.(*services.UpdateNodeRequest)
		if ok {
			node = updateRequest.Node
		}
	}
	if ok {
		var endSystem map[string]interface{}
		e, _ := json.Marshal(node)        // nolint: errcheck
		_ = json.Unmarshal(e, &endSystem) //nolint: errcheck
...
func (r *Replicator) replicateToVNCAPI(action, url string,
	...
	endSystem := r.convertNodeToEndSystem(data)
	if endSystem != nil {
		data = endSystem
