...
const (
	...
	createTagURL            = "/tag"
	createVirtualNetworkURL = "/virtual-network"
	createNetworkIpamURL    = "/network-ipam"
	updatePortURL           = "/port/"
...
func (r *Replicator) getAuthContext(clusterID string, apiClient *client.HTTP) context.Context {
	...
	if apiClient.Scope.GetDomain() == nil {
		apiClient.Init()
	...
	apiClient.Scope = keystone.GetScope(domainID, domainName,
...
func (r *Replicator) createAPIClient(ep *models.Endpoint) {
	...
	apiClient := &client.HTTP{
		Endpoint: endpoint,
		InSecure: inSecure,
		Scope: keystone.GetScope(domainID, domainName,
			projectID, projectName),
	}
