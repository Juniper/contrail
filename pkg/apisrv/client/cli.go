...
func (c *CLI) ListResources(schemaID string, lp *ListParameters) (string, error) {
	...
	if lp.Count {
		...
		return makeEventListDetailedResponse(schemaID, response)
	}
	return makeEventListStandardResponse(schemaID, response)
}
...
// makeEventListDetailedResponse creates response in format compatible with Sync command input.
func makeEventListDetailedResponse(schemaID string, response map[string]interface{}) (string, error) {
...
// makeEventListStandardResponse creates response in format compatible with Sync command input.
func makeEventListStandardResponse(schemaID string, response map[string]interface{}) (string, error) {
	var el services.EventList
	for _, rawList := range response {
		list, ok := rawList.([]interface{})
		if !ok {
