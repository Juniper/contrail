...
func (h *HTTP) Do(ctx context.Context, method, path string, data interface{}, output interface{}, expected []int) (*http.Response, error) {
	request, err := h.prepareHTTPRequest(ctx, method, path, data)
	if err != nil {
		return nil, err
	}
...
func (h *HTTP) prepareHTTPRequest(ctx context.Context, method, path string, data interface{}) (*http.Request, error) {
...
func (h *HTTP) doHTTPRequestRetryingOn401(ctx context.Context, request *http.Request, data interface{}) (*http.Response, error) {
	if h.Debug {
		log.WithFields(log.Fields{
