...
func (h *HTTP) AllocateInt(ctx context.Context, pool, owner string) (int64, error) {
	...
	_, err := h.Do(ctx, echo.POST, "/"+services.IntPoolPath, nil, &services.IntPoolAllocationBody{Pool: pool, Owner: owner}, &output, []int{http.StatusOK})
...
func (h *HTTP) SetInt(ctx context.Context, pool string, value int64, owner string) error {
	_, err := h.Do(
		...
		nil,
		&services.IntPoolAllocationBody{Pool: pool, Value: &value, Owner: owner},
		&struct{}{},
		[]int{http.StatusOK},
	)
...
func (h *HTTP) DeallocateInt(ctx context.Context, pool string, value int64) error {
	_, err := h.Do(
		...
		"/"+services.IntPoolPath,
		nil,
		&services.IntPoolAllocationBody{Pool: pool, Value: &value},
		&struct{}{},
		[]int{http.StatusOK},
	)
