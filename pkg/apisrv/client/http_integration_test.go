...
func TestRemoteIntPoolMethods(t *testing.T) {
	...
	owner, err := hc.GetIntPoolOwner(context.Background(), "test_int_pool_806f099f3", val)
	assert.NoError(t, err)
	assert.Equal(t, "test_owner_806f099f3", owner)

	err = hc.SetInt(context.Background(), "test_int_pool_806f099f3", val+1, db.IntPoolEmptyOwner)
	defer func() {
