...
func TestCreateRefMethod(t *testing.T) {
	...
	_, err := hc.DeleteVirtualNetworkNetworkIpamRef(context.Background(), deleteVnNiRefReq(vnUUID, niUUID))
	assert.Equal(t, nil, err)

	vn = hc.GetVirtualNetwork(t, vnUUID)
	assert.Equal(t, 0, len(vn.NetworkIpamRefs))

	_, err = hc.CreateVirtualNetworkNetworkIpamRef(context.Background(), createVnNiRefReq(vnUUID, niUUID))
	assert.Equal(t, nil, err)
...
func createVnNiRefReq(vnUUID, niUUID string) *services.CreateVirtualNetworkNetworkIpamRefRequest {
	return &services.CreateVirtualNetworkNetworkIpamRefRequest{
		ID: vnUUID,
...
func deleteVnNiRefReq(vnUUID, niUUID string) *services.DeleteVirtualNetworkNetworkIpamRefRequest {
	return &services.DeleteVirtualNetworkNetworkIpamRefRequest{
		ID: vnUUID,
		VirtualNetworkNetworkIpamRef: &models.VirtualNetworkNetworkIpamRef{
			UUID: niUUID,
		},
