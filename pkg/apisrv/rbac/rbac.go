...
type request struct {
	...
	objectType string                    // Objtype of the resource
...
func newRequest(ctx context.Context, apiAccessRules []*models.APIAccessList,
	objectType string, op constants.OpCRUD) *request {
	...
	//Add relevant RBAC rules for this object type to request.hash
...
func (rq *request) getHashKey(objectType string, _ string) (key string) {
...
func (rq *request) updateRuleToHash(role string, crud string) {
	...
	key := rq.getHashKey(rq.objectType, rq.fieldName)
...
func (rq *request) objectTypeRuleAddToHash(rbacRule *models.RbacRuleType) {
	...
	if ruleObjType != rq.objectType && ruleObjType != "*" {
		return
	}
...
func (rq *request) validateResourceAPILevel(objectType string) bool {

	key := rq.getHashKey(objectType, "_")
	if val, ok := rq.hash[key]; ok {
...
func (rq *request) validateAPILevel() bool {
	...
	if rq.validateResourceAPILevel(rq.objectType) {
		return true
	}
...
func crudToOpMap(crud rune) constants.OpCRUD {
...
func isROAccessAllowed(ctx *auth.Context, op constants.OpCRUD) bool {
...
func ValidateRequest(ctx context.Context, apiAccessRules []*models.APIAccessList,
	objectType string, op constants.OpCRUD) (bool, error) {
	...
	rbacReq := newRequest(ctx, apiAccessRules, objectType, op)
