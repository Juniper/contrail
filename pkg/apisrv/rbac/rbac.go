...
// addAPIAccessRulesToHash adds rules matching (resource )objectKind and wild card rules from the rule list to
...
// updateRuleToHash adds rules to the resourcePermission hash table. resourcePermission hash table will have
...
// objectKindPermsAddToHash adds objectKind or wildcard RBAC rules to hash.
// Input "perm" is in the following form
// Member:CRUD
func (r resourcePermission) objectKindPermAddToHash(ruleKind string, perm *models.RbacPermType) error {
...
func (r resourcePermission) objectKindPermsAddToHash(ruleKind string, perms []*models.RbacPermType) error {

	for _, perm := range perms {

		err := r.objectKindPermAddToHash(ruleKind, perm)
		if err != nil {
			return err
		}
	}
...
// objectKindRuleAddToHash add objectKind or wildcard RBAC rules to hash.
// Input rbacRule is in the the following form
// < virtual-network, *> => Member:CRUD ,Development:CRUD.
func (r resourcePermission) objectKindRuleAddToHash(rbacRule *models.RbacRuleType) error {
	...
	err := r.objectKindPermsAddToHash(ruleKind, perms)
	if err != nil {
		return err
	}
	return nil
...
// rulesAddToHash adds  matching the objectKind or wildcard RBAC rules to hash.
...
func (r resourcePermission) rulesAddToHash(rbacRules []*models.RbacRuleType) error {
	...
	for _, obj := range rbacRules {
		...
		err := r.objectKindRuleAddToHash(obj)
		if err != nil {
			return err
		}
