...
func ValidateRequest(ctx context.Context, l []*models.APIAccessList,
	kind string, op constants.OperationCRUD) error {

	if ctx == nil {

		log.Debug("Invalid Context. Access Denied ")
	...
	if authCtx == nil {
		...
		log.Debug("Invalid Auth Context. Access Denied ")
		return errutil.ErrorPermissionDenied
	}
	...
	if len(l) == 0 {
		...
		log.Debug("No API Access List rules present.Access Denied ")
		return errutil.ErrorPermissionDenied
	}
	// Create RBAC request object based on request parameters and rbac rule configuration
	rbacReq := newRequest(ctx, l, kind, op)
	...
	log.Debug("RBAC : No Matching  API Access List rules. Access Denied ")
	return errutil.ErrorPermissionDenied
