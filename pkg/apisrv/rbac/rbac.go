...
// resourcePermission is the 2st level hash table to get kind specific rules.
...
func CheckPermissions(ctx context.Context, l []*models.APIAccessList, aaaMode string, kind string, op Action) error {
	...
	if validateRoleAndConfig(authCtx, aaaMode) {
		return nil
	}
...
func (t tenantPermission) addTenantLevelPolicy(
	...
	for k, list := range project {
		...
		entry, err  := t.addAPIAccessRulesToHash(list)
		if err != nil {
		  return err
		}
		t[k] = entry
...
func (t tenantPermission) addGlobalLevelPolicy(global []*models.APIAccessList) error {
	...
	key := getTenantKey("*", "*")
	entry, err := t.addAPIAccessRulesToHash(global)
	if err != nil {
	  return err
	}
	t[key] = entry
...
func (t tenantPermission) accessListAdd(lists []*models.APIAccessList) error {
...
// addAPIAccessRulesToHash adds rules matching kind and wild card rules from the rule list to
...
func (t tenantPermission) addAPIAccessRulesToHash(apiAccessRules []*models.APIAccessList) (resourcePermission,error ) {

	res := make(resourcePermission)
	for _, apiRule := range apiAccessRules {

		rbacRules := getRBACRules(apiRule)
		err := res.rulesAddToHash(rbacRules)
		if err != nil {
		   return nil,err
		}
	}
...
// getProjectHashTable retrieves matching first level hash table entry .
func (t tenantPermission) getProjectHashTable(rq *request) resourcePermission {
...
// validateAPILevel checks whether any obj type rules or wildcard allow this operation
...
// crudToActionMap converts crud string to op slice.
func crudToActionMap(crud string) ([]Action, error) {
...
// getResourceHashKey forms a hash key from objName,objField.
...
// updateRuleToHash adds rules to the resourcePermission hash table. resourcePermission hash table will have entries of the form
// key => {{role1:actionSet1},{role2:actionSet2}} . This will merge the object_type CRUD in global,domain and
// project level.
func (r resourcePermission) updateRuleToHash(ruleKind string, role string, crud []Action) {
...
// kindPermsAddToHash adds kind or wildcard RBAC rules to hash.
// Input "perm" is in the following form
// Member:CRUD
func (r resourcePermission) kindPermAddToHash(ruleKind string, perm *models.RbacPermType) error {

	if perm == nil {
		return nil
	}
	role := perm.GetRoleName()
	crud,err := crudToActionMap(perm.GetRoleCrud())
	if err != nil {
	   return err
	}
...
func (r resourcePermission) kindPermsAddToHash(ruleKind string, perms []*models.RbacPermType) error {

	for _, perm := range perms {

		err := r.kindPermAddToHash(ruleKind, perm)
		if err != nil{
		  return err
		}
	}
...
// kindRuleAddToHash add kind or wildcard RBAC rules to hash.
// Input rbacRule is in the the following form
// < virtual-network, *> => Member:CRUD ,Development:CRUD.
func (r resourcePermission) kindRuleAddToHash(rbacRule *models.RbacRuleType) error {
	...
	err := r.kindPermsAddToHash(ruleKind, perms)
	if err != nil{
	    return err
	}
	return nil
...
// rulesAddToHash adds  matching the kind or wildcard RBAC rules to hash.
// Input is in the  following form
// < virtual-network, *> => Member:CRUD ,Development:CRUD
// < virtual-ip, *> => Member:CRUD
func (r resourcePermission) rulesAddToHash(rbacRules []*models.RbacRuleType) error {

	for _, obj := range rbacRules {

		err := r.kindRuleAddToHash(obj)
		if err != nil{
  	           return err
		}
...
// validateRoleAndConfig checks whether is Resource access is allowed purely based on  RBAC config.
// or user roles.
func validateRoleAndConfig(ctx *auth.Context, aaaMode string) bool {
