...
// request structure.
type request struct {
	rolePermissions map[resourceHashKey]roleToCRUDMap // Hash table maps resource hash key to role ToCRUDMap.
...
func (s *crudSet) setCRUD(crud []constants.OperationCRUD) {
...
func (m *roleToCRUDMap) updateEntry(role string, crud []constants.OperationCRUD) {
...
func newRequest(
	...
	n := request{
		...
		rolePermissions: make(map[resourceHashKey]roleToCRUDMap),
	}
...
// project level.
func (rq *request) updateRuleToHash(role string, crud []constants.OperationCRUD) {
...
// 'CRUD' to operation op slice
func crudToOpMap(crud string) []constants.OperationCRUD {
	...
	for _, c := range crud {
		switch c {
		case UpperCaseC:
			res = append(res, constants.OpCreate)
		case UpperCaseR:
			res = append(res, constants.OpRead)
		case UpperCaseU:
			res = append(res, constants.OpUpdate)
		case UpperCaseD:
			res = append(res, constants.OpDelete)
		default:
