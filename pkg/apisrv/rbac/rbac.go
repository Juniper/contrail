...
func (r *rbac) rbacGetHashKey(objName string, _ string, _ string) (key string) {
...
func (r *rbac) rbacUpdateRuleHash(hash map[string]*roleToCrudMap, objName string,
...
func (r *rbac) objTypePermAddToHash(perm *models.RbacPermType, hash map[string]*roleToCrudMap,
...
func (r *rbac) objTypePermsAddToHash(perms []*models.RbacPermType, hash map[string]*roleToCrudMap,
	roles map[string]bool, objType string, objField string) {
...
func (r *rbac) objTypeRuleAddToHash(rbacRule *models.RbacRuleType, hash map[string]*roleToCrudMap,
...
func (r *rbac) rbacRulesAddToHash(rbacRules []*models.RbacRuleType, hash map[string]*roleToCrudMap,
...
func (r *rbac) rbacGetObjTypeRules(apiAccessRules []*models.APIAccessList,
	roles map[string]bool, objType string) map[string]*roleToCrudMap {

	hash := make(map[string]*roleToCrudMap)
...
func (r *rbac) validateKeyAPILevel(hash map[string]*roleToCrudMap, roleSet map[string]bool,
	objType string, op constants.OpCrud) bool {

	key := r.rbacGetHashKey(objType, "_", "-")
	if val, ok := hash[key]; ok {

		return val.isRuleMatching(roleSet, op)
...
func (r *rbac) validateAPILevel(hash map[string]*roleToCrudMap, roleSet map[string]bool,
	objtype string, op constants.OpCrud) bool {

	// check for objtype rule .
	if r.validateKeyAPILevel(hash, roleSet, objtype, op) {
		return true
	}

	// check for wild card rule
	if r.validateKeyAPILevel(hash, roleSet, "*", op) {
		return true
	}
...
func (r *rbac) ValidateRequest(ctx context.Context, apiAccessRules []*models.APIAccessList,
	...
	var hash map[string]*roleToCrudMap
