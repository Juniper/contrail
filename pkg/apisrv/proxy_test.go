...
func TestKeystoneEndpointsProxying(t *testing.T) {
	...
	ksPrivate := integration.NewKeystoneServerFake(t, authURL, chrisUsername, chrisPassword) // remote Keystone
	defer ksPrivate.Close()
	ksPublic := integration.NewKeystoneServerFake(t, authURL, chrisUsername, chrisPassword) // remote Keystone
	defer ksPublic.Close()

	cleanupE := createEndpoint(t, hcBob, endpointParameters{
		clusterName:    clusterName,
		endpointName:   endpointName,
		endpointPrefix: keystoneEndpointPrefix,
		privateURL:     ksPrivate.URL,
		publicURL:      ksPublic.URL,
	...
	server.ForceProxyUpdate()
	// act/assert
	// TODO(dfurman): verify if results below are expected
	// Requests issued with Chris user always fail, which suggests local Keystone is used instead of remote Keystone.
	// Remote Keystone probably needs to be used, when proxy endpoint is created.
	verifyKeystoneCreateTokenRequest(t, hcBob.HTTP, "using remote Keystone, after endpoint create")
	verifyKeystoneCreateTokenRequestFails(t, hcChris, "using remote Keystone, after endpoint create")
	verifyKeystoneReadTokenRequest(t, hcBob.HTTP, "using remote Keystone, after endpoint create")
	verifyKeystoneReadTokenRequestFails(t, hcChris, "using remote Keystone, after endpoint create")

	integration.DeleteEndpoint(t, hcBob, endpointUUID(clusterName, endpointName))
	server.ForceProxyUpdate()

	verifyKeystoneCreateTokenRequest(t, hcBob.HTTP, "using local Keystone, after endpoint delete")
	verifyKeystoneCreateTokenRequestFails(t, hcChris, "using local Keystone, after endpoint delete")
	verifyKeystoneReadTokenRequest(t, hcBob.HTTP, "using local Keystone, after endpoint delete")
	verifyKeystoneReadTokenRequestFails(t, hcChris, "using local Keystone, after endpoint delete")
	...
	verifyKeystoneCreateTokenRequest(t, hcBob.HTTP, "using remote Keystone, after endpoint recreate")
	verifyKeystoneCreateTokenRequestFails(t, hcChris, "using remote Keystone, after endpoint recreate")
	verifyKeystoneReadTokenRequest(t, hcBob.HTTP, "using remote Keystone, after endpoint recreate")
	verifyKeystoneReadTokenRequestFails(t, hcChris, "using remote Keystone, after endpoint recreate")
}
...
func TestKeystoneEndpointWithMultipleClusters(t *testing.T) {
	...
	verifyKeystoneCreateTokenRequestWithContext(
		auth.WithXClusterID(context.Background(), contrailClusterUUID(clusterCName)),
		t,
		hcUserC,
		"using remote Keystone, with X-Cluster-ID",
	)
	verifyKeystoneCreateTokenRequestWithContext(
	...
	verifyKeystoneReadTokenRequestWithContext(
		auth.WithXClusterID(context.Background(), contrailClusterUUID(clusterCName)),
		t,
		hcUserC,
		"using remote Keystone, with X-Cluster-ID",
	...
	verifyKeystoneCreateTokenRequestFails(t, hcUserC, "using remote Keystone, without X-Cluster-ID")
	verifyKeystoneCreateTokenRequestFails(t, hcUserD, "using remote Keystone, without X-Cluster-ID")
	verifyKeystoneReadTokenRequestFails(t, hcUserC, "using remote Keystone, without X-Cluster-ID")
	verifyKeystoneReadTokenRequestFails(t, hcUserD, "using remote Keystone, without X-Cluster-ID")
}

func verifyKeystoneCreateTokenRequest(t *testing.T, hc *client.HTTP, msg string) {
	verifyKeystoneCreateTokenRequestWithContext(context.Background(), t, hc, msg)
}
func verifyKeystoneCreateTokenRequestWithContext(
...
func verifyKeystoneReadTokenRequest(t *testing.T, hc *client.HTTP, msg string) {
	verifyKeystoneReadTokenRequestWithContext(context.Background(), t, hc, msg)
}

func verifyKeystoneReadTokenRequestWithContext(ctx context.Context, t *testing.T, hc *client.HTTP, msg string) {
	var response pkgkeystone.ValidateTokenResponse
	_, err := hc.Read(ctx, path.Join(keystone.AuthPathPrefix, "auth/tokens"), &response)
