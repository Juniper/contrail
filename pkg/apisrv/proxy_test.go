...
func TestProxyEndpoint(t *testing.T) {
	...
	// Update first neutron endpoint with incorrect port
	var data interface{}
	...
	for _, client := range testScenario.Clients {
		...
		assert.NoError(t, err, "failed to update neutron1 endpoint port")
		break
	}

	// Update second neutron endpoint with incorrect port
	endpointUUID = fmt.Sprintf("endpoint_%s_neutron2_uuid", clusterAName)
	endpoint = map[string]interface{}{"uuid": endpointUUID,
		"public_url":  "http://127.0.0.1",
		"private_url": "http://127.0.0.1",
	}
	...
	for _, client := range testScenario.Clients {
		...
		_, err := client.Update(url, &data, &response)
		assert.NoError(t, err, "failed to update neutron2 endpoint port")
		break
	}
