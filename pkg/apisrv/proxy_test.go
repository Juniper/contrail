...
func verifyProxy(t *testing.T, testScenario *TestScenario, url string,
	clusterName string, excpected_key string, expected string) bool {
	for _, client := range testScenario.Clients {
		var response map[string]interface{}
		_, err := client.Read(url, &response)
		assert.NoError(t, err, "failed to proxy %s", url)
		ok := common.AssertEqual(t,
			map[string]interface{}{excpected_key: clusterName + expected},
			response,
...
func TestProxyEndpoint(t *testing.T) {
	...
	ok := verifyProxy(t, testScenario,
		"/neutron/ports", clusterAName, "name", publicPortList)
	if !ok {
		return
	}
	ok = verifyProxy(t, testScenario,
		"/neutron/private/ports", clusterAName, "name", privatePortList)
	if !ok {
		return
	}
	...
	// verify new proxies
	ok = verifyProxy(t, testScenario,
		"/"+clusterBName+"_uuid/neutron/ports",
		clusterBName, "name", publicPortList)
	if !ok {
		return
	}
	ok = verifyProxy(t, testScenario,
		"/"+clusterBName+"_uuid/neutron/private/ports",
	...
	ok = verifyProxy(t, testScenario,
		"/"+clusterAName+"_uuid/neutron/ports",
		clusterAName, "name", publicPortList)
	if !ok {
		return
	}
	ok = verifyProxy(t, testScenario,
		"/"+clusterAName+"_uuid/neutron/private/ports",
...
func TestKeystoneEndpoint(t *testing.T) {
	...
	ok = verifyProxy(t, &testScenario,
		"/keystone/private/v3/auth/tokens",
