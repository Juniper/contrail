...
func TestDynamicProxyServiceWithClosedTargetServer(t *testing.T) {
	t.Skip("Fix implementation") // TODO: fix implementation
...
func newNeutronServerStub(statusToReturn int) *httptest.Server {
	return newTestHTTPServer(routes{
		portsPath: func(c echo.Context) error {
			return c.JSON(statusToReturn, &portsResponse{Foo: strconv.Itoa(statusToReturn)})
		},
	})
}
...
func verifyMultipleNeutronReadRequests(t *testing.T, c *integration.HTTPAPIClient, clusterName string) {
	for i := 0; i < 5; i++ {
		verifyNeutronReadRequest(t, c, neutronPortsPrivatePath(clusterName), strconv.Itoa(http.StatusOK))
		verifyNeutronReadRequest(t, c, neutronPortsPublicPath(clusterName), strconv.Itoa(http.StatusOK))
	}
}
