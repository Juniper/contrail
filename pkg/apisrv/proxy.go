		...
		go func(endpoint *models.Endpoint) {
			...
			if ok { // proxy endpoint in memory store
				...
				if e.Name != endpoint.Name { // proxy endpoint name changed
					...
					log.Debugf("endpoint name changed from %s to %s", e.Name, endpoint.Name)
				...
				} else if !alreadyMultiCluster && multiCluster { // multi cluster proxies
					// store and create proxy middleware with cluster id prefix
					log.Debug("muliple clusters found, add cluter-id to prefix")
					p.endpointStore.Data.Store(endpoint.UUID, endpoint)
					p.proxyCreateChannel <- proxyCreateMessage{
						clusterID:    clusterID,
						endpointID:   endpoint.UUID,
				...
				} else if alreadyMultiCluster && !multiCluster { // single cluster proxies
					...
					log.Debugf("endpoint url changed from (%s,%s) to (%s,%s)",
						e.PublicURL, e.PrivateURL,
						endpoint.PublicURL, endpoint.PrivateURL)
					p.endpointStore.Data.Store(endpoint.UUID, endpoint)
				}
