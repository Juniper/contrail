...
func (p *proxyService) getReverseProxyPool(urlPath string) (prefix string, proxyPool *httputil.ReverseProxy) {
	var scope string
	...
	proxyPool = apicommon.NewReverseProxyPool(targets)
	return strings.TrimSuffix(proxyPrefix, public), proxyPool
}
...
func (p *proxyService) dynamicProxyMiddleware() func(next echo.HandlerFunc) echo.HandlerFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			prefix, proxyPool := p.getReverseProxyPool(r.URL.Path)
			if proxyPool == nil {
				return echo.NewHTTPError(http.StatusInternalServerError,
					"Proxy endpoint not found in endpoint store")
			}
			r.URL.Path = strings.TrimPrefix(r.URL.Path, prefix)
			proxyPool.ServeHTTP(c.Response(), r)
			return nil
		}
	}
}
