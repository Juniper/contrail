...
func filterProject(user *types.User, scope *types.Scope) (*types.Project, error) {
	if scope == nil {
		return nil, nil
	}
	domain := scope.Domain
...
func (keystone *Keystone) getVncProjects(c echo.Context, configEndpoint string) ([]*types.Project, error) {
...
func (keystone *Keystone) ValidateTokenAPI(c echo.Context) error {
	...
	validateTokenResponse := &types.ValidateTokenResponse{
		Token: token,
	}
