...
func TestClusterLogin(t *testing.T) {
	...
	// Login to cluster with correct credentials
	...
	// Login to cluster with no credentials and no infra(superuser) token
	for _, client := range clients {
		ctx = auth.WithXClusterID(ctx, clusterName+"_uuid")
		client.ID = ""
		client.Password = ""
	...
	// Login to cluster with no credentials and with infra(superuser) token
	for _, client := range clients {
		ctx = auth.WithXClusterID(ctx, clusterName+"_uuid")
		ctx = auth.WithXAuthToken(ctx, infraToken)
		client.ID = ""
		client.Password = ""
		client.Scope = nil
		_, err = client.Login(ctx)
