...
func startWatchers(t *testing.T, watchers map[string][]map[string]interface{}) func(t *testing.T) {
	...
	for key := range watchers {
		...
		checks = append(checks, func(t *testing.T) {
			collected := collect()
			assert.Equal(t, len(events), len(collected), "etcd emitted not enough events")
