...
func (s JSONSchema) Copy() *JSONSchema {
	...
	for i, p := range s.OrderedProperties {
		s.OrderedProperties[i] = s.Properties[p.ID]
	}
...
func (api *API) bumpTimestamp(t time.Time) {
	if t.After(api.Timestamp) {
		api.Timestamp = t
	}
}
...
func getPath(dir, subdir string) string {
	if subdir == "" {
		return ""
	}
