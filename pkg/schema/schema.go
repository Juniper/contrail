...
func (api *API) resolveAllRelation() error {
	for _, s := range api.Schemas {
		...
		for linkTo, reference := range s.References {
			if referenceSchema, err := api.resolveReference(s, reference, linkTo); err != nil {
				return err
			} else {
		...
		for linkTo, parent := range s.Parents {
			...
			if parentSchema, err := api.resolveReference(s, parent, linkTo); err != nil {
				return err
			} else {
...
func (api *API) resolveReference(from *Schema, to *Reference, toID string) (*Schema, error){
	...
	to.Table = ReferenceTableName(RefPrefix, from.Table, toID)
