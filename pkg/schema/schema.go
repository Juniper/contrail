...
func (api *API) resolveRelation(schema *Schema, linkTo string, reference *Reference) error {
	...
	linkToSchema.BackReferences[schema.ID] = &BackReference{
		LinkTo: schema,
	}
...
func (api *API) resolveAllRelation() error {
	for _, s := range api.Schemas {
		...
		for _, m := range mapSlice(s.ReferencesSlice) {
			...
			if err := api.resolveRelation(s, linkTo, reference); err != nil {
				return err
			}
		}
