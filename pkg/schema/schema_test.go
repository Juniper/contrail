...
func TestFieldname(t *testing.T) {
	...
	assert.Equal(t, "colon:separated:in:base", base.JSONSchema.Properties["colonseparatedinbase"].FieldName)

	derived := api.SchemaByID("derived_object")
	require.NotNil(t, derived, "derived_object schema shouldn't be <nil>")
	assert.Equal(t, "colon:separated:in:derived", derived.JSONSchema.Properties["colonseparatedinderived"].FieldName)
}
