...
func TestUpdateComplexField(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			db.CreateFirewallRule(context.Background(), createFireWallRuleRequest(tt.resource))
			db.UpdateFirewallRule(context.Background(), updateFireWallRuleRequest(tt.resource, tt.paths))
			...
			assert.Equal(t, tt.updateResource, response.GetFirewallRule())
		})
	}
}
