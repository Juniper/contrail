...
func (db *Service) AllocateIP(
	...
	if len(virtualNetwork.GetAddressAllocationMode()) > 0 &&
		virtualNetwork.GetAddressAllocationMode() != "user-defined-subnet-preferred" {
		return "", "", errors.Errorf("Allocation mode %v is not supported", virtualNetwork.GetAddressAllocationMode())
	}
