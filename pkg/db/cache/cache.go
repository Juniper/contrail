...
func (db *DB) updateDependentNodes(event *services.Event, oldResource services.Resource) {
	resource := event.GetResource()
	...
	case services.OperationUpdate:
		dependencies := oldResource.Depends()
		for _, dependencyID := range dependencies {
			dependentNode, ok := db.idMap[dependencyID]
			if ok {
				dependentNode.event.GetResource().RemoveDependency(oldResource)
				dependentNode.pop()
				db.append(dependentNode)
			}
		}
