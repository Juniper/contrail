...
//CacheDB is cache db implemenation.
type CacheDB struct {
	first        *node
	last         *node
	lastIndex    uint64
...
//nolint: gocyclo
func (w *Watcher) watch(ctx context.Context, db *CacheDB) {
	defer func() {
...
func NewDBCache(maxHistory uint64) *CacheDB {
	//TODO(nati) db dump
	db := &CacheDB{
		versionMap:   map[uint64]*node{},
		idMap:        map[string]*node{},
...
func (db *CacheDB) AddWatcher(ctx context.Context, versionID uint64) (*Watcher, error) {
...
func (db *CacheDB) update(event *services.Event) {
	db.mutex.Lock()
...
func (db *CacheDB) updateDBVersion(n *node) {
	db.versionMap[db.lastIndex] = n
	n.version = db.lastIndex
	db.lastIndex++
}

func (db *CacheDB) append(n *node) {
	if db.first == nil {
		db.first = n
	}
	db.last.setNext(n)
...
func (db *CacheDB) removeTooOldNodesIfNeeded() {
...
func (db *CacheDB) addDependencies(resource services.Resource) {
...
func (db *CacheDB) removeDependencies(resource services.Resource) {
	dependencies := resource.Depends()
	for _, dependencyID := range dependencies {
		dependentNode, ok := db.idMap[dependencyID]
		if ok {
...
func (db *CacheDB) updateDependentNodes(event services.HasResource, oldResource services.Resource) {
	resource := event.GetResource()
...
func (db *CacheDB) Process(ctx context.Context, event *services.Event) (*services.Event, error) {
	db.update(event)
...
func (db *CacheDB) Get(id string) *services.Event {
	db.mutex.RLock()
	defer db.mutex.RUnlock()
...
func (db *CacheDB) getNodeByVersion(version uint64) *node {
	db.mutex.RLock()
	defer db.mutex.RUnlock()
