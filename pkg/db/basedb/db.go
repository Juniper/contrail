...
//DSN formtas.
const (
	DSNFormatMySQL      = "%s:%s@tcp(%s:3306)/%s"
	DSNFormatPostgreSQL = "sslmode=disable user=%s password=%s host=%s dbname=%s"
)
...
//MakeConnection opens sql connection.
...
func ConnectDB() (*sql.DB, error) {
	...
	case DriverPostgreSQL:
		dbDSNFormat = DSNFormatPostgreSQL
	case DriverMySQL:
		dbDSNFormat = DSNFormatMySQL
	default:
