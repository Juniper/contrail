...
func NewQueryBuilder(
	dialect Dialect, table string, fields []string, refFields map[string][]string, childFields map[string][]string, backRefFields map[string][]string,
) *QueryBuilder {
	qb := &QueryBuilder{}
	qb.Dialect = dialect
	qb.Table = table
...
func (qb *QueryBuilder) buildFilterQuery(ctx *queryContext) {
	...
	// This will be reenabled later when back references will be supported
...
func (qb *QueryBuilder) buildChildQuery(ctx *queryContext) {
	...
	for refTable, refFields := range qb.ChildFields {
		...
		ctx.columnParts = append(
			ctx.columnParts,
			subQuery)
		ctx.columns["child_"+refTable+"_"+qb.Table] = len(ctx.columns)
	}
}
