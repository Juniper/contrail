...
func (qb *QueryBuilder) buildBackRefQuery(ctx *queryContext) {
	...
	// 		ctx.columnParts = append(
	// 			ctx.columnParts,
	// 			qb.quote(refTable+"_ref"),
	// 		)
	// 		ctx.columns["backref_"+refTable] = len(ctx.columns)
	// 	}
	// 	if spec.Detail || len(spec.BackRefUUIDs) > 0 {
	// 		subQuery := "left join (select " +
	// 			qb.as(qb.anyValue(refTable, "uuid"), "uuid") + " , " +
	// 			qb.as(qb.anyValue(refTable, "parent_uuid"), "parent_uuid") + " , " +
	// 			qb.as(qb.jsonAgg(refTable, refFields...), qb.quote(refTable+"_ref")) +
	// 			" from " + qb.quote(refTable) +
	// 			" group by " + qb.quote(refTable, "parent_uuid") +
	// 			" ) as " +
	// 			qb.quote(refTable+"_temp") + " on " +
	// 			qb.quote(refTable+"_temp", "parent_uuid") + " = " + qb.quote(qb.Table, "uuid")
	// 		ctx.joins = append(ctx.joins, subQuery)
	// 	}
	// }
	spec := ctx.spec
	for refTable, refFields := range qb.BackRefFields {
