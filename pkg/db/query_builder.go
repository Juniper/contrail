...
func (qb *QueryBuilder) buildBackRefQuery(ctx *queryContext) {
	...
	for backref, backrefFields := range qb.BackRefFields {
		if !qb.islinkToInField(ctx, backref+"backrefs") {
			continue
		}
		backref = strings.ToLower(backref)
		backrefFields = append(backrefFields, "from")
		backrefFields = append(backrefFields, "to")
		subQuery := "(select " +
			qb.as(qb.jsonAgg(refTable+"_t", refFields...), qb.quote(refTable+"_ref")) +
			" from " + qb.as(qb.quote(refTable), refTable+"_t") +
			" where " + qb.quote(qb.Table, "uuid") + " = " + qb.quote(refTable+"_t", "from") +
		...
		ctx.columnParts = append(
			ctx.columnParts,
			subQuery)
		ctx.columns["backref_"+linkTo] = len(ctx.columns)
	}
}
