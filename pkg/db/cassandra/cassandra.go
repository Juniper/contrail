...
func DumpCassandra(host string, port int) (*services.EventList, error) {
...
func NewEventProducer(processor services.EventProcessor, queueName, cassandraHost string, cassandraPort int, rabbitMQHost string) *EventProducer {
...
func (p *EventProducer) Start(ctx context.Context) error {
	events, err := DumpCassandra(p.cassandraHost, p.cassandraPort)
	if err != nil {
		return err
	}
	events.Sort()
