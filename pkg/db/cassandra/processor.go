...
func handleEvent(session *gocql.Session, event *services.Event) error { // nolint: interfacer
	...
	case services.OperationCreate, services.OperationUpdate:
		...
		for column1, value := range cassandraMap {
			...
			updateBackref(batch, rsrc, referredType, referredColumn1)
		}
...
func updateBackref(b *gocql.Batch, r services.Resource, referredType string, referredUUID string) {
	...
	if r.Kind() == referredType { // TODO: is this correct? _ vs -
		...
		referredColumn1 = fmt.Sprintf("ref:%s:%s", r.Kind(), rsrc.GetUUID())
	} else {
		// We add a "backref:" to the referred object
		referredColumn1 = fmt.Sprintf("backref:%s:%s", r.Kind(), rsrc.GetUUID())
	}

	withTimestamp(batch, deleteRowByColumn1, referredUUID, referredColumn1)
	withTimestamp(batch, insertQuery, referredUUID, referredColumn1, value)
}
