...
func childColumn1(r services.Resource) string {
	return fmt.Sprintf("children:%s:%s", r.SchemaID(), r.GetUUID())
}
...
func deleteFQNameToUUIDRow(b *gocql.Batch, r services.Resource) {
	withTimestamp(b, deleteFromFQNameTable, r.SchemaID(), getFQNameToUUIDColumn1(r))
}

func createFQNameToUUIDRow(b *gocql.Batch, r services.Resource) {
	withTimestamp(b, insertIntoFQNameTable, r.SchemaID(), getFQNameToUUIDColumn1(r), "null")
}
...
func updateBackref(b *gocql.Batch, r services.Resource, referredType, referredUUID, value string) {
	...
	if r.SchemaID() == referredType { // TODO: is this correct? _ vs -
		// This is a symmetric ref, we add a "ref:" to the referred object
		referredColumn1 = fmt.Sprintf("ref:%s:%s", r.SchemaID(), r.GetUUID())
	} else {
		// We add a "backref:" to the referred object
		referredColumn1 = fmt.Sprintf("backref:%s:%s", r.SchemaID(), r.GetUUID())
	}
