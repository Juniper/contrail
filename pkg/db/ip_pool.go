...
func (db *Service) getIPPools(ctx context.Context, target *ipPool) ([]*ipPool, error) {
	...
	query.WriteString("select " + db.Dialect.selectIp("start") + ", " + db.Dialect.selectIp("end") + " from ipaddress_pool where ")
	...
	if target.end.Equal(net.IP{}) {
		...
		query.WriteString(" and " + d.literalIp(target.start) + " < " + d.quote("end") + " and " +
			d.quote("start") + " < " + d.literalIp(target.end) + " order by start for update ")
		rows, err = tx.QueryContext(ctx, query.String())
	}
	pools := []*ipPool{}
	err = handleError(err)
...
// deleteIPPools deletes ip pool that overlaps target range. delete all if target.End is zero. Whole pools are removed, not only overlapping parts.
func (db *Service) deleteIPPools(ctx context.Context, target *ipPool) error {
	...
	if target.end.Equal(net.IP{}) {
		_, err = tx.ExecContext(ctx, "delete from ipaddress_pool where "+
			db.Dialect.quote("key")+" = '"+target.key+"';")
	} else {
...
func (db *Service) allocateIP(ctx context.Context, key string) (net.IP, error) {
	...
	query := "select " + db.Dialect.selectIp("start") + ", " + db.Dialect.selectIp("end") +
		...
		d.quote("key") + " = '" + key + "' limit 1 for update"
	...
	if bytes.Compare(updatedStart.To16(), end.To16()) < 0 {
		_, err = tx.ExecContext(ctx,
			"update ipaddress_pool set "+d.quote("start")+" = "+d.literalIp(updatedStart)+
				" where "+d.quote("key")+" = '"+key+"' and "+d.quote("start")+" = "+d.literalIp(start))
	} else {
		_, err = tx.ExecContext(ctx,
			"delete from ipaddress_pool where "+d.quote("key")+" = '"+key+"' and "+
				d.quote("start")+" = "+d.literalIp(start))
	}
	if err != nil {
		return nil, handleError(err)
	}
...
func (db *Service) setIP(ctx context.Context, key string, ip net.IP) error {
	...
	if pool.start.Equal(ip) {
		_, err = tx.ExecContext(
			ctx,
			"insert into ipaddress_pool ("+d.quoteSep("key", "start", "end")+") values ( '"+key+"', "+d.literalIp(cidr.Inc(pool.start))+", "+d.literalIp(pool.end)+")")
		if err != nil {
		...
		_, err = tx.ExecContext(
			ctx,
			"insert into ipaddress_pool ("+d.quoteSep("key", "start", "end")+") values ( '"+key+"', "+d.literalIp(pool.start)+", "+d.literalIp(cidr.Dec(pool.end))+")")
		if err != nil {
			return handleError(err)
		}
	} else {
		...
		_, err = tx.ExecContext(
			ctx,
			"insert into ipaddress_pool ("+d.quoteSep("key", "start", "end")+") values ( '"+key+"', "+d.literalIp(pool.start)+", "+d.literalIp(ip)+")")
		if err != nil {
			return handleError(err)
		}
		_, err = tx.ExecContext(
			ctx,
			"insert into ipaddress_pool ("+d.quoteSep("key", "start", "end")+") values ( '"+key+"', "+d.literalIp(cidr.Inc(ip))+", "+d.literalIp(pool.end)+")")
		if err != nil {
			return handleError(err)
		}
	}
...
func (db *Service) deallocateIPRange(ctx context.Context, target *ipPool) error {
	...
	q := "insert into ipaddress_pool (" + d.quoteSep("key", "start", "end") + ") values ( '" + target.key + "', " + d.literalIp(start) + ", " + d.literalIp(end) + ")"
