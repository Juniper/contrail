...
func (db *Service) getIPPools(ctx context.Context, target *ipPool) ([]*ipPool, error) {
	...
	query.WriteString(db.Dialect.quote("key") + " = '" + target.key + "'") //db.Dialect.placeholder(1))
	var rows *sql.Rows
	var err error
...
func (db *Service) setIP(ctx context.Context, key string, ip net.IP) error {
	...
	if bytes.Compare(pool.start, ip) == 0 {
		_, err = tx.ExecContext(
			...
			key, d.insertIp(cidr.Inc(pool.start)), d.insertIp(pool.end))
		if err != nil {
		...
		_, err = tx.ExecContext(
			...
			"insert into ipaddress_pool ("+d.quoteSep("key", "start", "end")+") values ("+
				d.values("key", "start", "end")+");",
			key, d.insertIp(pool.start), d.insertIp(cidr.Dec(pool.end)))
		if err != nil {
			return handleError(err)
		}
	} else {
		...
		_, err = tx.ExecContext(
			...
			"insert into ipaddress_pool ("+d.quoteSep("key", "start", "end")+") values ("+
				d.values("key", "start", "end")+");",
			key, d.insertIp(pool.start), d.insertIp(ip))
		if err != nil {
			return handleError(err)
		}
		_, err = tx.ExecContext(
			...
			"insert into ipaddress_pool ("+d.quoteSep("key", "start", "end")+") values ("+
				d.values("key", "start", "end")+");",
			key, d.insertIp(cidr.Inc(ip)), d.insertIp(pool.end))
		if err != nil {
			return handleError(err)
		}
	}
