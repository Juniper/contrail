...
func TestRouteAggregate(t *testing.T) {
	...
	ServiceInstanceCreateRef = append(ServiceInstanceCreateRef,
		&models.RouteAggregateServiceInstanceRef{UUID: ServiceInstanceRefUUID, To: []string{"test", ServiceInstanceRefUUID}})
	ServiceInstanceCreateRef = append(ServiceInstanceCreateRef,
		&models.RouteAggregateServiceInstanceRef{UUID: ServiceInstanceRefUUID2, To: []string{"test", ServiceInstanceRefUUID2}})
	...
	_, err = db.GetRouteAggregate(ctx, &models.GetRouteAggregateRequest{
		ID: model.UUID})
	if err == nil {
