...
func TestRoutingPolicy(t *testing.T) {
	...
	ServiceInstanceCreateRef = append(ServiceInstanceCreateRef,
		&models.RoutingPolicyServiceInstanceRef{UUID: ServiceInstanceRefUUID, To: []string{"test", ServiceInstanceRefUUID}})
	ServiceInstanceCreateRef = append(ServiceInstanceCreateRef,
		&models.RoutingPolicyServiceInstanceRef{UUID: ServiceInstanceRefUUID2, To: []string{"test", ServiceInstanceRefUUID2}})
	...
	_, err = db.GetRoutingPolicy(ctx, &models.GetRoutingPolicyRequest{
		ID: model.UUID})
	if err == nil {
