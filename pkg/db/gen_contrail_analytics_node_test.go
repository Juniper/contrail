...
func TestContrailAnalyticsNode(t *testing.T) {
	...
	NodeCreateRef = append(NodeCreateRef,
		&models.ContrailAnalyticsNodeNodeRef{UUID: NodeRefUUID, To: []string{"test", NodeRefUUID}})
	NodeCreateRef = append(NodeCreateRef,
		&models.ContrailAnalyticsNodeNodeRef{UUID: NodeRefUUID2, To: []string{"test", NodeRefUUID2}})
	...
	_, err = db.CreateContrailAnalyticsNode(ctx,
	...
	response, err := db.ListContrailAnalyticsNode(ctx, &models.ListContrailAnalyticsNodeRequest{
	...
	if len(response.ContrailAnalyticsNodes) != 1 {
		t.Fatal("expected one element", err)
	}

	ctxDemo := context.WithValue(ctx, "auth", common.NewAuthContext("default", "demo", "demo", []string{}))
	_, err = db.DeleteContrailAnalyticsNode(ctxDemo,
	...
	_, err = db.DeleteContrailAnalyticsNode(ctx,
		&models.DeleteContrailAnalyticsNodeRequest{
			ID: model.UUID})
	if err != nil {
	...
	_, err = db.GetContrailAnalyticsNode(ctx, &models.GetContrailAnalyticsNodeRequest{
		ID: model.UUID})
	if err == nil {
