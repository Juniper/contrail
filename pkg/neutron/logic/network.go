...
func (n *Network) toVncForCreate(
	...
	if n.RouterExternal {
		vncNet.Perms2 = &models.PermType2{
			...
			OwnerAccess:  PERMS_RWX,
			GlobalAccess: PERMS_RX,
		}
	}
...
func (n *Network) toVncForUpdate(
	...
	if n.RouterExternal {
		vncNet.Perms2.GlobalAccess = PERMS_RX
	} else {
		vncNet.Perms2.GlobalAccess = PERMS_NONE
	}
	return vncNet, nil

}

func (n *Network) toVnc(ctx context.Context, rp RequestParameters) (*models.VirtualNetwork, error) {
	var vncNet *models.VirtualNetwork
	var err error
	operation := rp.RequestContext.Operation
	if operation == "CREATE" {
