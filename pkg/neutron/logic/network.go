...
func (n *Network) updateVnc(
	...
	if basemodels.FieldMaskContains(&rp.FieldMask, buildDataResourcePath(NetworkFieldPolicys)) && len(n.Policys) > 0 {
		//TODO handle policy refs and verify type of 'policys' field with multiple items
	}
	if basemodels.FieldMaskContains(&rp.FieldMask, buildDataResourcePath(NetworkFieldRouteTable)) && len(n.RouteTable) > 0 {
