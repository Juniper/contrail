...
func (n *Network) Create(ctx context.Context, rp RequestParameters) (Response, error) {
	...
	nn := makeNetworkResponse(rp, vnResp.GetVirtualNetwork(), neutron.OperationCreate)
	return nn, nil
}
...
func (n *Network) Read(
	...
	return makeNetworkResponse(rp, vnRes.GetVirtualNetwork(), neutron.OperationRead), nil
}
...
func (n *Network) updateVnc(
	...
	if operation == neutron.OperationCreate || operation == neutron.OperationUpdate {
...
func (n *Network) toVnc(ctx context.Context, rp RequestParameters) (vncNet *models.VirtualNetwork, err error) {
	if rp.RequestContext.Operation == neutron.OperationCreate {
...
func convertVNsToNeutronResponse(
	...
	for _, vn := range vns {
		...
		nn := makeNetworkResponse(rp, vn, neutron.OperationReadAll)
		if nn == nil {
			continue
		}
