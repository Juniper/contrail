...
// HasKeys checks if one or more keys are present in filters.
// Will return true if at least one key has been defined and all keys are present and not empty.
func (f Filters) HasKeys(keys ...FilterKey) bool {
	if len(keys) == 0 {
		return false
	}
...
func (f Filters) Match(key FilterKey, values ...string) bool {
	...
	return f.HasValues(key, values...)
}

// HasValues check equality of values in filters struct under specific key and provided sequence of strings.
func (f Filters) HasValues(key FilterKey, values ...string) bool {
