...
// TODO: reorder methods and functions. Because now they are little messy...
...
func (sg *SecurityGroup) Create(ctx context.Context, rp RequestParameters) (Response, error) {
	vncSg, err := sg.vncFromNeutronOnCreate(ctx, rp)
	if err != nil {
		return nil, newSecurityGroupError(err,
			fmt.Sprintf("error while casting %s from neutron into vnc resource", securityGroupResourceName))
	}
...
func (sg *SecurityGroup) neutronFromVncMany(vncSgs []*models.SecurityGroup, f Filters) ([]*SecurityGroupResponse, error) {
...
func (sg *SecurityGroup) readSecurityGroupRules(vncSG *models.SecurityGroup) (rules []*SecurityGroupRuleResponse, err error) {
	for _, rule := range vncSG.GetSecurityGroupEntries().GetPolicyRule() {
		sgr, err := securityGroupRuleNeutronFromVnc(vncSG, rule)
		if err != nil {
			return nil, err
		}
...
func (sg *SecurityGroup) vncFromNeutronOnCreate(ctx context.Context, rp RequestParameters) (
	*models.SecurityGroup, error) {
	project, err := sg.getProject(ctx, rp)
	if err != nil {
		return nil, err
	...
	sgUUID, err := neutronIDToVncUUID(sg.ID)
	if err != nil {
