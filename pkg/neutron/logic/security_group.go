...
func (*SecurityGroup) getFilterProjectIDS(ctx context.Context, rp RequestParameters, f Filters) []string {
	if !rp.RequestContext.IsAdmin {
		return []string{rp.RequestContext.Tenant}
	}
	return f[SecurityGroupFieldTenantID]
}
...
func (sg *SecurityGroup) listSecurityGroups(
	...
	for _, uuid := range sg.getFilterProjectIDS(ctx, rp, filter) {
