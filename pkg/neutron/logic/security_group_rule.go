...
func (sgr *SecurityGroupRule) newSecurityGroupRuleError(err error, message string) error {
...
func (sgr *SecurityGroupRule) getGenericDefaultSecurityGroupRule() *SecurityGroupRule {
...
func (sgr *SecurityGroupRule) getDefaultSecurityGroupRuleIPv4() *SecurityGroupRule {
	defaultSgr := sgr.getGenericDefaultSecurityGroupRule()
	defaultSgr.Ethertype = ethertypeIPv4
	defaultSgr.RemoteIPPrefix = emptyadressIPv4withMask
	return defaultSgr
}

func (sgr *SecurityGroupRule) getDefaultSecurityGroupRuleIPv6() *SecurityGroupRule {
	defaultSgr := sgr.getGenericDefaultSecurityGroupRule()
	defaultSgr.Ethertype = ethertypeIPv6
	defaultSgr.RemoteIPPrefix = emptyadressIPv6withMask
	return defaultSgr
}
...
func (sgr *SecurityGroupRule) securityGroupRuleNeutronToContrail(ctx context.Context, rp RequestParameters,
	...
	if sgr.RemoteIPPrefix != "" {
		...
		if err != nil {
			return nil, sgr.newSecurityGroupRuleError(err, "can't determinate ip version of the ip: "+string(ipNetworkVersion))
		}
		...
		if err != nil {
			return nil, sgr.newSecurityGroupRuleError(err, "can't determinate ip prefix and ip prefix length"+
