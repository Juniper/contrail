...
func (sgr SecurityGroupRule) initAddressType(ctx context.Context, rp RequestParameters, vncSgr *models.PolicyRuleType) error {
	...
	if sgr.RemoteIPPrefix != "" {
		...
		if err != nil {
			return errors.Wrapf(err, "can't determinate ip version of the IP: "+string(sgr.RemoteIPPrefix))
		}

		if (ipNetworkVersion == 4 && etherType != ethertypeIPv4) ||
			(ipNetworkVersion == 6 && etherType != ethertypeIPv6) {
			return newNeutronError(securityGroupRuleParameterConflict, errorFields{
				"ethertype": etherType,
		...
		if err != nil {
			return errors.Wrapf(err, "can't determinate ip prefix and ip prefix length"+
