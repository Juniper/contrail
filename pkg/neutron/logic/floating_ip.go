...
func (fip *Floatingip) Read(ctx context.Context, rp RequestParameters, id string) (Response, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "invalid uuid format %v for READ FloatingIp", id)
	}
	resp, err := rp.ReadService.GetFloatingIP(ctx, &services.GetFloatingIPRequest{ID: uuid})
	// TODO: This error is also returned when virtual network can't be found during creating response
	//	find a way to separate those errors, meanwhile message is added to below error handling
