...
func (fip *Floatingip) ReadAll(
	...
	refs := createRefFilters(rp.RequestContext, filters)
	vncFilters := createVncFilters(filters)
...
func createRefFilters(rc RequestContext, f Filters) map[string]*baseservices.UUIDs {
	refs := make(map[string]*baseservices.UUIDs)
	if !rc.IsAdmin {
		refs[models.FloatingIPFieldProjectRefs] = &baseservices.UUIDs{[]string{rc.TenantID}}
	} else if projects, ok := f["tenant_id"]; ok {
		ps := neutronIDsToVncUUIDs(projects)
		if len(ps) > 0 {
			refs[models.FloatingIPFieldProjectRefs] = &baseservices.UUIDs{ps}
		}
	}

	if vmis, ok := f["port_id"]; ok {
		refs[models.FloatingIPFieldVirtualMachineInterfaceRefs] = &baseservices.UUIDs{vmis}
	}
