	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if ipPrefix != tt.outputIPPrefix {
				t.Errorf("Error wrong IP adress. Expected: '%+v', got: '%+v'", tt.outputIPPrefix, ipPrefix)
			}

			if ipNetwork != tt.outputIPNetworPrefix {
				t.Errorf("Error wrong IP Network adress. Expected: '%+v', got: '%+v'", tt.outputIPNetworPrefix, ipNetwork)
			}

			if ipPrefixLen != tt.outputIPPrefixLen {
				t.Errorf("Error wrong IP Prefix lenght. Expected: '%+v', got: '%+v'", tt.outputIPPrefixLen, ipPrefixLen)
			}
		})
	}
}
