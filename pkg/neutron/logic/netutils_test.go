...
func TestGetIPPrefixAndPrefixLen(t *testing.T) {
	...
	tests := []struct {
		...
		{"wrong ip - without mask", "137.105.215.54", "", "", 0, true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ipPrefix, netIP, prefixLen, err := getIPPrefixAndPrefixLen(tt.ipCIDR)

			if err != nil && !tt.expectError {
				t.Errorf("expected no error but got: '%+v'", err)
			}

			if ipPrefix != tt.expectedIPPrefix {
				t.Errorf("error wrong IP adress. Expected: '%+v', got: '%+v'", tt.expectedIPPrefix, ipPrefix)
			}

			if netIP != tt.expectedNetIP {
				t.Errorf("error wrong IP Network adress. Expected: '%+v', got: '%+v'", tt.expectedNetIP, netIP)
			}

			if prefixLen != tt.expectedPrefixLen {
				t.Errorf("error wrong IP Prefix lenght. Expected: '%+v', got: '%+v'", tt.expectedPrefixLen, prefixLen)
			}
		})
	}
}
