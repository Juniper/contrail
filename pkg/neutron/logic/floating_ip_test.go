	...
	tests := []*neutronTestCase{
		...
		},
		{
			...
			expectedCalls: func() {
				...
				}).Return(loadGetFloatingIPPoolResponse(t, createDirectory+"get_floating-ip-pool_response.json"), nil)
				...
				read.EXPECT().GetFloatingIP(gomock.Any(),
					...
					}).Return(loadGetFloatingIPResponse(t, createDirectory+"get_floating-ip_response.json"), nil)
		...
		},
		{
			...
			expectedCalls: func() {
				...
				write.EXPECT().UpdateFloatingIP(gomock.Any(), loadUpdateFloatingIPRequest(t, updateDirectory+"update_floating-ip_request.json"))
				read.EXPECT().GetFloatingIP(gomock.Any(),
					&services.GetFloatingIPRequest{
						ID: "f4d63b5a-22e6-4aad-8b83-624b75a82e45",
...
func loadUpdateFloatingIPResponse(t *testing.T, path string) (r *services.UpdateFloatingIPResponse) {
	require.NoError(t, fileutil.LoadFile(path, &r))
	return r
}
