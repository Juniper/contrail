...
type neutronTestCase struct {
	...
	expected  Response
	given     func()
	operation func(context.Context, *testing.T, RequestParameters, *Request) (Response, error)
	fails     bool
}
...
func TestFloatingip_Create(t *testing.T) {
	...
	tests := []*neutronTestCase{
		{
			name: "create",
			operation: func(ctx context.Context, t *testing.T, rp RequestParameters, r *Request) (Response, error) {
...
func (tt *neutronTestCase) run(t *testing.T, ctx context.Context, rp RequestParameters) {
	...
	if tt.operation != nil {
		got, err = tt.operation(ctx, t, rp, &tt.request)
	}
	if tt.fails {
		assert.Error(t, err)
	} else {
