...
func (data *Subnet) updateSubnet(
	...
	if basemodels.FieldMaskContains(&fm, buildDataResourcePath(SubnetFieldDNSNameservers)) {
		origin.SetDNSNameservers(data.DNSNameservers)
	}
...
func portUpdateIfaceRouteTable(
	...
	if err = handleDeprecatedHostPrefixes(ctx, rp, IPsResponse.InstanceIPs, prefixes, subnetID); err != nil {
		return err
	}

	if err = handleNewHostPrefixes(ctx, rp, IPsResponse.InstanceIPs, prefixes, subnetID); err != nil {
		return err
	}
...
func portRemoveIfaceRouteTable(
	...
	for _, port := range portRefs {
		vmi, err := rp.ReadService.GetVirtualMachineInterfaceRead(ctx, &services.GetVirtualMachineInterfaceRequest{
			ID: port.GetUUID(),
		})
		if err != nil {
			return err
		...
		irt := vmi.FindInterfaceRouteTable(func(ref *models.VirtualMachineInterfaceInterfaceRouteTableRef) bool {
		...
		_, err = rp.WriteService.UpdateVirtualMachineInterface(ctx, &services.UpdateVirtualMachineInterfaceRequest{
			...
			FieldMask: []string{models.VirtualMachineInterfaceFieldInterfaceRouteTableRefs}
		})
		if err != nil {
			return err
		}
		...
		// 	if basemodels.FQNameToName(irt.To) == irtName {
...
func GetHostPrefixes(hostRoutes []*RouteTableType, subnetCIDR string) (hostPrefixes, error) {
	hostPrefs := make(map[string][]string)
	...
	for id, route := range hostRoutes {
		...
		if subnet.Contains(ip) {
			...
			if len(hostPrefs[route.Nexthop]) > 0 {
				hostPrefs[route.Nexthop] = append(hostPrefs[route.Nexthop], route.Destination)
			} else {
				hostPrefs[route.Nexthop] = []string{route.Destination}
			}
		} else {
	...
	if len(unresolvedHostRoutes) > 0 {
		for id := range hostPrefs {
			hostPrefs[id], unresolvedHostRoutes, err = updatePrefixes(hostPrefs[id], unresolvedHostRoutes)
			if err != nil {
				return nil, err
			}
		}
