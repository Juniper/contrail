...
func (s *Subnet) updateSubnet(
	...
	if basemodels.FieldMaskContains(fm, buildDataResourcePath(SubnetFieldHostRoutes)) {
		...
		if true {
			...
			if err := portUpdateIfaceRouteTable(ctx, rp, vn, cidr, subnetUUID, s.HostRoutes, oldHostRoutes); err != nil {
				return nil, err
			}
		}
...
func portUpdateIfaceRouteTable(
	...
	if err = handleDeprecatedHostPrefixes(ctx, rp, IPsResponse.InstanceIPs, hostPrefixesToDelete, subnetID); err != nil {
		return err
	}

	if err = handleNewHostPrefixes(ctx, rp, IPsResponse.InstanceIPs, hostPrefixesToAdd, subnetID); err != nil {
		return err
	}
...
func extractPrefixesRequiredForUpdate(
	...
	for _, ipaddr := range hostPrefixesToDelete.GetIPAddresses() {
		if hostPrefixesToAdd.HasAnyDestinations(ipaddr) {
			if arePrefixesEqual(hostPrefixesToDelete.GetDestinations(ipaddr), hostPrefixesToAdd.GetDestinations(ipaddr)) {
				// Nothing changed for this nexthop address so there is no need to change it.
				hostPrefixesToAdd.RemoveDestinationsForIP(ipaddr)
			}
			// There are still destinations for this nexthop address so we don't want to delete it.
			hostPrefixesToDelete.RemoveDestinationsForIP(ipaddr)
		}
	}
