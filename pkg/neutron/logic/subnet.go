...
func (*Subnet) ReadAll(ctx context.Context, rp RequestParameters, filters Filters, fields Fields) (response Response, err error) {
	var vns []*models.VirtualNetwork
	switch {
	case filters.haveKeys(idKey):
		vns, err = collectVNsUsingKV(ctx, rp, v)
...
func (*Subnet) Read(ctx context.Context, rp RequestParameters, id string) (Response, error) {
	...
	if sub == nil {
		return &SubnetResponse{}, newSubnetError(
