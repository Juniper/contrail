...
func listVirtualNetworksWithShared(ctx context.Context, rp RequestParameters, filters Filters) (vns []*models.VirtualNetwork, err error) {
	...
	tenantVNsMap := make(map[string]bool, len(tenantVNs))
	for _, vn := range tenantVNs {
		tenantVNsMap[vn.GetUUID()] = true
	}
	...
	for _, vn := range sharedVNs {
		if !tenantVNsMap[vn.GetUUID()] {
			vns = append(vns, vn)
		}
	}
