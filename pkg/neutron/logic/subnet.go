...
func subnetAllocationPoolsToNeutron(aps []*models.AllocationPoolType, ipamHasSubnet bool, subnet *SubnetResponse) {
	...
	if !ipamHasSubnet {
		...
		subnet.AllocationPools = append(subnet.AllocationPools, subnetDefaultAllocationPool(subnet.GatewayIP))
	}
}

func subnetDefaultAllocationPool(gatewayIP string) *AllocationPool {
	// TODO: calculate default allocation pool neutron_plugin_db.py:1690
