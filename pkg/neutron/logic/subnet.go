...
func portAddIfaceRouteTable(
	...
	fqNameIRT := append(proj.Project.FQName, irtName)

	uuidIRT, err := getInterfaceRouteTableUUID(ctx, rp, fqNameIRT)
	if err != nil {
		return err
	}

	var irt *models.InterfaceRouteTable
	...
	if uuidIRT == "" {
		...
		uuidIRT = irtCreated.InterfaceRouteTable.UUID
	}
	...
	irt = irtResponse.InterfaceRouteTable
	...
	for _, prefix := range prefixes {
		irt.InterfaceRouteTableRoutes.Route = append(irt.InterfaceRouteTableRoutes.Route, &models.RouteType{
			Prefix: prefix,
			// Should we not add next hop here?
		})
	}
...
func getInterfaceRouteTableUUID(ctx context.Context, rp *RequestParameters, fqName []string) (string, error) {
