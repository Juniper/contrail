...
func TestSubnet_ReadAll(t *testing.T) {
	...
	tests := []struct {
			...
			mockVN: mockVN{
				VirtualNetworks: &services.ListVirtualNetworkResponse{
					VirtualNetworks: []*models.VirtualNetwork{
						fakeVirtualNetwork("blue", 2),
						fakeVirtualNetwork("red", 1),
						fakeVirtualNetwork("green", 0),
					},
				},
			},
			expected: []*logic.SubnetResponse{
		...
		},
		{
			...
			filters: logic.Filters{"id": []string{"virtual_network_blue"}},
			mockVN: mockVN{
				VirtualNetworks: &services.ListVirtualNetworkResponse{
					VirtualNetworks: []*models.VirtualNetwork{
						fakeVirtualNetwork("blue", 1),
					},
				},
			},
			expected: []*logic.SubnetResponse{
...
func mockUserAgentService(
	...
	err error,
) *servicesmock.MockUserAgentKVService {
...
func fakeVirtualNetwork(name string, subnets int) *models.VirtualNetwork {
