...
func (port *Port) setPortSecurity(
	...
	for _, sc := range securityGroups {
		vmi.SecurityGroupRefs = append(vmi.SecurityGroupRefs, &models.VirtualMachineInterfaceSecurityGroupRef{
			UUID: sc.GetUUID(),
		})
	}
...
func (port *Port) createVirtualMachineInterface(
	...
	if err := port.setPortSecurity(ctx, rp, vmi, vn); err != nil {
		return errors.Wrap(err, "couldn't setup port security")
	}
	...
	if err != nil {
		return errors.Wrap(err, "couldn't create virtual-machine-interface")
	}
