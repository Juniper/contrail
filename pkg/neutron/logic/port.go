...
func (port *Port) ensureVMInstanceExists(
	...
	gr, err := rp.ReadService.GetVirtualMachine(ctx, &services.GetVirtualMachineRequest{
		ID: parseUUID(port.DeviceID),
	})
	if errutil.IsNotFound(err) {
		rp.Log.WithField("uuid", parseUUID(port.DeviceID)).Debug("No virtual machine for port - creating new one")
		...
		if cErr != nil {
			return nil, errors.Wrapf(err, "failed to create new virtual machine (device) %q", parseUUID(port.DeviceID))
		}

		return cr.GetVirtualMachine(), nil
	} else if err != nil {
		return nil, errors.Wrapf(err, "failed to get virtual machine (device) %q", parseUUID(port.DeviceID))
	}
