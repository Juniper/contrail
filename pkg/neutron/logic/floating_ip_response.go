...
func makeFloatingipResponse(
	...
	if err != nil {
		return nil, newFloatingIPError(fmt.Sprintf("could not find uuid of network with fq_name: %v", netFQName),
			err)
	}
	...
	if len(vnRefs) == 0 {
		return nil, newFloatingIPError(
			fmt.Sprintf("missing VirtualNetworkRefs for port with UUID %v when building FloatingIPResponse %v",
				port.GetUUID(),
				fip.GetUUID(),
			), nil)
	}
...
func getTenant(fip *models.FloatingIP) (string, error) {
	...
	return "", newFloatingIPError(
		fmt.Sprintf("could not get tenant from Project refs from FloatingIp with UUID %v", fip.GetUUID()), nil)
}
