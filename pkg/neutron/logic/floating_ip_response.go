...
func makeFloatingipResponse(
	...
	if len(vnRefs) == 0 {
		return nil, errors.Errorf("missing VirtualNetworkRefs for port with UUID %v when building FloatingIPResponse %v", port.GetUUID(), fip.GetUUID())
	}
...
func makeVMIList(ctx context.Context, rp RequestParameters, ports []*models.VirtualMachineInterface, vmiRouters map[string]string) ([]*models.VirtualMachineInterface, error) {
	if len(ports) == 0 {
		if len(vmiRouters) == 0 {
			return nil, nil
		}
